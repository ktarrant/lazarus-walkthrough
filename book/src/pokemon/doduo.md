<div class="pokemon-tabs" id="pokemon-tabs-doduo-84">
<input type="radio" name="pokemon-tabs-doduo-84-group" id="pokemon-tabs-doduo-84-tab-0" checked>
<label for="pokemon-tabs-doduo-84-tab-0">Doduo</label>
<input type="radio" name="pokemon-tabs-doduo-84-group" id="pokemon-tabs-doduo-84-tab-1">
<label for="pokemon-tabs-doduo-84-tab-1">Dodrio</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-doduo-84-panel-0">
## Doduo (#084)
<details class="pokemon-card-container" open>
<summary>Doduo overview</summary>
_Twin Bird Pokémon_ • Types: Normal / Flying
Height: 1.4 m · Weight: 39.2 kg · Base EXP: 62

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Run Away
- Early Bird
- Tangled Feet *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Ground (0×)
- Bug (0.5×)
- Ghost (0×)

*Weak to*
- Electric (2×)
- Ice (2×)
- Rock (2×)

**Evolution Paths**
- Doduo → Dodrio (Lv 31)

**Encounter Locations**
- Fresco Isles — Grass (Day) (10%)
- Fresco Isles — Grass (Night) (10%)
- Kipos Town — Grass (Day) (5%)
- Kipos Town — Grass (Night) (5%)
- Pythios Town — Grass (Day) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 35 |
| Attack | 85 |
| Defense | 45 |
| Sp. Atk | 35 |
| Sp. Def | 35 |
| Speed | 75 |
| Total | 310 |

**Notable Level-Up Moves**
- Growl (Lv 1, Gold Silver)
- Peck (Lv 1, Red Blue)
- Quick Attack (Lv 5, Diamond Pearl)
- Leer (Lv 8, Brilliant Diamond And Shining Pearl)
- Rage (Lv 8, Sun Moon)
- Fury Attack (Lv 9, Scarlet Violet)
</div>
</div>

**Flavor Text**
> A two-headed Pokémon that was discovered as a sudden mutation. It runs at a pace of over 60 miles per hour.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-doduo-84-panel-1">
## Dodrio (#085)
<details class="pokemon-card-container" open>
<summary>Dodrio overview</summary>
_Triple Bird Pokémon_ • Types: Normal / Flying
Height: 1.8 m · Weight: 85.2 kg · Base EXP: 165

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Run Away
- Early Bird
- Tangled Feet *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Ground (0×)
- Bug (0.5×)
- Ghost (0×)

*Weak to*
- Electric (2×)
- Ice (2×)
- Rock (2×)

**Evolution Paths**
- Doduo → Dodrio (Lv 31)

**Encounter Locations**
- Pollen Road — Grass (Day) (5%)
- Wakewater Isle — Grass (Day) (10%)
- Wakewater Isle — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 110 |
| Defense | 70 |
| Sp. Atk | 60 |
| Sp. Def | 60 |
| Speed | 110 |
| Total | 470 |

**Notable Level-Up Moves**
- Fury Attack (Lv 1, Red Blue)
- Growl (Lv 1, Red Blue)
- Mirror Move (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Peck (Lv 1, Red Blue)
- Pluck (Lv 1, Diamond Pearl)
- Pursuit (Lv 1, Gold Silver)
</div>
</div>

**Flavor Text**
> An enemy that takes its eyes off any of the three heads--even for a second--will get pecked severely.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-doduo-84-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-doduo-84-panel-0 { display: block; }
#pokemon-tabs-doduo-84-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-doduo-84-panel-1 { display: block; }
</style>
