<div class="pokemon-tabs" id="pokemon-tabs-pawmo-922">
<input type="radio" name="pokemon-tabs-pawmo-922-group" id="pokemon-tabs-pawmo-922-tab-0">
<label for="pokemon-tabs-pawmo-922-tab-0">Pawmi</label>
<input type="radio" name="pokemon-tabs-pawmo-922-group" id="pokemon-tabs-pawmo-922-tab-1" checked>
<label for="pokemon-tabs-pawmo-922-tab-1">Pawmo</label>
<input type="radio" name="pokemon-tabs-pawmo-922-group" id="pokemon-tabs-pawmo-922-tab-2">
<label for="pokemon-tabs-pawmo-922-tab-2">Pawmot</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-pawmo-922-panel-0">
## Pawmi (#921)
<details class="pokemon-card-container" open>
<summary>Pawmi overview</summary>
_Mouse Pokémon_ • Types: Electric
Height: 0.3 m · Weight: 2.5 kg · Base EXP: 48

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Static
- Natural Cure
- Iron Fist *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Pawmi → Pawmo (Lv 18) → Pawmot (Other)

**Encounter Locations**
- Asfal Hills — Grass (Day) (15%)
- Asfal Hills — Grass (Night) (15%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 45 |
| Attack | 50 |
| Defense | 20 |
| Sp. Atk | 40 |
| Sp. Def | 25 |
| Speed | 60 |
| Total | 240 |

**Notable Level-Up Moves**
- Growl (Lv 1, Scarlet Violet)
- Scratch (Lv 1, Scarlet Violet)
- Thunder Shock (Lv 3, Scarlet Violet)
- Quick Attack (Lv 6, Scarlet Violet)
- Charge (Lv 8, Scarlet Violet)
- Nuzzle (Lv 12, Scarlet Violet)
</div>
</div>

**Flavor Text**
> The pads of its paws are electricity-discharging organs. Pawmi fires electricity from its forepaws while standing unsteadily on its hind legs.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-pawmo-922-panel-1">
## Pawmo (#922)
<details class="pokemon-card-container" open>
<summary>Pawmo overview</summary>
_Mouse Pokémon_ • Types: Electric / Fighting
Height: 0.4 m · Weight: 6.5 kg · Base EXP: 123

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Volt Absorb
- Natural Cure
- Iron Fist *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Bug (0.5×)
- Rock (0.5×)
- Dark (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)
- Psychic (2×)
- Fairy (2×)

**Evolution Paths**
- Pawmi → Pawmo (Lv 18) → Pawmot (Other)

**Encounter Locations**
- Palati City — Grass (Day) (10%)
- Péntepetal City — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 75 |
| Defense | 40 |
| Sp. Atk | 50 |
| Sp. Def | 40 |
| Speed | 85 |
| Total | 350 |

**Notable Level-Up Moves**
- Growl (Lv 1, Scarlet Violet)
- Scratch (Lv 1, Scarlet Violet)
- Thunder Shock (Lv 3, Scarlet Violet)
- Quick Attack (Lv 6, Scarlet Violet)
- Charge (Lv 8, Scarlet Violet)
- Nuzzle (Lv 12, Scarlet Violet)
</div>
</div>

**Flavor Text**
> Pawmo uses a unique fighting technique in which it uses its forepaws to strike foes and zap them with electricity from its paw pads simultaneously.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-pawmo-922-panel-2">
## Pawmot (#923)
<details class="pokemon-card-container" open>
<summary>Pawmot overview</summary>
_Hands-On Pokémon_ • Types: Electric / Fighting
Height: 0.9 m · Weight: 41.0 kg · Base EXP: 245

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Volt Absorb
- Natural Cure
- Iron Fist *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Bug (0.5×)
- Rock (0.5×)
- Dark (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)
- Psychic (2×)
- Fairy (2×)

**Evolution Paths**
- Pawmi → Pawmo (Lv 18) → Pawmot (Other)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 70 |
| Attack | 115 |
| Defense | 70 |
| Sp. Atk | 70 |
| Sp. Def | 60 |
| Speed | 105 |
| Total | 490 |

**Notable Level-Up Moves**
- Growl (Lv 1, Scarlet Violet)
- Scratch (Lv 1, Scarlet Violet)
- Wild Charge (Lv 1, Scarlet Violet)
- Thunder Shock (Lv 3, Scarlet Violet)
- Quick Attack (Lv 6, Scarlet Violet)
- Charge (Lv 8, Scarlet Violet)
</div>
</div>

**Flavor Text**
> Pawmot’s fluffy fur acts as a battery. It can store the same amount of electricity as an electric car.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-pawmo-922-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-pawmo-922-panel-0 { display: block; }
#pokemon-tabs-pawmo-922-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-pawmo-922-panel-1 { display: block; }
#pokemon-tabs-pawmo-922-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-pawmo-922-panel-2 { display: block; }
</style>
