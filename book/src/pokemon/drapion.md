<div class="pokemon-tabs" id="pokemon-tabs-drapion-452">
<input type="radio" name="pokemon-tabs-drapion-452-group" id="pokemon-tabs-drapion-452-tab-0">
<label for="pokemon-tabs-drapion-452-tab-0">Skorupi</label>
<input type="radio" name="pokemon-tabs-drapion-452-group" id="pokemon-tabs-drapion-452-tab-1" checked>
<label for="pokemon-tabs-drapion-452-tab-1">Drapion</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-drapion-452-panel-0">
## Skorupi (#451)
<details class="pokemon-card-container" open>
<summary>Skorupi overview</summary>
_Scorpion Pokémon_ • Types: Poison / Bug
Height: 0.8 m · Weight: 12.0 kg · Base EXP: 66

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Battle Armor
- Sniper
- Keen Eye *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.25×)
- Fighting (0.25×)
- Poison (0.5×)
- Bug (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Flying (2×)
- Psychic (2×)
- Rock (2×)

**Evolution Paths**
- Skorupi → Drapion (Lv 40)

**Encounter Locations**
- Erinys Path (East) — Grass (Night) (10%)
- Fresco Isles — Grass (Day) (4%)
- Fresco Isles — Grass (Night) (4%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 40 |
| Attack | 50 |
| Defense | 90 |
| Sp. Atk | 30 |
| Sp. Def | 55 |
| Speed | 65 |
| Total | 330 |

**Notable Level-Up Moves**
- Bite (Lv 1, Diamond Pearl)
- Leer (Lv 1, Platinum)
- Poison Sting (Lv 1, Diamond Pearl)
- Hone Claws (Lv 3, Sword Shield)
- Knock Off (Lv 5, Black 2 White 2)
- Fell Stinger (Lv 6, Sword Shield)
</div>
</div>

**Flavor Text**
> After burrowing into the sand, it waits patiently for prey to come near. This Pokémon and Sizzlipede share common descent.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-drapion-452-panel-1">
## Drapion (#452)
<details class="pokemon-card-container" open>
<summary>Drapion overview</summary>
_Ogre Scorpion Pokémon_ • Types: Poison / Dark
Height: 1.3 m · Weight: 61.5 kg · Base EXP: 175

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Battle Armor
- Sniper
- Keen Eye *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Poison (0.5×)
- Psychic (0×)
- Ghost (0.5×)
- Dark (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Skorupi → Drapion (Lv 40)

**Encounter Locations**
- Areios Hideout — Grass (Night) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 70 |
| Attack | 90 |
| Defense | 110 |
| Sp. Atk | 60 |
| Sp. Def | 75 |
| Speed | 95 |
| Total | 500 |

**Notable Level-Up Moves**
- Bite (Lv 1, Diamond Pearl)
- Fell Stinger (Lv 1, Sword Shield)
- Fire Fang (Lv 1, Diamond Pearl)
- Hone Claws (Lv 1, Sword Shield)
- Ice Fang (Lv 1, Diamond Pearl)
- Knock Off (Lv 1, Platinum)
</div>
</div>

**Flavor Text**
> Its poison is potent, but it rarely sees use. This Pokémon prefers to use physical force instead, going on rampages with its car-crushing strength.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-drapion-452-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-drapion-452-panel-0 { display: block; }
#pokemon-tabs-drapion-452-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-drapion-452-panel-1 { display: block; }
</style>
