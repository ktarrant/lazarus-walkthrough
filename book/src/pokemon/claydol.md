<div class="pokemon-tabs" id="pokemon-tabs-claydol-344">
<input type="radio" name="pokemon-tabs-claydol-344-group" id="pokemon-tabs-claydol-344-tab-0">
<label for="pokemon-tabs-claydol-344-tab-0">Baltoy</label>
<input type="radio" name="pokemon-tabs-claydol-344-group" id="pokemon-tabs-claydol-344-tab-1" checked>
<label for="pokemon-tabs-claydol-344-tab-1">Claydol</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-claydol-344-panel-0">
## Baltoy (#343)
<details class="pokemon-card-container" open>
<summary>Baltoy overview</summary>
_Clay Doll Pokémon_ • Types: Ground / Psychic • Egg Groups: Mineral • Base EXP: 60

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Levitate

**Type Matchups**

*Resists / Immune to*
- Electric (0×)
- Fighting (0.5×)
- Poison (0.5×)
- Psychic (0.5×)
- Rock (0.5×)

*Weak to*
- Water (2×)
- Grass (2×)
- Ice (2×)
- Bug (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Baltoy → Claydol (Lv 36)

**Encounter Locations**
- Erinys Path (West) — Grass (Day) (10%)
- Erinys Path (West) — Grass (Night) (20%)
- Palati City — Grass (Day) (20%)
- Palati City — Grass (Night) (20%)
- Sofos City — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">40</span> |
| Attack | <span class="stat-value stat-low">40</span> |
| Defense | <span class="stat-value stat-mid">55</span> |
| Sp. Atk | <span class="stat-value stat-low">40</span> |
| Sp. Def | <span class="stat-value stat-mid">70</span> |
| Speed | <span class="stat-value stat-mid">55</span> |
| Total | <span class="stat-value stat-low">300</span> |

**Notable Level-Up Moves**
- Harden (Lv 1)
- Mud Slap (Lv 1)
- Rapid Spin (Lv 3)
- Confusion (Lv 6)
- Rock Tomb (Lv 9)
- Power Trick (Lv 12)
- Psybeam (Lv 15)
- Ancient Power (Lv 18)
- Imprison (Lv 21)
- Cosmic Power (Lv 24)
- Extrasensory (Lv 27)
- Earth Power (Lv 30)
- Self Destruct (Lv 33)
- Guard Split (Lv 36)
- Power Split (Lv 36)
- Sandstorm (Lv 39)
- Explosion (Lv 42)
</div>
</div>

**Flavor Text**
> It moves while spinning around on its single foot. Some Baltoy have been seen spinning on their heads.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-claydol-344-panel-1">
## Claydol (#344)
<details class="pokemon-card-container" open>
<summary>Claydol overview</summary>
_Clay Doll Pokémon_ • Types: Ground / Psychic • Egg Groups: Mineral • Base EXP: 175

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Levitate

**Type Matchups**

*Resists / Immune to*
- Electric (0×)
- Fighting (0.5×)
- Poison (0.5×)
- Psychic (0.5×)
- Rock (0.5×)

*Weak to*
- Water (2×)
- Grass (2×)
- Ice (2×)
- Bug (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Baltoy → Claydol (Lv 36)

**Encounter Locations**
- Kaptara Island (West) — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">60</span> |
| Attack | <span class="stat-value stat-mid">70</span> |
| Defense | <span class="stat-value stat-high">105</span> |
| Sp. Atk | <span class="stat-value stat-mid">70</span> |
| Sp. Def | <span class="stat-value stat-high">120</span> |
| Speed | <span class="stat-value stat-mid">75</span> |
| Total | <span class="stat-value stat-mid">500</span> |

**Notable Level-Up Moves**
- Confusion (Lv 1)
- Harden (Lv 1)
- Hyper Beam (Lv 1)
- Mud Slap (Lv 1)
- Rapid Spin (Lv 1)
- Teleport (Lv 1)
- Rock Tomb (Lv 9)
- Power Trick (Lv 12)
- Psybeam (Lv 15)
- Ancient Power (Lv 18)
- Imprison (Lv 21)
- Cosmic Power (Lv 24)
- Extrasensory (Lv 27)
- Earth Power (Lv 30)
- Self Destruct (Lv 33)
- Guard Split (Lv 38)
- Power Split (Lv 38)
- Sandstorm (Lv 43)
- Explosion (Lv 48)
</div>
</div>

**Flavor Text**
> This mysterious Pokémon started life as an ancient clay figurine made over 20,000 years ago.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-claydol-344-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-claydol-344-panel-0 { display: block; }
#pokemon-tabs-claydol-344-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-claydol-344-panel-1 { display: block; }
</style>
