<div class="pokemon-tabs" id="pokemon-tabs-baltoy-343">
<input type="radio" name="pokemon-tabs-baltoy-343-group" id="pokemon-tabs-baltoy-343-tab-0" checked>
<label for="pokemon-tabs-baltoy-343-tab-0">Baltoy</label>
<input type="radio" name="pokemon-tabs-baltoy-343-group" id="pokemon-tabs-baltoy-343-tab-1">
<label for="pokemon-tabs-baltoy-343-tab-1">Claydol</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-baltoy-343-panel-0">
## Baltoy (#343)
<details class="pokemon-card-container" open>
<summary>Baltoy overview</summary>
_Clay Doll Pokémon_ • Types: Ground / Psychic
Height: 0.5 m · Weight: 21.5 kg · Base EXP: 60

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Levitate

**Type Matchups**

*Resists / Immune to*
- Electric (0×)
- Fighting (0.5×)
- Poison (0.5×)
- Psychic (0.5×)
- Rock (0.5×)

*Weak to*
- Water (2×)
- Grass (2×)
- Ice (2×)
- Bug (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Baltoy → Claydol (Lv 36)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 40 |
| Attack | 40 |
| Defense | 55 |
| Sp. Atk | 40 |
| Sp. Def | 70 |
| Speed | 55 |
| Total | 300 |

**Notable Level-Up Moves**
- Confusion (Lv 1, Ruby Sapphire)
- Harden (Lv 1, Black 2 White 2)
- Mud Slap (Lv 1, Sword Shield)
- Rapid Spin (Lv 3, Sword Shield)
- Rock Tomb (Lv 9, Sword Shield)
- Heal Block (Lv 10, Omega Ruby Alpha Sapphire)
</div>
</div>

**Flavor Text**
> It moves while spinning around on its single foot. Some Baltoy have been seen spinning on their heads.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-baltoy-343-panel-1">
## Claydol (#344)
<details class="pokemon-card-container" open>
<summary>Claydol overview</summary>
_Clay Doll Pokémon_ • Types: Ground / Psychic
Height: 1.5 m · Weight: 108.0 kg · Base EXP: 175

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Levitate

**Type Matchups**

*Resists / Immune to*
- Electric (0×)
- Fighting (0.5×)
- Poison (0.5×)
- Psychic (0.5×)
- Rock (0.5×)

*Weak to*
- Water (2×)
- Grass (2×)
- Ice (2×)
- Bug (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Baltoy → Claydol (Lv 36)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 70 |
| Defense | 105 |
| Sp. Atk | 70 |
| Sp. Def | 120 |
| Speed | 75 |
| Total | 500 |

**Notable Level-Up Moves**
- Confusion (Lv 1, Ruby Sapphire)
- Harden (Lv 1, Ruby Sapphire)
- Hyper Beam (Lv 1, Sun Moon)
- Mud Slap (Lv 1, Sword Shield)
- Rapid Spin (Lv 1, Ruby Sapphire)
- Teleport (Lv 1, Ruby Sapphire)
</div>
</div>

**Flavor Text**
> This mysterious Pokémon started life as an ancient clay figurine made over 20,000 years ago.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-baltoy-343-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-baltoy-343-panel-0 { display: block; }
#pokemon-tabs-baltoy-343-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-baltoy-343-panel-1 { display: block; }
</style>
