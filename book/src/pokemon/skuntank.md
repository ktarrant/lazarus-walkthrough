<div class="pokemon-tabs" id="pokemon-tabs-skuntank-435">
<input type="radio" name="pokemon-tabs-skuntank-435-group" id="pokemon-tabs-skuntank-435-tab-0">
<label for="pokemon-tabs-skuntank-435-tab-0">Stunky</label>
<input type="radio" name="pokemon-tabs-skuntank-435-group" id="pokemon-tabs-skuntank-435-tab-1" checked>
<label for="pokemon-tabs-skuntank-435-tab-1">Skuntank</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-skuntank-435-panel-0">
## Stunky (#434)
<details class="pokemon-card-container" open>
<summary>Stunky overview</summary>
_Skunk Pokémon_ • Types: Poison / Dark
Height: 0.4 m · Weight: 19.2 kg · Base EXP: 66

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Stench
- Aftermath
- Keen Eye *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Poison (0.5×)
- Psychic (0×)
- Ghost (0.5×)
- Dark (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Stunky → Skuntank (Lv 34)

**Encounter Locations**
- Asfal Hills — Grass (Night) (6%)
- Kipos Town — Grass (Night) (20%)
- Pythios Cemetery — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 63 |
| Attack | 63 |
| Defense | 47 |
| Sp. Atk | 41 |
| Sp. Def | 41 |
| Speed | 74 |
| Total | 329 |

**Notable Level-Up Moves**
- Focus Energy (Lv 1, Diamond Pearl)
- Poison Gas (Lv 1, Sword Shield)
- Scratch (Lv 1, Diamond Pearl)
- Tackle (Lv 1, Legends Arceus)
- Feint (Lv 3, Sword Shield)
- Acid Spray (Lv 6, Legends Arceus)
</div>
</div>

**Flavor Text**
> From its rear, it sprays a foul-smelling liquid at opponents. It aims for their faces, and it can hit them from over 16 feet away.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-skuntank-435-panel-1">
## Skuntank (#435)
<details class="pokemon-card-container" open>
<summary>Skuntank overview</summary>
_Skunk Pokémon_ • Types: Poison / Dark
Height: 1.0 m · Weight: 38.0 kg · Base EXP: 168

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Stench
- Aftermath
- Keen Eye *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Poison (0.5×)
- Psychic (0×)
- Ghost (0.5×)
- Dark (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Stunky → Skuntank (Lv 34)

**Encounter Locations**
- Kaptara Island (East) — Grass (Night) (4%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 103 |
| Attack | 93 |
| Defense | 67 |
| Sp. Atk | 71 |
| Sp. Def | 61 |
| Speed | 84 |
| Total | 479 |

**Notable Level-Up Moves**
- Feint (Lv 1, Sword Shield)
- Flamethrower (Lv 1, Sun Moon)
- Focus Energy (Lv 1, Diamond Pearl)
- Poison Gas (Lv 1, Diamond Pearl)
- Scratch (Lv 1, Diamond Pearl)
- Screech (Lv 1, Sun Moon)
</div>
</div>

**Flavor Text**
> In its belly, it reserves stinky fluid that it shoots from its tail during battle. As this Pokémon’s diet varies, so does the stench of its fluid.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-skuntank-435-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-skuntank-435-panel-0 { display: block; }
#pokemon-tabs-skuntank-435-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-skuntank-435-panel-1 { display: block; }
</style>
