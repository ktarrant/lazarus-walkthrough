<div class="pokemon-tabs" id="pokemon-tabs-stunky-434">
<input type="radio" name="pokemon-tabs-stunky-434-group" id="pokemon-tabs-stunky-434-tab-0" checked>
<label for="pokemon-tabs-stunky-434-tab-0">Stunky</label>
<input type="radio" name="pokemon-tabs-stunky-434-group" id="pokemon-tabs-stunky-434-tab-1">
<label for="pokemon-tabs-stunky-434-tab-1">Skuntank</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-stunky-434-panel-0">
## Stunky (#434)
<details class="pokemon-card-container" open>
<summary>Stunky overview</summary>
_Skunk Pokémon_ • Types: Poison / Dark • Egg Groups: Ground • Base EXP: 66

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Stench
- Aftermath
- Keen Eye *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Poison (0.5×)
- Psychic (0×)
- Ghost (0.5×)
- Dark (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Stunky → Skuntank (Lv 34)

**Encounter Locations**
- Asfal Hills — Grass (Night) (6%)
- Kipos Town — Grass (Night) (20%)
- Pythios Cemetery — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">63</span> |
| Attack | <span class="stat-value stat-mid">63</span> |
| Defense | <span class="stat-value stat-low">47</span> |
| Sp. Atk | <span class="stat-value stat-low">41</span> |
| Sp. Def | <span class="stat-value stat-low">41</span> |
| Speed | <span class="stat-value stat-mid">74</span> |
| Total | <span class="stat-value stat-mid">329</span> |
</div>
</div>

**Flavor Text**
> From its rear, it sprays a foul-smelling liquid at opponents. It aims for their faces, and it can hit them from over 16 feet away.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-stunky-434-panel-1">
## Skuntank (#435)
<details class="pokemon-card-container" open>
<summary>Skuntank overview</summary>
_Skunk Pokémon_ • Types: Poison / Dark • Egg Groups: Ground • Base EXP: 168

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Stench
- Aftermath
- Keen Eye *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Poison (0.5×)
- Psychic (0×)
- Ghost (0.5×)
- Dark (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Stunky → Skuntank (Lv 34)

**Encounter Locations**
- Kaptara Island (East) — Grass (Night) (4%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-high">103</span> |
| Attack | <span class="stat-value stat-high">93</span> |
| Defense | <span class="stat-value stat-mid">67</span> |
| Sp. Atk | <span class="stat-value stat-mid">71</span> |
| Sp. Def | <span class="stat-value stat-mid">61</span> |
| Speed | <span class="stat-value stat-mid">84</span> |
| Total | <span class="stat-value stat-mid">479</span> |
</div>
</div>

**Flavor Text**
> In its belly, it reserves stinky fluid that it shoots from its tail during battle. As this Pokémon’s diet varies, so does the stench of its fluid.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-stunky-434-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-stunky-434-panel-0 { display: block; }
#pokemon-tabs-stunky-434-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-stunky-434-panel-1 { display: block; }
</style>
