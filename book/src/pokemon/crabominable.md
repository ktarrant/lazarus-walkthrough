<div class="pokemon-tabs" id="pokemon-tabs-crabominable-740">
<input type="radio" name="pokemon-tabs-crabominable-740-group" id="pokemon-tabs-crabominable-740-tab-0">
<label for="pokemon-tabs-crabominable-740-tab-0">Crabrawler</label>
<input type="radio" name="pokemon-tabs-crabominable-740-group" id="pokemon-tabs-crabominable-740-tab-1" checked>
<label for="pokemon-tabs-crabominable-740-tab-1">Crabominable</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-crabominable-740-panel-0">
## Crabrawler (#739)
<details class="pokemon-card-container" open>
<summary>Crabrawler overview</summary>
_Boxing Pokémon_ • Types: Fighting
Height: 0.6 m · Weight: 7.0 kg · Base EXP: 68

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Hyper Cutter
- Iron Fist
- Anger Point *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Bug (0.5×)
- Rock (0.5×)
- Dark (0.5×)

*Weak to*
- Flying (2×)
- Psychic (2×)
- Fairy (2×)

**Evolution Paths**
- Crabrawler → Crabominable (Level at Mount Lanakila)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 47 |
| Attack | 82 |
| Defense | 57 |
| Sp. Atk | 42 |
| Sp. Def | 47 |
| Speed | 63 |
| Total | 338 |

**Notable Level-Up Moves**
- Bubble (Lv 1, Sun Moon)
- Rock Smash (Lv 5, Sun Moon)
- Leer (Lv 9, Sun Moon)
- Bubble Beam (Lv 13, Scarlet Violet)
- Pursuit (Lv 13, Sun Moon)
- Protect (Lv 17, Scarlet Violet)
</div>
</div>

**Flavor Text**
> While guarding its weak points with its pincers, it looks for an opening and unleashes punches. When it loses, it foams at the mouth and faints.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-crabominable-740-panel-1">
## Crabominable (#740)
<details class="pokemon-card-container" open>
<summary>Crabominable overview</summary>
_Woolly Crab Pokémon_ • Types: Fighting / Ice
Height: 1.7 m · Weight: 180.0 kg · Base EXP: 167

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Hyper Cutter
- Iron Fist
- Anger Point *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Ice (0.5×)
- Bug (0.5×)
- Dark (0.5×)

*Weak to*
- Fire (2×)
- Fighting (2×)
- Flying (2×)
- Psychic (2×)
- Steel (2×)
- Fairy (2×)

**Evolution Paths**
- Crabrawler → Crabominable (Level at Mount Lanakila)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 97 |
| Attack | 132 |
| Defense | 77 |
| Sp. Atk | 62 |
| Sp. Def | 67 |
| Speed | 43 |
| Total | 478 |

**Notable Level-Up Moves**
- Bubble (Lv 1, Sun Moon)
- Ice Punch (Lv 1, Sun Moon)
- Leer (Lv 1, Sun Moon)
- Protect (Lv 1, Scarlet Violet)
- Pursuit (Lv 1, Sun Moon)
- Rock Smash (Lv 1, Sun Moon)
</div>
</div>

**Flavor Text**
> It aimed for the top but got lost and ended up on a snowy mountain. Being forced to endure the cold, this Pokémon evolved and grew fur.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-crabominable-740-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-crabominable-740-panel-0 { display: block; }
#pokemon-tabs-crabominable-740-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-crabominable-740-panel-1 { display: block; }
</style>
