<div class="pokemon-tabs" id="pokemon-tabs-doublade-680">
<input type="radio" name="pokemon-tabs-doublade-680-group" id="pokemon-tabs-doublade-680-tab-0">
<label for="pokemon-tabs-doublade-680-tab-0">Honedge</label>
<input type="radio" name="pokemon-tabs-doublade-680-group" id="pokemon-tabs-doublade-680-tab-1" checked>
<label for="pokemon-tabs-doublade-680-tab-1">Doublade</label>
<input type="radio" name="pokemon-tabs-doublade-680-group" id="pokemon-tabs-doublade-680-tab-2">
<label for="pokemon-tabs-doublade-680-tab-2">Aegislash Shield</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-doublade-680-panel-0">
## Honedge (#679)
<details class="pokemon-card-container" open>
<summary>Honedge overview</summary>
_Sword Pokémon_ • Types: Steel / Ghost
Height: 0.8 m · Weight: 2.0 kg · Base EXP: 65

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- No Guard

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Ice (0.5×)
- Fighting (0×)
- Poison (0×)
- Flying (0.5×)
- Psychic (0.5×)
- Bug (0.25×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Ground (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Honedge → Doublade (Lv 35) → Aegislash (Use Dusk Stone)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 45 |
| Attack | 80 |
| Defense | 100 |
| Sp. Atk | 35 |
| Sp. Def | 37 |
| Speed | 28 |
| Total | 325 |

**Notable Level-Up Moves**
- Fury Cutter (Lv 1, Sword Shield)
- Swords Dance (Lv 1, X Y)
- Tackle (Lv 1, X Y)
- Shadow Sneak (Lv 4, Sword Shield)
- Autotomize (Lv 8, Sword Shield)
- Metal Sound (Lv 8, X Y)
</div>
</div>

**Flavor Text**
> Honedge’s soul once belonged to a person who was killed a long time ago by the sword that makes up Honedge’s body.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-doublade-680-panel-1">
## Doublade (#680)
<details class="pokemon-card-container" open>
<summary>Doublade overview</summary>
_Sword Pokémon_ • Types: Steel / Ghost
Height: 0.8 m · Weight: 4.5 kg · Base EXP: 157

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- No Guard

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Ice (0.5×)
- Fighting (0×)
- Poison (0×)
- Flying (0.5×)
- Psychic (0.5×)
- Bug (0.25×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Ground (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Honedge → Doublade (Lv 35) → Aegislash (Use Dusk Stone)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 59 |
| Attack | 110 |
| Defense | 150 |
| Sp. Atk | 45 |
| Sp. Def | 49 |
| Speed | 35 |
| Total | 448 |

**Notable Level-Up Moves**
- Autotomize (Lv 1, Sword Shield)
- Fury Cutter (Lv 1, Sun Moon)
- Shadow Sneak (Lv 1, Sword Shield)
- Swords Dance (Lv 1, X Y)
- Tackle (Lv 1, X Y)
- Metal Sound (Lv 8, X Y)
</div>
</div>

**Flavor Text**
> Honedge evolves into twins. The two blades rub together to emit a metallic sound that unnerves opponents.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-doublade-680-panel-2">
## Aegislash Shield (#681)
<details class="pokemon-card-container" open>
<summary>Aegislash Shield overview</summary>
_Royal Sword Pokémon_ • Types: Steel / Ghost
Height: 1.7 m · Weight: 53.0 kg · Base EXP: 234

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Stance Change

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Ice (0.5×)
- Fighting (0×)
- Poison (0×)
- Flying (0.5×)
- Psychic (0.5×)
- Bug (0.25×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Ground (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Honedge → Doublade (Lv 35) → Aegislash (Use Dusk Stone)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 50 |
| Defense | 140 |
| Sp. Atk | 50 |
| Sp. Def | 140 |
| Speed | 60 |
| Total | 500 |

**Notable Level-Up Moves**
- Aerial Ace (Lv 1, X Y)
- Autotomize (Lv 1, X Y)
- Fury Cutter (Lv 1, X Y)
- Head Smash (Lv 1, X Y)
- Iron Defense (Lv 1, X Y)
- Iron Head (Lv 1, X Y)
</div>
</div>

**Flavor Text**
> In this defensive stance, Aegislash uses its steel body and a force field of spectral power to reduce the damage of any attack.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-doublade-680-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-doublade-680-panel-0 { display: block; }
#pokemon-tabs-doublade-680-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-doublade-680-panel-1 { display: block; }
#pokemon-tabs-doublade-680-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-doublade-680-panel-2 { display: block; }
</style>
