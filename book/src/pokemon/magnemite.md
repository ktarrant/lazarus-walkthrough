<div class="pokemon-tabs" id="pokemon-tabs-magnemite-81">
<input type="radio" name="pokemon-tabs-magnemite-81-group" id="pokemon-tabs-magnemite-81-tab-0" checked>
<label for="pokemon-tabs-magnemite-81-tab-0">Magnemite</label>
<input type="radio" name="pokemon-tabs-magnemite-81-group" id="pokemon-tabs-magnemite-81-tab-1">
<label for="pokemon-tabs-magnemite-81-tab-1">Magneton</label>
<input type="radio" name="pokemon-tabs-magnemite-81-group" id="pokemon-tabs-magnemite-81-tab-2">
<label for="pokemon-tabs-magnemite-81-tab-2">Magnezone</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-magnemite-81-panel-0">
## Magnemite (#081)
<details class="pokemon-card-container" open>
<summary>Magnemite overview</summary>
_Magnet Pokémon_ • Types: Electric / Steel
Height: 0.3 m · Weight: 6.0 kg · Base EXP: 65

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Magnet Pull
- Sturdy
- Analytic *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ice (0.5×)
- Poison (0×)
- Flying (0.25×)
- Psychic (0.5×)
- Bug (0.5×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.25×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Fighting (2×)
- Ground (4×)

**Evolution Paths**
- Magnemite → Magneton (Lv 30) → Magnezone (Level at Mt Coronet)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 25 |
| Attack | 35 |
| Defense | 70 |
| Sp. Atk | 95 |
| Sp. Def | 55 |
| Speed | 45 |
| Total | 325 |

**Notable Level-Up Moves**
- Metal Sound (Lv 1, Ruby Sapphire)
- Supersonic (Lv 1, Sun Moon)
- Tackle (Lv 1, Red Blue)
- Thunder Shock (Lv 1, Sword Shield)
- Magnet Bomb (Lv 7, Sun Moon)
- Thunder Wave (Lv 7, Ultra Sun Ultra Moon)
</div>
</div>

**Flavor Text**
> At times, Magnemite runs out of electricity and ends up on the ground. If you give batteries to a grounded Magnemite, it’ll start moving again.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-magnemite-81-panel-1">
## Magneton (#082)
<details class="pokemon-card-container" open>
<summary>Magneton overview</summary>
_Magnet Pokémon_ • Types: Electric / Steel
Height: 1.0 m · Weight: 60.0 kg · Base EXP: 163

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Magnet Pull
- Sturdy
- Analytic *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ice (0.5×)
- Poison (0×)
- Flying (0.25×)
- Psychic (0.5×)
- Bug (0.5×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.25×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Fighting (2×)
- Ground (4×)

**Evolution Paths**
- Magnemite → Magneton (Lv 30) → Magnezone (Level at Mt Coronet)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 50 |
| Attack | 60 |
| Defense | 95 |
| Sp. Atk | 120 |
| Sp. Def | 70 |
| Speed | 70 |
| Total | 465 |

**Notable Level-Up Moves**
- Electric Terrain (Lv 1, Omega Ruby Alpha Sapphire)
- Magnet Bomb (Lv 1, Sun Moon)
- Metal Sound (Lv 1, Ruby Sapphire)
- Sonic Boom (Lv 1, Red Blue)
- Supersonic (Lv 1, Gold Silver)
- Tackle (Lv 1, Red Blue)
</div>
</div>

**Flavor Text**
> This Pokémon is three Magnemite that have linked together. Magneton sends out powerful radio waves to study its surroundings.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-magnemite-81-panel-2">
## Magnezone (#462)
<details class="pokemon-card-container" open>
<summary>Magnezone overview</summary>
_Magnet Area Pokémon_ • Types: Electric / Steel
Height: 1.2 m · Weight: 180.0 kg · Base EXP: 241

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Magnet Pull
- Sturdy
- Analytic *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ice (0.5×)
- Poison (0×)
- Flying (0.25×)
- Psychic (0.5×)
- Bug (0.5×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.25×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Fighting (2×)
- Ground (4×)

**Evolution Paths**
- Magnemite → Magneton (Lv 30) → Magnezone (Level at Mt Coronet)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 70 |
| Attack | 70 |
| Defense | 115 |
| Sp. Atk | 130 |
| Sp. Def | 90 |
| Speed | 60 |
| Total | 535 |

**Notable Level-Up Moves**
- Barrier (Lv 1, Diamond Pearl)
- Electric Terrain (Lv 1, Omega Ruby Alpha Sapphire)
- Magnet Bomb (Lv 1, Sun Moon)
- Magnetic Flux (Lv 1, X Y)
- Metal Sound (Lv 1, Diamond Pearl)
- Mirror Coat (Lv 1, Diamond Pearl)
</div>
</div>

**Flavor Text**
> Some say that Magnezone receives signals from space via the antenna on its head and that it’s being controlled by some mysterious being.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-magnemite-81-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-magnemite-81-panel-0 { display: block; }
#pokemon-tabs-magnemite-81-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-magnemite-81-panel-1 { display: block; }
#pokemon-tabs-magnemite-81-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-magnemite-81-panel-2 { display: block; }
</style>
