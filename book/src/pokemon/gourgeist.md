<div class="pokemon-tabs" id="pokemon-tabs-gourgeist-average-711">
<input type="radio" name="pokemon-tabs-gourgeist-average-711-group" id="pokemon-tabs-gourgeist-average-711-tab-0">
<label for="pokemon-tabs-gourgeist-average-711-tab-0">Pumpkaboo Average</label>
<input type="radio" name="pokemon-tabs-gourgeist-average-711-group" id="pokemon-tabs-gourgeist-average-711-tab-1" checked>
<label for="pokemon-tabs-gourgeist-average-711-tab-1">Gourgeist Average</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-gourgeist-average-711-panel-0">
## Pumpkaboo Average (#710)
<details class="pokemon-card-container" open>
<summary>Pumpkaboo Average overview</summary>
_Pumpkin Pokémon_ • Types: Ghost / Grass • Egg Groups: Indeterminate • Base EXP: 67

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Pickup
- Frisk
- Insomnia *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Fighting (0×)
- Ground (0.5×)

*Weak to*
- Fire (2×)
- Ice (2×)
- Flying (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Pumpkaboo → Gourgeist (Trade)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">49</span> |
| Attack | <span class="stat-value stat-mid">66</span> |
| Defense | <span class="stat-value stat-mid">70</span> |
| Sp. Atk | <span class="stat-value stat-low">44</span> |
| Sp. Def | <span class="stat-value stat-mid">55</span> |
| Speed | <span class="stat-value stat-mid">51</span> |
| Total | <span class="stat-value stat-mid">335</span> |
</div>
</div>

**Flavor Text**
> Spirits that wander this world are placed into Pumpkaboo’s body. They’re then moved on to the afterlife.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-gourgeist-average-711-panel-1">
## Gourgeist Average (#711)
<details class="pokemon-card-container" open>
<summary>Gourgeist Average overview</summary>
_Pumpkin Pokémon_ • Types: Ghost / Grass • Egg Groups: Indeterminate • Base EXP: 173

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Pickup
- Frisk
- Insomnia *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Fighting (0×)
- Ground (0.5×)

*Weak to*
- Fire (2×)
- Ice (2×)
- Flying (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Pumpkaboo → Gourgeist (Trade)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">65</span> |
| Attack | <span class="stat-value stat-mid">90</span> |
| Defense | <span class="stat-value stat-high">122</span> |
| Sp. Atk | <span class="stat-value stat-mid">58</span> |
| Sp. Def | <span class="stat-value stat-mid">75</span> |
| Speed | <span class="stat-value stat-mid">84</span> |
| Total | <span class="stat-value stat-mid">494</span> |
</div>
</div>

**Flavor Text**
> Eerie cries emanate from its body in the dead of night. The sounds are said to be the wails of spirits who are suffering in the afterlife.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-gourgeist-average-711-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-gourgeist-average-711-panel-0 { display: block; }
#pokemon-tabs-gourgeist-average-711-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-gourgeist-average-711-panel-1 { display: block; }
</style>
