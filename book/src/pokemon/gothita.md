<div class="pokemon-tabs" id="pokemon-tabs-gothita-574">
<input type="radio" name="pokemon-tabs-gothita-574-group" id="pokemon-tabs-gothita-574-tab-0" checked>
<label for="pokemon-tabs-gothita-574-tab-0">Gothita</label>
<input type="radio" name="pokemon-tabs-gothita-574-group" id="pokemon-tabs-gothita-574-tab-1">
<label for="pokemon-tabs-gothita-574-tab-1">Gothorita</label>
<input type="radio" name="pokemon-tabs-gothita-574-group" id="pokemon-tabs-gothita-574-tab-2">
<label for="pokemon-tabs-gothita-574-tab-2">Gothitelle</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-gothita-574-panel-0">
## Gothita (#574)
<details class="pokemon-card-container" open>
<summary>Gothita overview</summary>
_Fixation Pokémon_ • Types: Psychic • Egg Groups: Humanshape • Base EXP: 58

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Frisk
- Competitive
- Shadow Tag *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fighting (0.5×)
- Psychic (0.5×)

*Weak to*
- Bug (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Gothita → Gothorita (Lv 32) → Gothitelle (Lv 41)

**Encounter Locations**
- Erinys Path (West) — Grass (Night) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">45</span> |
| Attack | <span class="stat-value stat-low">30</span> |
| Defense | <span class="stat-value stat-low">50</span> |
| Sp. Atk | <span class="stat-value stat-mid">55</span> |
| Sp. Def | <span class="stat-value stat-mid">65</span> |
| Speed | <span class="stat-value stat-low">45</span> |
| Total | <span class="stat-value stat-low">290</span> |

**Notable Level-Up Moves**
- Confusion (Lv 1)
- Pound (Lv 1)
- Play Nice (Lv 4)
- Tickle (Lv 8)
- Psybeam (Lv 12)
- Charm (Lv 16)
- Psyshock (Lv 20)
- Hypnosis (Lv 24)
- Fake Tears (Lv 28)
- Psych Up (Lv 33)
- Psychic (Lv 36)
- Flatter (Lv 40)
- Future Sight (Lv 44)
- Magic Room (Lv 48)
</div>
</div>

**Flavor Text**
> Though they’re still only babies, there’s psychic power stored in their ribbonlike feelers, and sometimes they use that power to fight.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-gothita-574-panel-1">
## Gothorita (#575)
<details class="pokemon-card-container" open>
<summary>Gothorita overview</summary>
_Manipulate Pokémon_ • Types: Psychic • Egg Groups: Humanshape • Base EXP: 137

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Frisk
- Competitive
- Shadow Tag *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fighting (0.5×)
- Psychic (0.5×)

*Weak to*
- Bug (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Gothita → Gothorita (Lv 32) → Gothitelle (Lv 41)

**Encounter Locations**
- Kaptara Island (West) — Grass (Night) (8%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">60</span> |
| Attack | <span class="stat-value stat-low">45</span> |
| Defense | <span class="stat-value stat-mid">70</span> |
| Sp. Atk | <span class="stat-value stat-mid">75</span> |
| Sp. Def | <span class="stat-value stat-mid">85</span> |
| Speed | <span class="stat-value stat-mid">55</span> |
| Total | <span class="stat-value stat-mid">390</span> |

**Notable Level-Up Moves**
- Confusion (Lv 1)
- Play Nice (Lv 1)
- Pound (Lv 1)
- Tickle (Lv 1)
- Psybeam (Lv 12)
- Charm (Lv 16)
- Psyshock (Lv 20)
- Hypnosis (Lv 24)
- Fake Tears (Lv 28)
- Psych Up (Lv 35)
- Psychic (Lv 40)
- Flatter (Lv 46)
- Future Sight (Lv 52)
- Magic Room (Lv 58)
</div>
</div>

**Flavor Text**
> It’s said that when stars shine in the night sky, this Pokémon will spirit away sleeping children. Some call it the Witch of Punishment.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-gothita-574-panel-2">
## Gothitelle (#576)
<details class="pokemon-card-container" open>
<summary>Gothitelle overview</summary>
_Astral Body Pokémon_ • Types: Psychic • Egg Groups: Humanshape • Base EXP: 221

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Frisk
- Competitive
- Shadow Tag *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fighting (0.5×)
- Psychic (0.5×)

*Weak to*
- Bug (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Gothita → Gothorita (Lv 32) → Gothitelle (Lv 41)

**Encounter Locations**
- Kaptara Island (West) — Grass (Night) (2%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">70</span> |
| Attack | <span class="stat-value stat-mid">55</span> |
| Defense | <span class="stat-value stat-high">95</span> |
| Sp. Atk | <span class="stat-value stat-high">95</span> |
| Sp. Def | <span class="stat-value stat-high">110</span> |
| Speed | <span class="stat-value stat-mid">65</span> |
| Total | <span class="stat-value stat-mid">490</span> |

**Notable Level-Up Moves**
- Confusion (Lv 1)
- Play Nice (Lv 1)
- Pound (Lv 1)
- Tickle (Lv 1)
- Psybeam (Lv 12)
- Charm (Lv 16)
- Psyshock (Lv 20)
- Hypnosis (Lv 24)
- Fake Tears (Lv 28)
- Psych Up (Lv 35)
- Psychic (Lv 40)
- Flatter (Lv 48)
- Future Sight (Lv 56)
- Magic Room (Lv 64)
</div>
</div>

**Flavor Text**
> It has tremendous psychic power, but it dislikes conflict. It’s also able to predict the future based on the movement of the stars.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-gothita-574-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-gothita-574-panel-0 { display: block; }
#pokemon-tabs-gothita-574-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-gothita-574-panel-1 { display: block; }
#pokemon-tabs-gothita-574-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-gothita-574-panel-2 { display: block; }
</style>
