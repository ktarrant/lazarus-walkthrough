<div class="pokemon-tabs" id="pokemon-tabs-pikachu-25">
<input type="radio" name="pokemon-tabs-pikachu-25-group" id="pokemon-tabs-pikachu-25-tab-0">
<label for="pokemon-tabs-pikachu-25-tab-0">Pichu</label>
<input type="radio" name="pokemon-tabs-pikachu-25-group" id="pokemon-tabs-pikachu-25-tab-1" checked>
<label for="pokemon-tabs-pikachu-25-tab-1">Pikachu</label>
<input type="radio" name="pokemon-tabs-pikachu-25-group" id="pokemon-tabs-pikachu-25-tab-2">
<label for="pokemon-tabs-pikachu-25-tab-2">Raichu</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-pikachu-25-panel-0">
## Pichu (#172)
<details class="pokemon-card-container" open>
<summary>Pichu overview</summary>
_Tiny Mouse Pokémon_ • Types: Electric • Egg Groups: No Eggs • Base EXP: 41

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Static
- Lightning Rod *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Pichu → Pikachu (Friendship 220) → Raichu (Use Thunder Stone)

**Encounter Locations**
- Acrisia Mountains (Peak) — Grass (Day) (20%)
- Acrisia Mountains (Peak) — Grass (Night) (10%)
- Wanderer's Woods (North) — Grass (Day) (4%)
- Wanderer's Woods (North) — Grass (Night) (4%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">20</span> |
| Attack | <span class="stat-value stat-low">40</span> |
| Defense | <span class="stat-value stat-low">15</span> |
| Sp. Atk | <span class="stat-value stat-low">35</span> |
| Sp. Def | <span class="stat-value stat-low">35</span> |
| Speed | <span class="stat-value stat-mid">60</span> |
| Total | <span class="stat-value stat-low">205</span> |

**Notable Level-Up Moves**
- Tail Whip (Lv 1)
- Thunder Shock (Lv 1)
- Play Nice (Lv 4)
- Sweet Kiss (Lv 8)
- Nuzzle (Lv 12)
- Nasty Plot (Lv 16)
- Charm (Lv 20)
</div>
</div>

**Flavor Text**
> Despite its small size, it can zap even adult humans. However, if it does so, it also surprises itself.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-pikachu-25-panel-1">
## Pikachu (#025)
<details class="pokemon-card-container" open>
<summary>Pikachu overview</summary>
_Mouse Pokémon_ • Types: Electric • Egg Groups: Ground / Fairy • Base EXP: 112

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Static
- Lightning Rod *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Pichu → Pikachu (Friendship 220) → Raichu (Use Thunder Stone)

**Encounter Locations**
- Kipos Town — Grass (Day) (10%)
- Kipos Town — Grass (Night) (10%)
- Pollen Road — Grass (Day) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">35</span> |
| Attack | <span class="stat-value stat-mid">55</span> |
| Defense | <span class="stat-value stat-low">40</span> |
| Sp. Atk | <span class="stat-value stat-low">50</span> |
| Sp. Def | <span class="stat-value stat-low">50</span> |
| Speed | <span class="stat-value stat-mid">90</span> |
| Total | <span class="stat-value stat-mid">320</span> |

**Notable Level-Up Moves**
- Charm (Lv 1)
- Growl (Lv 1)
- Nasty Plot (Lv 1)
- Nuzzle (Lv 1)
- Play Nice (Lv 1)
- Quick Attack (Lv 1)
- Sweet Kiss (Lv 1)
- Tail Whip (Lv 1)
- Thunder Shock (Lv 1)
- Thunder Wave (Lv 4)
- Double Team (Lv 8)
- Electro Ball (Lv 12)
- Feint (Lv 16)
- Spark (Lv 20)
- Agility (Lv 24)
- Slam (Lv 28)
- Discharge (Lv 32)
- Thunderbolt (Lv 36)
- Light Screen (Lv 40)
- Thunder (Lv 44)
</div>
</div>

**Flavor Text**
> Pikachu that can generate powerful electricity have cheek sacs that are extra soft and super stretchy.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-pikachu-25-panel-2">
## Raichu (#026)
<details class="pokemon-card-container" open>
<summary>Raichu overview</summary>
_Mouse Pokémon_ • Types: Electric • Egg Groups: Ground / Fairy • Base EXP: 218

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Static
- Lightning Rod *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Pichu → Pikachu (Friendship 220) → Raichu (Use Thunder Stone)

**Encounter Locations**
- Port Pello — Grass (Night) (8%)
- Wakewater Isle — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">60</span> |
| Attack | <span class="stat-value stat-mid">90</span> |
| Defense | <span class="stat-value stat-mid">55</span> |
| Sp. Atk | <span class="stat-value stat-mid">90</span> |
| Sp. Def | <span class="stat-value stat-mid">80</span> |
| Speed | <span class="stat-value stat-high">110</span> |
| Total | <span class="stat-value stat-mid">485</span> |

**Notable Level-Up Moves**
- Agility (Lv 1)
- Charm (Lv 1)
- Discharge (Lv 1)
- Double Team (Lv 1)
- Electro Ball (Lv 1)
- Feint (Lv 1)
- Growl (Lv 1)
- Light Screen (Lv 1)
- Nasty Plot (Lv 1)
- Nuzzle (Lv 1)
- Play Nice (Lv 1)
- Quick Attack (Lv 1)
- Slam (Lv 1)
- Spark (Lv 1)
- Sweet Kiss (Lv 1)
- Tail Whip (Lv 1)
- Thunder (Lv 1)
- Thunder Punch (Lv 1)
- Thunder Shock (Lv 1)
- Thunder Wave (Lv 1)
- Thunderbolt (Lv 1)
</div>
</div>

**Flavor Text**
> Its long tail serves as a ground to protect itself from its own high-voltage power.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-pikachu-25-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-pikachu-25-panel-0 { display: block; }
#pokemon-tabs-pikachu-25-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-pikachu-25-panel-1 { display: block; }
#pokemon-tabs-pikachu-25-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-pikachu-25-panel-2 { display: block; }
</style>
