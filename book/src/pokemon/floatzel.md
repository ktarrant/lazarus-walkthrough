<div class="pokemon-tabs" id="pokemon-tabs-floatzel-419">
<input type="radio" name="pokemon-tabs-floatzel-419-group" id="pokemon-tabs-floatzel-419-tab-0">
<label for="pokemon-tabs-floatzel-419-tab-0">Buizel</label>
<input type="radio" name="pokemon-tabs-floatzel-419-group" id="pokemon-tabs-floatzel-419-tab-1" checked>
<label for="pokemon-tabs-floatzel-419-tab-1">Floatzel</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-floatzel-419-panel-0">
## Buizel (#418)
<details class="pokemon-card-container" open>
<summary>Buizel overview</summary>
_Sea Weasel Pokémon_ • Types: Water
Height: 0.7 m · Weight: 29.5 kg · Base EXP: 66

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Swift Swim
- Water Veil *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Ice (0.5×)
- Steel (0.5×)

*Weak to*
- Electric (2×)
- Grass (2×)

**Evolution Paths**
- Buizel → Floatzel (Lv 26)

**Encounter Locations**
- Asfal Hills — Grass (Day) (5%)
- Kalami City — Fishing (40%)
- Kalami City — Grass (Day) (20%)
- Kalami City — Grass (Night) (20%)
- Riverwalk Trail (South) — Fishing (40%)
- Riverwalk Trail (South) — Grass (Day) (20%)
- Riverwalk Trail (South) — Grass (Night) (20%)
- Riverwalk Trail (West) — Fishing (40%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 55 |
| Attack | 65 |
| Defense | 35 |
| Sp. Atk | 60 |
| Sp. Def | 30 |
| Speed | 85 |
| Total | 330 |

**Notable Level-Up Moves**
- Growl (Lv 1, Diamond Pearl)
- Quick Attack (Lv 1, Legends Arceus)
- Sonic Boom (Lv 1, Diamond Pearl)
- Tackle (Lv 1, Brilliant Diamond And Shining Pearl)
- Water Sport (Lv 1, Diamond Pearl)
- Aqua Jet (Lv 6, Legends Arceus)
</div>
</div>

**Flavor Text**
> It swims by rotating its two tails like a screw. When it dives, its flotation sac collapses.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-floatzel-419-panel-1">
## Floatzel (#419)
<details class="pokemon-card-container" open>
<summary>Floatzel overview</summary>
_Sea Weasel Pokémon_ • Types: Water
Height: 1.1 m · Weight: 33.5 kg · Base EXP: 173

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Swift Swim
- Water Veil *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Ice (0.5×)
- Steel (0.5×)

*Weak to*
- Electric (2×)
- Grass (2×)

**Evolution Paths**
- Buizel → Floatzel (Lv 26)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 85 |
| Attack | 105 |
| Defense | 55 |
| Sp. Atk | 85 |
| Sp. Def | 50 |
| Speed | 115 |
| Total | 495 |

**Notable Level-Up Moves**
- Crunch (Lv 1, Black 2 White 2)
- Growl (Lv 1, Diamond Pearl)
- Ice Fang (Lv 1, Diamond Pearl)
- Quick Attack (Lv 1, Diamond Pearl)
- Soak (Lv 1, Scarlet Violet)
- Sonic Boom (Lv 1, Diamond Pearl)
</div>
</div>

**Flavor Text**
> It floats using its well-developed flotation sac. It assists in the rescues of drowning people.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-floatzel-419-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-floatzel-419-panel-0 { display: block; }
#pokemon-tabs-floatzel-419-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-floatzel-419-panel-1 { display: block; }
</style>
