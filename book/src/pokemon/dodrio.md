<div class="pokemon-tabs" id="pokemon-tabs-dodrio-85">
<input type="radio" name="pokemon-tabs-dodrio-85-group" id="pokemon-tabs-dodrio-85-tab-0">
<label for="pokemon-tabs-dodrio-85-tab-0">Doduo</label>
<input type="radio" name="pokemon-tabs-dodrio-85-group" id="pokemon-tabs-dodrio-85-tab-1" checked>
<label for="pokemon-tabs-dodrio-85-tab-1">Dodrio</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-dodrio-85-panel-0">
## Doduo (#084)
<details class="pokemon-card-container" open>
<summary>Doduo overview</summary>
_Twin Bird Pokémon_ • Types: Normal / Flying • Egg Groups: Flying • Base EXP: 62

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Run Away
- Early Bird
- Tangled Feet *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Ground (0×)
- Bug (0.5×)
- Ghost (0×)

*Weak to*
- Electric (2×)
- Ice (2×)
- Rock (2×)

**Evolution Paths**
- Doduo → Dodrio (Lv 31)

**Encounter Locations**
- Fresco Isles — Grass (Day) (10%)
- Fresco Isles — Grass (Night) (10%)
- Kipos Town — Grass (Day) (5%)
- Kipos Town — Grass (Night) (5%)
- Pythios Town — Grass (Day) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">35</span> |
| Attack | <span class="stat-value stat-mid">85</span> |
| Defense | <span class="stat-value stat-low">45</span> |
| Sp. Atk | <span class="stat-value stat-low">35</span> |
| Sp. Def | <span class="stat-value stat-low">35</span> |
| Speed | <span class="stat-value stat-mid">75</span> |
| Total | <span class="stat-value stat-mid">310</span> |

**Notable Level-Up Moves**
- Growl (Lv 1)
- Peck (Lv 1)
- Pursuit (Lv 9)
- Fury Attack (Lv 13)
- Tri Attack (Lv 21)
- Rage (Lv 25)
- Uproar (Lv 33)
- Drill Peck (Lv 37)
- Agility (Lv 45)
</div>
</div>

**Flavor Text**
> A two-headed Pokémon that was discovered as a sudden mutation. It runs at a pace of over 60 miles per hour.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-dodrio-85-panel-1">
## Dodrio (#085)
<details class="pokemon-card-container" open>
<summary>Dodrio overview</summary>
_Triple Bird Pokémon_ • Types: Normal / Flying • Egg Groups: Flying • Base EXP: 165

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Run Away
- Early Bird
- Tangled Feet *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Ground (0×)
- Bug (0.5×)
- Ghost (0×)

*Weak to*
- Electric (2×)
- Ice (2×)
- Rock (2×)

**Evolution Paths**
- Doduo → Dodrio (Lv 31)

**Encounter Locations**
- Pollen Road — Grass (Day) (5%)
- Wakewater Isle — Grass (Day) (10%)
- Wakewater Isle — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">60</span> |
| Attack | <span class="stat-value stat-high">110</span> |
| Defense | <span class="stat-value stat-mid">70</span> |
| Sp. Atk | <span class="stat-value stat-mid">60</span> |
| Sp. Def | <span class="stat-value stat-mid">60</span> |
| Speed | <span class="stat-value stat-high">110</span> |
| Total | <span class="stat-value stat-mid">470</span> |

**Notable Level-Up Moves**
- Fury Attack (Lv 1)
- Growl (Lv 1)
- Peck (Lv 1)
- Pursuit (Lv 1)
- Pursuit (Lv 9)
- Fury Attack (Lv 13)
- Tri Attack (Lv 21)
- Rage (Lv 25)
- Uproar (Lv 38)
- Drill Peck (Lv 47)
- Agility (Lv 60)
</div>
</div>

**Flavor Text**
> An enemy that takes its eyes off any of the three heads--even for a second--will get pecked severely.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-dodrio-85-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-dodrio-85-panel-0 { display: block; }
#pokemon-tabs-dodrio-85-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-dodrio-85-panel-1 { display: block; }
</style>
