<div class="pokemon-tabs" id="pokemon-tabs-jigglypuff-39">
<input type="radio" name="pokemon-tabs-jigglypuff-39-group" id="pokemon-tabs-jigglypuff-39-tab-0">
<label for="pokemon-tabs-jigglypuff-39-tab-0">Igglybuff</label>
<input type="radio" name="pokemon-tabs-jigglypuff-39-group" id="pokemon-tabs-jigglypuff-39-tab-1" checked>
<label for="pokemon-tabs-jigglypuff-39-tab-1">Jigglypuff</label>
<input type="radio" name="pokemon-tabs-jigglypuff-39-group" id="pokemon-tabs-jigglypuff-39-tab-2">
<label for="pokemon-tabs-jigglypuff-39-tab-2">Wigglytuff</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-jigglypuff-39-panel-0">
## Igglybuff (#174)
<details class="pokemon-card-container" open>
<summary>Igglybuff overview</summary>
_Balloon Pokémon_ • Types: Normal / Fairy
Height: 0.3 m · Weight: 1.0 kg · Base EXP: 42

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Cute Charm
- Competitive
- Friend Guard *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Bug (0.5×)
- Ghost (0×)
- Dragon (0×)
- Dark (0.5×)

*Weak to*
- Poison (2×)
- Steel (2×)

**Evolution Paths**
- Igglybuff → Jigglypuff (Friendship 160) → Wigglytuff (Use Moon Stone)

**Encounter Locations**
- Kipos Town — Grass (Day) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 90 |
| Attack | 30 |
| Defense | 15 |
| Sp. Atk | 40 |
| Sp. Def | 20 |
| Speed | 15 |
| Total | 210 |

**Notable Level-Up Moves**
- Charm (Lv 1, Gold Silver)
- Copycat (Lv 1, Sword Shield)
- Pound (Lv 1, Sword Shield)
- Sing (Lv 1, Gold Silver)
- Defense Curl (Lv 3, Omega Ruby Alpha Sapphire)
- Sweet Kiss (Lv 8, Sword Shield)
</div>
</div>

**Flavor Text**
> Igglybuff loves to sing. Its marshmallow-like body gives off a faint sweet smell.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-jigglypuff-39-panel-1">
## Jigglypuff (#039)
<details class="pokemon-card-container" open>
<summary>Jigglypuff overview</summary>
_Balloon Pokémon_ • Types: Normal / Fairy
Height: 0.5 m · Weight: 5.5 kg · Base EXP: 95

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Cute Charm
- Competitive
- Friend Guard *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Bug (0.5×)
- Ghost (0×)
- Dragon (0×)
- Dark (0.5×)

*Weak to*
- Poison (2×)
- Steel (2×)

**Evolution Paths**
- Igglybuff → Jigglypuff (Friendship 160) → Wigglytuff (Use Moon Stone)

**Encounter Locations**
- Myrrini Island — Grass (Day) (10%)
- Port Pello — Grass (Day) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 115 |
| Attack | 45 |
| Defense | 20 |
| Sp. Atk | 45 |
| Sp. Def | 25 |
| Speed | 20 |
| Total | 270 |

**Notable Level-Up Moves**
- Charm (Lv 1, Sword Shield)
- Copycat (Lv 1, Sword Shield)
- Defense Curl (Lv 1, Sword Shield)
- Disable (Lv 1, Sword Shield)
- Disarming Voice (Lv 1, Sword Shield)
- Pound (Lv 1, Sword Shield)
</div>
</div>

**Flavor Text**
> Jigglypuff has top-notch lung capacity, even by comparison to other Pokémon. It won’t stop singing its lullabies until its foes fall asleep.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-jigglypuff-39-panel-2">
## Wigglytuff (#040)
<details class="pokemon-card-container" open>
<summary>Wigglytuff overview</summary>
_Balloon Pokémon_ • Types: Normal / Fairy
Height: 1.0 m · Weight: 12.0 kg · Base EXP: 196

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Cute Charm
- Competitive
- Frisk *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Bug (0.5×)
- Ghost (0×)
- Dragon (0×)
- Dark (0.5×)

*Weak to*
- Poison (2×)
- Steel (2×)

**Evolution Paths**
- Igglybuff → Jigglypuff (Friendship 160) → Wigglytuff (Use Moon Stone)

**Encounter Locations**
- Port Pello — Grass (Day) (5%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 140 |
| Attack | 70 |
| Defense | 45 |
| Sp. Atk | 85 |
| Sp. Def | 50 |
| Speed | 45 |
| Total | 435 |

**Notable Level-Up Moves**
- Bide (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Body Slam (Lv 1, Sword Shield)
- Charm (Lv 1, Sword Shield)
- Copycat (Lv 1, Sword Shield)
- Covet (Lv 1, Sword Shield)
- Defense Curl (Lv 1, Red Blue)
</div>
</div>

**Flavor Text**
> The more air it takes in, the more it inflates. If opponents catch it in a bad mood, it will inflate itself to an enormous size to intimidate them.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-jigglypuff-39-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-jigglypuff-39-panel-0 { display: block; }
#pokemon-tabs-jigglypuff-39-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-jigglypuff-39-panel-1 { display: block; }
#pokemon-tabs-jigglypuff-39-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-jigglypuff-39-panel-2 { display: block; }
</style>
