<div class="pokemon-tabs" id="pokemon-tabs-raichu">
<input type="radio" name="pokemon-tabs-raichu-group" id="pokemon-tabs-raichu-tab-0">
<label for="pokemon-tabs-raichu-tab-0">Pikachu</label>
<input type="radio" name="pokemon-tabs-raichu-group" id="pokemon-tabs-raichu-tab-1" checked>
<label for="pokemon-tabs-raichu-tab-1">Raichu</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-raichu-panel-0">
## Pikachu (#229)
<details class="pokemon-card-container" open>
<summary>Pikachu overview</summary>
Types: Electric • Egg Groups: Field / Fairy

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Static
- Lightning Rod *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**TM/HM Moves**
- TM01 - Focus Punch
- TM04 - Calm Mind
- TM06 - Toxic
- TM16 - Light Screen
- TM17 - Protect
- TM18 - Rain Dance
- TM23 - Iron Tail
- TM24 - Thunderbolt
- TM25 - Thunder
- TM28 - Dig
- TM31 - Brick Break
- TM32 - Double Team
- TM33 - Reflect
- TM34 - Shock Wave
- TM42 - Facade
- TM44 - Rest
- TM45 - Attract
- TM46 - Thief
- TM58 - Play Rough
- TM59 - Volt Switch
- HM03 - Surf
- HM04 - Strength
- HM05 - Flash
- HM06 - Rock Smash

**Evolution Info**
Lv. 15

**Encounter Locations**
- Kipos Town — Grass (Day) (10%)
- Kipos Town — Grass (Night) (10%)
- Pollen Road — Grass (Day) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">35</span> |
| Attack | <span class="stat-value stat-mid">55</span> |
| Defense | <span class="stat-value stat-low">40</span> |
| Sp. Atk | <span class="stat-value stat-low">50</span> |
| Sp. Def | <span class="stat-value stat-low">50</span> |
| Speed | <span class="stat-value stat-mid">90</span> |
| Total | <span class="stat-value stat-mid">320</span> |

**Level-Up Moves**
- Tail Whip (Lv 1)
- Thunder Shock (Lv 1)
- Growl (Lv 5)
- Play Nice (Lv 7)
- Quick Attack (Lv 10)
- Electro Ball (Lv 13)
- Thunder Wave (Lv 18)
- Feint (Lv 21)
- Double Team (Lv 23)
- Spark (Lv 26)
- Nuzzle (Lv 29)
- Discharge (Lv 34)
- Slam (Lv 37)
- Thunderbolt (Lv 42)
- Agility (Lv 45)
- Wild Charge (Lv 50)
- Light Screen (Lv 53)
- Thunder (Lv 58)

**Egg Moves**
- Reversal
- Bide
- Present
- Encore
- Double Slap
- Wish
- Charge
- Fake Out
- Thunder Punch
- Tickle
- Flail
- Endure
- Lucky Chant
- Bestow
- Disarming Voice
- Electric Terrain

**Tutor Moves**
- Body Slam
- Counter
- Defense Curl
- Double-Edge
- Dynamic Punch
- Endure
- Mega Kick
- Mega Punch
- Mud-Slap
- Rollout
- Seismic Toss
- Sleep Talk
- Snore
- Swagger
- Swift
- Thunder Punch
- Thunder Wave
</div>
</div>
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-raichu-panel-1">
## Raichu (#230)
<details class="pokemon-card-container" open>
<summary>Raichu overview</summary>
Types: Electric • Egg Groups: Field / Fairy

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Static
- Lightning Rod *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**TM/HM Moves**
- TM01 - Focus Punch
- TM04 - Calm Mind
- TM06 - Toxic
- TM15 - Hyper Beam
- TM16 - Light Screen
- TM17 - Protect
- TM18 - Rain Dance
- TM20 - Safeguard
- TM23 - Iron Tail
- TM24 - Thunderbolt
- TM25 - Thunder
- TM28 - Dig
- TM31 - Brick Break
- TM32 - Double Team
- TM33 - Reflect
- TM34 - Shock Wave
- TM42 - Facade
- TM44 - Rest
- TM45 - Attract
- TM46 - Thief
- TM58 - Play Rough
- TM59 - Volt Switch
- HM03 - Surf
- HM04 - Strength
- HM05 - Flash
- HM06 - Rock Smash

**Evolution Info**
Thunder Stone

**Encounter Locations**
- Port Pello — Grass (Night) (8%)
- Wakewater Isle — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">70</span> |
| Attack | <span class="stat-value stat-mid">90</span> |
| Defense | <span class="stat-value stat-mid">55</span> |
| Sp. Atk | <span class="stat-value stat-mid">90</span> |
| Sp. Def | <span class="stat-value stat-mid">80</span> |
| Speed | <span class="stat-value stat-high">120</span> |
| Total | <span class="stat-value stat-mid">505</span> |

**Level-Up Moves**
- Thunder Shock (Lv 1)
- Tail Whip (Lv 1)
- Quick Attack (Lv 1)
- Thunderbolt (Lv 1)

**Egg Moves**
- Reversal
- Bide
- Present
- Encore
- Double Slap
- Wish
- Charge
- Fake Out
- Thunder Punch
- Tickle
- Flail
- Endure
- Lucky Chant
- Bestow
- Disarming Voice
- Electric Terrain

**Tutor Moves**
- Body Slam
- Counter
- Defense Curl
- Double-Edge
- Dynamic Punch
- Endure
- Mega Kick
- Mega Punch
- Mud-Slap
- Rollout
- Seismic Toss
- Sleep Talk
- Snore
- Swagger
- Swift
- Thunder Punch
- Thunder Wave
</div>
</div>
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-raichu-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-raichu-panel-0 { display: block; }
#pokemon-tabs-raichu-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-raichu-panel-1 { display: block; }
</style>
