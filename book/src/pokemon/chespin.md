<details class="pokemon-card-container">
<summary>Chespin (#010)</summary>
<div class="pokemon-tabs" id="pokemon-tabs-chespin">
<input type="radio" name="pokemon-tabs-chespin-group" id="pokemon-tabs-chespin-tab-0" checked>
<label for="pokemon-tabs-chespin-tab-0">Chespin</label>
<input type="radio" name="pokemon-tabs-chespin-group" id="pokemon-tabs-chespin-tab-1">
<label for="pokemon-tabs-chespin-tab-1">Quilladin</label>
<input type="radio" name="pokemon-tabs-chespin-group" id="pokemon-tabs-chespin-tab-2">
<label for="pokemon-tabs-chespin-tab-2">Chesnaught</label>
<input type="radio" name="pokemon-tabs-chespin-group" id="pokemon-tabs-chespin-tab-3">
<label for="pokemon-tabs-chespin-tab-3">Mega Chesnaught</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-chespin-panel-0">
Types: Grass • Egg Groups: Field

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Overgrow
- Long Reach
- Spiked Shell *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ground (0.5×)

*Weak to*
- Fire (2×)
- Ice (2×)
- Poison (2×)
- Flying (2×)
- Bug (2×)

**TM/HM Moves**
- <a href="move-lookup.html?q=tm06-toxic">TM06 - Toxic</a>
- <a href="move-lookup.html?q=tm08-bulk-up">TM08 - Bulk Up</a>
- <a href="move-lookup.html?q=tm09-bullet-seed">TM09 - Bullet Seed</a>
- <a href="move-lookup.html?q=tm11-sunny-day">TM11 - Sunny Day</a>
- <a href="move-lookup.html?q=tm12-taunt">TM12 - Taunt</a>
- <a href="move-lookup.html?q=tm17-protect">TM17 - Protect</a>
- <a href="move-lookup.html?q=tm18-rain-dance">TM18 - Rain Dance</a>
- <a href="move-lookup.html?q=tm19-giga-drain">TM19 - Giga Drain</a>
- <a href="move-lookup.html?q=tm20-poison-jab">TM20 - Poison Jab</a>
- <a href="move-lookup.html?q=tm22-solar-beam">TM22 - Solar Beam</a>
- <a href="move-lookup.html?q=tm28-dig">TM28 - Dig</a>
- <a href="move-lookup.html?q=tm31-brick-break">TM31 - Brick Break</a>
- <a href="move-lookup.html?q=tm32-double-team">TM32 - Double Team</a>
- <a href="move-lookup.html?q=tm33-reflect">TM33 - Reflect</a>
- <a href="move-lookup.html?q=tm36-sludge-bomb">TM36 - Sludge Bomb</a>
- <a href="move-lookup.html?q=tm39-rock-tomb">TM39 - Rock Tomb</a>
- <a href="move-lookup.html?q=tm40-aerial-ace">TM40 - Aerial Ace</a>
- <a href="move-lookup.html?q=tm42-facade">TM42 - Facade</a>
- <a href="move-lookup.html?q=tm44-rest">TM44 - Rest</a>
- <a href="move-lookup.html?q=tm45-attract">TM45 - Attract</a>
- <a href="move-lookup.html?q=tm49-bulldoze">TM49 - Bulldoze</a>
- <a href="move-lookup.html?q=tm53-power-up-punch">TM53 - Power-Up Punch</a>
- <a href="move-lookup.html?q=hm01-cut">HM01 - Cut</a>
- <a href="move-lookup.html?q=hm04-strength">HM04 - Strength</a>
- <a href="move-lookup.html?q=hm05-flash">HM05 - Flash</a>
- <a href="move-lookup.html?q=hm06-rock-smash">HM06 - Rock Smash</a>

**Encounter Locations**
- Sea of Asteri (East) — Grass (Night) (5%)
</div>
<div class="card-column">
<label><input type="checkbox" class="caught-check" data-species="chespin" /> Caught</label>

**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">56</span> |
| Attack | <span class="stat-value stat-mid">61</span> |
| Defense | <span class="stat-value stat-mid">65</span> |
| Sp. Atk | <span class="stat-value stat-low">48</span> |
| Sp. Def | <span class="stat-value stat-low">45</span> |
| Speed | <span class="stat-value stat-low">38</span> |
| Total | <span class="stat-value stat-mid">313</span> |

**Level-Up Moves**
- <a href="move-lookup.html?q=tackle">Tackle</a> (Lv 1)
- <a href="move-lookup.html?q=growl">Growl</a> (Lv 1)
- <a href="move-lookup.html?q=vine-whip">Vine Whip</a> (Lv 5)
- <a href="move-lookup.html?q=rollout">Rollout</a> (Lv 8)
- <a href="move-lookup.html?q=bite">Bite</a> (Lv 11)
- <a href="move-lookup.html?q=leech-seed">Leech Seed</a> (Lv 15)
- <a href="move-lookup.html?q=pin-missile">Pin Missile</a> (Lv 18)
- <a href="move-lookup.html?q=mud-shot">Mud Shot</a> (Lv 24)
- <a href="move-lookup.html?q=vital-throw">Vital Throw</a> (Lv 27)
- <a href="move-lookup.html?q=seed-bomb">Seed Bomb</a> (Lv 32)
- <a href="move-lookup.html?q=dig">Dig</a> (Lv 35)
- <a href="move-lookup.html?q=bulk-up">Bulk Up</a> (Lv 39)
- <a href="move-lookup.html?q=body-slam">Body Slam</a> (Lv 42)
- <a href="move-lookup.html?q=pain-split">Pain Split</a> (Lv 45)
- <a href="move-lookup.html?q=wood-hammer">Wood Hammer</a> (Lv 48)

**Egg Moves**
- <a href="move-lookup.html?q=synthesis">Synthesis</a>
- <a href="move-lookup.html?q=belly-drum">Belly Drum</a>
- <a href="move-lookup.html?q=curse">Curse</a>
- <a href="move-lookup.html?q=quick-guard">Quick Guard</a>
- <a href="move-lookup.html?q=spikes">Spikes</a>
- <a href="move-lookup.html?q=defense-curl">Defense Curl</a>
- <a href="move-lookup.html?q=rollout">Rollout</a>
- <a href="move-lookup.html?q=power-up-punch">Power-Up Punch</a>

**Tutor Moves**
- <a href="move-lookup.html?q=body-slam">Body Slam</a>
- <a href="move-lookup.html?q=defense-curl">Defense Curl</a>
- <a href="move-lookup.html?q=endure">Endure</a>
- <a href="move-lookup.html?q=rock-slide">Rock Slide</a>
- <a href="move-lookup.html?q=rollout">Rollout</a>
- <a href="move-lookup.html?q=sleep-talk">Sleep Talk</a>
- <a href="move-lookup.html?q=snore">Snore</a>
- <a href="move-lookup.html?q=swagger">Swagger</a>
- <a href="move-lookup.html?q=swift">Swift</a>
- <a href="move-lookup.html?q=swords-dance">Swords Dance</a>
- <a href="move-lookup.html?q=thunder-punch">Thunder Punch</a>
</div>
</div>
<script>
(function() {
  if (window.__lazarusCaughtInit) return; window.__lazarusCaughtInit = true;
  const STORAGE_KEY = 'lazarusCaught';
  function loadState() {
    try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}'); } catch (_) { return {}; }
  }
  function saveState(state) {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (_) {}
  }
  function applyState() {
    const state = loadState();
    const boxes = Array.from(document.querySelectorAll('.caught-check'));
    const bySpecies = boxes.reduce((m, cb) => {
      const s = cb.dataset.species; if (!s) return m; (m[s] ||= []).push(cb); return m; }, {});
    boxes.forEach(cb => {
      const key = cb.dataset.species;
      cb.checked = !!state[key];
      cb.onchange = () => {
        const checked = cb.checked;
        if (checked) state[key] = true; else delete state[key];
        saveState(state);
        (bySpecies[key] || []).forEach(other => { if (other !== cb) other.checked = checked; });
      };
    });
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', applyState);
  } else {
    applyState();
  }
})();
</script>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-chespin-panel-1">
Types: Grass • Egg Groups: Field

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Overgrow
- Long Reach
- Spiked Shell *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ground (0.5×)

*Weak to*
- Fire (2×)
- Ice (2×)
- Poison (2×)
- Flying (2×)
- Bug (2×)

**TM/HM Moves**
- <a href="move-lookup.html?q=tm06-toxic">TM06 - Toxic</a>
- <a href="move-lookup.html?q=tm08-bulk-up">TM08 - Bulk Up</a>
- <a href="move-lookup.html?q=tm09-bullet-seed">TM09 - Bullet Seed</a>
- <a href="move-lookup.html?q=tm11-sunny-day">TM11 - Sunny Day</a>
- <a href="move-lookup.html?q=tm12-taunt">TM12 - Taunt</a>
- <a href="move-lookup.html?q=tm17-protect">TM17 - Protect</a>
- <a href="move-lookup.html?q=tm18-rain-dance">TM18 - Rain Dance</a>
- <a href="move-lookup.html?q=tm19-giga-drain">TM19 - Giga Drain</a>
- <a href="move-lookup.html?q=tm20-poison-jab">TM20 - Poison Jab</a>
- <a href="move-lookup.html?q=tm22-solar-beam">TM22 - Solar Beam</a>
- <a href="move-lookup.html?q=tm28-dig">TM28 - Dig</a>
- <a href="move-lookup.html?q=tm31-brick-break">TM31 - Brick Break</a>
- <a href="move-lookup.html?q=tm32-double-team">TM32 - Double Team</a>
- <a href="move-lookup.html?q=tm33-reflect">TM33 - Reflect</a>
- <a href="move-lookup.html?q=tm36-sludge-bomb">TM36 - Sludge Bomb</a>
- <a href="move-lookup.html?q=tm39-rock-tomb">TM39 - Rock Tomb</a>
- <a href="move-lookup.html?q=tm40-aerial-ace">TM40 - Aerial Ace</a>
- <a href="move-lookup.html?q=tm42-facade">TM42 - Facade</a>
- <a href="move-lookup.html?q=tm44-rest">TM44 - Rest</a>
- <a href="move-lookup.html?q=tm45-attract">TM45 - Attract</a>
- <a href="move-lookup.html?q=tm49-bulldoze">TM49 - Bulldoze</a>
- <a href="move-lookup.html?q=tm53-power-up-punch">TM53 - Power-Up Punch</a>
- <a href="move-lookup.html?q=hm01-cut">HM01 - Cut</a>
- <a href="move-lookup.html?q=hm04-strength">HM04 - Strength</a>
- <a href="move-lookup.html?q=hm05-flash">HM05 - Flash</a>
- <a href="move-lookup.html?q=hm06-rock-smash">HM06 - Rock Smash</a>

**Evolution Info**
Lv. 16
</div>
<div class="card-column">
<label><input type="checkbox" class="caught-check" data-species="quilladin" /> Caught</label>

**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">61</span> |
| Attack | <span class="stat-value stat-mid">78</span> |
| Defense | <span class="stat-value stat-high">95</span> |
| Sp. Atk | <span class="stat-value stat-mid">56</span> |
| Sp. Def | <span class="stat-value stat-mid">58</span> |
| Speed | <span class="stat-value stat-mid">57</span> |
| Total | <span class="stat-value stat-mid">405</span> |

**Level-Up Moves**
- <a href="move-lookup.html?q=needle-arm">Needle Arm</a> (Lv Evo)
- <a href="move-lookup.html?q=tackle">Tackle</a> (Lv 1)
- <a href="move-lookup.html?q=growl">Growl</a> (Lv 1)
- <a href="move-lookup.html?q=vine-whip">Vine Whip</a> (Lv 5)
- <a href="move-lookup.html?q=rollout">Rollout</a> (Lv 8)
- <a href="move-lookup.html?q=bite">Bite</a> (Lv 11)
- <a href="move-lookup.html?q=leech-seed">Leech Seed</a> (Lv 15)
- <a href="move-lookup.html?q=pin-missile">Pin Missile</a> (Lv 18)
- <a href="move-lookup.html?q=mud-shot">Mud Shot</a> (Lv 24)
- <a href="move-lookup.html?q=vital-throw">Vital Throw</a> (Lv 27)
- <a href="move-lookup.html?q=seed-bomb">Seed Bomb</a> (Lv 32)
- <a href="move-lookup.html?q=dig">Dig</a> (Lv 35)
- <a href="move-lookup.html?q=bulk-up">Bulk Up</a> (Lv 39)
- <a href="move-lookup.html?q=body-slam">Body Slam</a> (Lv 42)
- <a href="move-lookup.html?q=pain-split">Pain Split</a> (Lv 45)
- <a href="move-lookup.html?q=wood-hammer">Wood Hammer</a> (Lv 48)

**Egg Moves**
- <a href="move-lookup.html?q=synthesis">Synthesis</a>
- <a href="move-lookup.html?q=belly-drum">Belly Drum</a>
- <a href="move-lookup.html?q=curse">Curse</a>
- <a href="move-lookup.html?q=quick-guard">Quick Guard</a>
- <a href="move-lookup.html?q=spikes">Spikes</a>
- <a href="move-lookup.html?q=defense-curl">Defense Curl</a>
- <a href="move-lookup.html?q=rollout">Rollout</a>
- <a href="move-lookup.html?q=power-up-punch">Power-Up Punch</a>

**Tutor Moves**
- <a href="move-lookup.html?q=body-slam">Body Slam</a>
- <a href="move-lookup.html?q=defense-curl">Defense Curl</a>
- <a href="move-lookup.html?q=endure">Endure</a>
- <a href="move-lookup.html?q=rock-slide">Rock Slide</a>
- <a href="move-lookup.html?q=rollout">Rollout</a>
- <a href="move-lookup.html?q=sleep-talk">Sleep Talk</a>
- <a href="move-lookup.html?q=snore">Snore</a>
- <a href="move-lookup.html?q=swagger">Swagger</a>
- <a href="move-lookup.html?q=swift">Swift</a>
- <a href="move-lookup.html?q=swords-dance">Swords Dance</a>
- <a href="move-lookup.html?q=thunder-punch">Thunder Punch</a>
</div>
</div>
<script>
(function() {
  if (window.__lazarusCaughtInit) return; window.__lazarusCaughtInit = true;
  const STORAGE_KEY = 'lazarusCaught';
  function loadState() {
    try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}'); } catch (_) { return {}; }
  }
  function saveState(state) {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (_) {}
  }
  function applyState() {
    const state = loadState();
    const boxes = Array.from(document.querySelectorAll('.caught-check'));
    const bySpecies = boxes.reduce((m, cb) => {
      const s = cb.dataset.species; if (!s) return m; (m[s] ||= []).push(cb); return m; }, {});
    boxes.forEach(cb => {
      const key = cb.dataset.species;
      cb.checked = !!state[key];
      cb.onchange = () => {
        const checked = cb.checked;
        if (checked) state[key] = true; else delete state[key];
        saveState(state);
        (bySpecies[key] || []).forEach(other => { if (other !== cb) other.checked = checked; });
      };
    });
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', applyState);
  } else {
    applyState();
  }
})();
</script>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-chespin-panel-2">
Types: Grass / Fighting • Egg Groups: Field

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Overgrow
- Long Reach
- Spiked Shell *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ground (0.5×)
- Rock (0.5×)
- Dark (0.5×)

*Weak to*
- Fire (2×)
- Ice (2×)
- Poison (2×)
- Flying (4×)
- Psychic (2×)
- Fairy (2×)

**TM/HM Moves**
- <a href="move-lookup.html?q=tm02-dragon-claw">TM02 - Dragon Claw</a>
- <a href="move-lookup.html?q=tm06-toxic">TM06 - Toxic</a>
- <a href="move-lookup.html?q=tm08-bulk-up">TM08 - Bulk Up</a>
- <a href="move-lookup.html?q=tm09-bullet-seed">TM09 - Bullet Seed</a>
- <a href="move-lookup.html?q=tm11-sunny-day">TM11 - Sunny Day</a>
- <a href="move-lookup.html?q=tm12-taunt">TM12 - Taunt</a>
- <a href="move-lookup.html?q=tm17-protect">TM17 - Protect</a>
- <a href="move-lookup.html?q=tm18-rain-dance">TM18 - Rain Dance</a>
- <a href="move-lookup.html?q=tm19-giga-drain">TM19 - Giga Drain</a>
- <a href="move-lookup.html?q=tm20-poison-jab">TM20 - Poison Jab</a>
- <a href="move-lookup.html?q=tm22-solar-beam">TM22 - Solar Beam</a>
- <a href="move-lookup.html?q=tm26-earthquake">TM26 - Earthquake</a>
- <a href="move-lookup.html?q=tm28-dig">TM28 - Dig</a>
- <a href="move-lookup.html?q=tm31-brick-break">TM31 - Brick Break</a>
- <a href="move-lookup.html?q=tm32-double-team">TM32 - Double Team</a>
- <a href="move-lookup.html?q=tm33-reflect">TM33 - Reflect</a>
- <a href="move-lookup.html?q=tm36-sludge-bomb">TM36 - Sludge Bomb</a>
- <a href="move-lookup.html?q=tm39-rock-tomb">TM39 - Rock Tomb</a>
- <a href="move-lookup.html?q=tm40-aerial-ace">TM40 - Aerial Ace</a>
- <a href="move-lookup.html?q=tm42-facade">TM42 - Facade</a>
- <a href="move-lookup.html?q=tm44-rest">TM44 - Rest</a>
- <a href="move-lookup.html?q=tm45-attract">TM45 - Attract</a>
- <a href="move-lookup.html?q=tm49-bulldoze">TM49 - Bulldoze</a>
- <a href="move-lookup.html?q=tm53-power-up-punch">TM53 - Power-Up Punch</a>
- <a href="move-lookup.html?q=hm01-cut">HM01 - Cut</a>
- <a href="move-lookup.html?q=hm04-strength">HM04 - Strength</a>
- <a href="move-lookup.html?q=hm05-flash">HM05 - Flash</a>
- <a href="move-lookup.html?q=hm06-rock-smash">HM06 - Rock Smash</a>

**Evolution Info**
Lv. 36
</div>
<div class="card-column">
<label><input type="checkbox" class="caught-check" data-species="chesnaught" /> Caught</label>

**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">88</span> |
| Attack | <span class="stat-value stat-high">107</span> |
| Defense | <span class="stat-value stat-high">122</span> |
| Sp. Atk | <span class="stat-value stat-mid">74</span> |
| Sp. Def | <span class="stat-value stat-mid">75</span> |
| Speed | <span class="stat-value stat-mid">64</span> |
| Total | <span class="stat-value stat-mid">530</span> |

**Level-Up Moves**
- <a href="move-lookup.html?q=spiky-shield">Spiky Shield</a> (Lv Evo)
- <a href="move-lookup.html?q=needle-arm">Needle Arm</a> (Lv 1)
- <a href="move-lookup.html?q=hammer-arm">Hammer Arm</a> (Lv 1)
- <a href="move-lookup.html?q=feint">Feint</a> (Lv 1)
- <a href="move-lookup.html?q=belly-drum">Belly Drum</a> (Lv 1)
- <a href="move-lookup.html?q=growl">Growl</a> (Lv 1)
- <a href="move-lookup.html?q=tackle">Tackle</a> (Lv 1)
- <a href="move-lookup.html?q=vine-whip">Vine Whip</a> (Lv 5)
- <a href="move-lookup.html?q=rollout">Rollout</a> (Lv 8)
- <a href="move-lookup.html?q=bite">Bite</a> (Lv 11)
- <a href="move-lookup.html?q=leech-seed">Leech Seed</a> (Lv 15)
- <a href="move-lookup.html?q=pin-missile">Pin Missile</a> (Lv 18)
- <a href="move-lookup.html?q=mud-shot">Mud Shot</a> (Lv 24)
- <a href="move-lookup.html?q=vital-throw">Vital Throw</a> (Lv 27)
- <a href="move-lookup.html?q=seed-bomb">Seed Bomb</a> (Lv 32)
- <a href="move-lookup.html?q=dig">Dig</a> (Lv 35)
- <a href="move-lookup.html?q=bulk-up">Bulk Up</a> (Lv 39)
- <a href="move-lookup.html?q=body-slam">Body Slam</a> (Lv 42)
- <a href="move-lookup.html?q=pain-split">Pain Split</a> (Lv 45)
- <a href="move-lookup.html?q=wood-hammer">Wood Hammer</a> (Lv 48)
- <a href="move-lookup.html?q=hammer-arm">Hammer Arm</a> (Lv 52)
- <a href="move-lookup.html?q=giga-impact">Giga Impact</a> (Lv 58)

**Egg Moves**
- <a href="move-lookup.html?q=synthesis">Synthesis</a>
- <a href="move-lookup.html?q=belly-drum">Belly Drum</a>
- <a href="move-lookup.html?q=curse">Curse</a>
- <a href="move-lookup.html?q=quick-guard">Quick Guard</a>
- <a href="move-lookup.html?q=spikes">Spikes</a>
- <a href="move-lookup.html?q=defense-curl">Defense Curl</a>
- <a href="move-lookup.html?q=rollout">Rollout</a>
- <a href="move-lookup.html?q=power-up-punch">Power-Up Punch</a>

**Tutor Moves**
- <a href="move-lookup.html?q=body-slam">Body Slam</a>
- <a href="move-lookup.html?q=defense-curl">Defense Curl</a>
- <a href="move-lookup.html?q=double-edge">Double-Edge</a>
- <a href="move-lookup.html?q=endure">Endure</a>
- <a href="move-lookup.html?q=mud-slap">Mud-Slap</a>
- <a href="move-lookup.html?q=rock-slide">Rock Slide</a>
- <a href="move-lookup.html?q=rollout">Rollout</a>
- <a href="move-lookup.html?q=sleep-talk">Sleep Talk</a>
- <a href="move-lookup.html?q=snore">Snore</a>
- <a href="move-lookup.html?q=swagger">Swagger</a>
- <a href="move-lookup.html?q=swift">Swift</a>
- <a href="move-lookup.html?q=swords-dance">Swords Dance</a>
- <a href="move-lookup.html?q=thunder-punch">Thunder Punch</a>
</div>
</div>
<script>
(function() {
  if (window.__lazarusCaughtInit) return; window.__lazarusCaughtInit = true;
  const STORAGE_KEY = 'lazarusCaught';
  function loadState() {
    try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}'); } catch (_) { return {}; }
  }
  function saveState(state) {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (_) {}
  }
  function applyState() {
    const state = loadState();
    const boxes = Array.from(document.querySelectorAll('.caught-check'));
    const bySpecies = boxes.reduce((m, cb) => {
      const s = cb.dataset.species; if (!s) return m; (m[s] ||= []).push(cb); return m; }, {});
    boxes.forEach(cb => {
      const key = cb.dataset.species;
      cb.checked = !!state[key];
      cb.onchange = () => {
        const checked = cb.checked;
        if (checked) state[key] = true; else delete state[key];
        saveState(state);
        (bySpecies[key] || []).forEach(other => { if (other !== cb) other.checked = checked; });
      };
    });
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', applyState);
  } else {
    applyState();
  }
})();
</script>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-chespin-panel-3">
Types: Grass / Fighting • Egg Groups: Field

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Spiked Shell

**Type Matchups**

*Resists / Immune to*
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ground (0.5×)
- Rock (0.5×)
- Dark (0.5×)

*Weak to*
- Fire (2×)
- Ice (2×)
- Poison (2×)
- Flying (4×)
- Psychic (2×)
- Fairy (2×)

**TM/HM Moves**
- <a href="move-lookup.html?q=tm02-dragon-claw">TM02 - Dragon Claw</a>
- <a href="move-lookup.html?q=tm06-toxic">TM06 - Toxic</a>
- <a href="move-lookup.html?q=tm08-bulk-up">TM08 - Bulk Up</a>
- <a href="move-lookup.html?q=tm09-bullet-seed">TM09 - Bullet Seed</a>
- <a href="move-lookup.html?q=tm11-sunny-day">TM11 - Sunny Day</a>
- <a href="move-lookup.html?q=tm12-taunt">TM12 - Taunt</a>
- <a href="move-lookup.html?q=tm17-protect">TM17 - Protect</a>
- <a href="move-lookup.html?q=tm18-rain-dance">TM18 - Rain Dance</a>
- <a href="move-lookup.html?q=tm19-giga-drain">TM19 - Giga Drain</a>
- <a href="move-lookup.html?q=tm20-poison-jab">TM20 - Poison Jab</a>
- <a href="move-lookup.html?q=tm22-solar-beam">TM22 - Solar Beam</a>
- <a href="move-lookup.html?q=tm26-earthquake">TM26 - Earthquake</a>
- <a href="move-lookup.html?q=tm28-dig">TM28 - Dig</a>
- <a href="move-lookup.html?q=tm31-brick-break">TM31 - Brick Break</a>
- <a href="move-lookup.html?q=tm32-double-team">TM32 - Double Team</a>
- <a href="move-lookup.html?q=tm33-reflect">TM33 - Reflect</a>
- <a href="move-lookup.html?q=tm36-sludge-bomb">TM36 - Sludge Bomb</a>
- <a href="move-lookup.html?q=tm39-rock-tomb">TM39 - Rock Tomb</a>
- <a href="move-lookup.html?q=tm40-aerial-ace">TM40 - Aerial Ace</a>
- <a href="move-lookup.html?q=tm42-facade">TM42 - Facade</a>
- <a href="move-lookup.html?q=tm44-rest">TM44 - Rest</a>
- <a href="move-lookup.html?q=tm45-attract">TM45 - Attract</a>
- <a href="move-lookup.html?q=tm49-bulldoze">TM49 - Bulldoze</a>
- <a href="move-lookup.html?q=tm53-power-up-punch">TM53 - Power-Up Punch</a>
- <a href="move-lookup.html?q=hm01-cut">HM01 - Cut</a>
- <a href="move-lookup.html?q=hm04-strength">HM04 - Strength</a>
- <a href="move-lookup.html?q=hm05-flash">HM05 - Flash</a>
- <a href="move-lookup.html?q=hm06-rock-smash">HM06 - Rock Smash</a>

**Evolution Info**
Chesnaughtite
</div>
<div class="card-column">
<label><input type="checkbox" class="caught-check" data-species="mega-chesnaught" /> Caught</label>

**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">88</span> |
| Attack | <span class="stat-value stat-high">137</span> |
| Defense | <span class="stat-value stat-high">172</span> |
| Sp. Atk | <span class="stat-value stat-mid">79</span> |
| Sp. Def | <span class="stat-value stat-high">95</span> |
| Speed | <span class="stat-value stat-mid">64</span> |
| Total | <span class="stat-value stat-high">635</span> |

**Level-Up Moves**
- <a href="move-lookup.html?q=spiky-shield">Spiky Shield</a> (Lv Evo)
- <a href="move-lookup.html?q=needle-arm">Needle Arm</a> (Lv 1)
- <a href="move-lookup.html?q=hammer-arm">Hammer Arm</a> (Lv 1)
- <a href="move-lookup.html?q=feint">Feint</a> (Lv 1)
- <a href="move-lookup.html?q=belly-drum">Belly Drum</a> (Lv 1)
- <a href="move-lookup.html?q=growl">Growl</a> (Lv 1)
- <a href="move-lookup.html?q=tackle">Tackle</a> (Lv 1)
- <a href="move-lookup.html?q=vine-whip">Vine Whip</a> (Lv 5)
- <a href="move-lookup.html?q=rollout">Rollout</a> (Lv 8)
- <a href="move-lookup.html?q=bite">Bite</a> (Lv 11)
- <a href="move-lookup.html?q=leech-seed">Leech Seed</a> (Lv 15)
- <a href="move-lookup.html?q=pin-missile">Pin Missile</a> (Lv 18)
- <a href="move-lookup.html?q=mud-shot">Mud Shot</a> (Lv 24)
- <a href="move-lookup.html?q=vital-throw">Vital Throw</a> (Lv 27)
- <a href="move-lookup.html?q=seed-bomb">Seed Bomb</a> (Lv 32)
- <a href="move-lookup.html?q=dig">Dig</a> (Lv 35)
- <a href="move-lookup.html?q=bulk-up">Bulk Up</a> (Lv 39)
- <a href="move-lookup.html?q=body-slam">Body Slam</a> (Lv 42)
- <a href="move-lookup.html?q=pain-split">Pain Split</a> (Lv 45)
- <a href="move-lookup.html?q=wood-hammer">Wood Hammer</a> (Lv 48)
- <a href="move-lookup.html?q=hammer-arm">Hammer Arm</a> (Lv 52)
- <a href="move-lookup.html?q=giga-impact">Giga Impact</a> (Lv 58)

**Egg Moves**
- <a href="move-lookup.html?q=synthesis">Synthesis</a>
- <a href="move-lookup.html?q=belly-drum">Belly Drum</a>
- <a href="move-lookup.html?q=curse">Curse</a>
- <a href="move-lookup.html?q=quick-guard">Quick Guard</a>
- <a href="move-lookup.html?q=spikes">Spikes</a>
- <a href="move-lookup.html?q=defense-curl">Defense Curl</a>
- <a href="move-lookup.html?q=rollout">Rollout</a>
- <a href="move-lookup.html?q=power-up-punch">Power-Up Punch</a>

**Tutor Moves**
- <a href="move-lookup.html?q=body-slam">Body Slam</a>
- <a href="move-lookup.html?q=defense-curl">Defense Curl</a>
- <a href="move-lookup.html?q=double-edge">Double-Edge</a>
- <a href="move-lookup.html?q=endure">Endure</a>
- <a href="move-lookup.html?q=mud-slap">Mud-Slap</a>
- <a href="move-lookup.html?q=rock-slide">Rock Slide</a>
- <a href="move-lookup.html?q=rollout">Rollout</a>
- <a href="move-lookup.html?q=sleep-talk">Sleep Talk</a>
- <a href="move-lookup.html?q=snore">Snore</a>
- <a href="move-lookup.html?q=swagger">Swagger</a>
- <a href="move-lookup.html?q=swift">Swift</a>
- <a href="move-lookup.html?q=swords-dance">Swords Dance</a>
- <a href="move-lookup.html?q=thunder-punch">Thunder Punch</a>
</div>
</div>
<script>
(function() {
  if (window.__lazarusCaughtInit) return; window.__lazarusCaughtInit = true;
  const STORAGE_KEY = 'lazarusCaught';
  function loadState() {
    try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}'); } catch (_) { return {}; }
  }
  function saveState(state) {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); } catch (_) {}
  }
  function applyState() {
    const state = loadState();
    const boxes = Array.from(document.querySelectorAll('.caught-check'));
    const bySpecies = boxes.reduce((m, cb) => {
      const s = cb.dataset.species; if (!s) return m; (m[s] ||= []).push(cb); return m; }, {});
    boxes.forEach(cb => {
      const key = cb.dataset.species;
      cb.checked = !!state[key];
      cb.onchange = () => {
        const checked = cb.checked;
        if (checked) state[key] = true; else delete state[key];
        saveState(state);
        (bySpecies[key] || []).forEach(other => { if (other !== cb) other.checked = checked; });
      };
    });
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', applyState);
  } else {
    applyState();
  }
})();
</script>
</div>
</div>
</div>
<style>
#pokemon-tabs-chespin-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-chespin-panel-0 { display: block; }
#pokemon-tabs-chespin-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-chespin-panel-1 { display: block; }
#pokemon-tabs-chespin-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-chespin-panel-2 { display: block; }
#pokemon-tabs-chespin-tab-3:checked ~ .pokemon-tab-panels #pokemon-tabs-chespin-panel-3 { display: block; }
</style>
</details>
