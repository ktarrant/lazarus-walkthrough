<div class="pokemon-tabs" id="pokemon-tabs-voltorb-100">
<input type="radio" name="pokemon-tabs-voltorb-100-group" id="pokemon-tabs-voltorb-100-tab-0" checked>
<label for="pokemon-tabs-voltorb-100-tab-0">Voltorb</label>
<input type="radio" name="pokemon-tabs-voltorb-100-group" id="pokemon-tabs-voltorb-100-tab-1">
<label for="pokemon-tabs-voltorb-100-tab-1">Electrode</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-voltorb-100-panel-0">
## Voltorb (#100)
<details class="pokemon-card-container" open>
<summary>Voltorb overview</summary>
_Ball Pokémon_ • Types: Electric • Egg Groups: Mineral • Base EXP: 66

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Soundproof
- Static
- Aftermath *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Voltorb → Electrode (Lv 30)

**Encounter Locations**
- Jusmail Town — Grass (Night) (10%)
- Sea of Vulcai — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">40</span> |
| Attack | <span class="stat-value stat-low">30</span> |
| Defense | <span class="stat-value stat-low">50</span> |
| Sp. Atk | <span class="stat-value stat-mid">55</span> |
| Sp. Def | <span class="stat-value stat-mid">55</span> |
| Speed | <span class="stat-value stat-high">100</span> |
| Total | <span class="stat-value stat-mid">330</span> |
</div>
</div>

**Flavor Text**
> It was discovered when Poké Balls were introduced. It is said that there is some connection.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-voltorb-100-panel-1">
## Electrode (#101)
<details class="pokemon-card-container" open>
<summary>Electrode overview</summary>
_Ball Pokémon_ • Types: Electric • Egg Groups: Mineral • Base EXP: 172

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Soundproof
- Static
- Aftermath *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Voltorb → Electrode (Lv 30)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">60</span> |
| Attack | <span class="stat-value stat-low">50</span> |
| Defense | <span class="stat-value stat-mid">70</span> |
| Sp. Atk | <span class="stat-value stat-mid">80</span> |
| Sp. Def | <span class="stat-value stat-mid">80</span> |
| Speed | <span class="stat-value stat-high">150</span> |
| Total | <span class="stat-value stat-mid">490</span> |
</div>
</div>

**Flavor Text**
> It explodes in response to even minor stimuli. It is feared, with the nickname of “The Bomb Ball.”
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-voltorb-100-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-voltorb-100-panel-0 { display: block; }
#pokemon-tabs-voltorb-100-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-voltorb-100-panel-1 { display: block; }
</style>
