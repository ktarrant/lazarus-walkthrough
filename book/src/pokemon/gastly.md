<div class="pokemon-tabs" id="pokemon-tabs-gastly-92">
<input type="radio" name="pokemon-tabs-gastly-92-group" id="pokemon-tabs-gastly-92-tab-0" checked>
<label for="pokemon-tabs-gastly-92-tab-0">Gastly</label>
<input type="radio" name="pokemon-tabs-gastly-92-group" id="pokemon-tabs-gastly-92-tab-1">
<label for="pokemon-tabs-gastly-92-tab-1">Haunter</label>
<input type="radio" name="pokemon-tabs-gastly-92-group" id="pokemon-tabs-gastly-92-tab-2">
<label for="pokemon-tabs-gastly-92-tab-2">Gengar</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-gastly-92-panel-0">
## Gastly (#092)
<details class="pokemon-card-container" open>
<summary>Gastly overview</summary>
_Gas Pokémon_ • Types: Ghost / Poison • Egg Groups: Indeterminate • Base EXP: 62

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Levitate

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Fighting (0×)
- Poison (0.25×)
- Bug (0.25×)
- Fairy (0.5×)

*Weak to*
- Ground (2×)
- Psychic (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Gastly → Haunter (Lv 25) → Gengar (Trade)

**Encounter Locations**
- Erinys Path (East) — Grass (Day) (20%)
- Erinys Path (East) — Grass (Night) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">30</span> |
| Attack | <span class="stat-value stat-low">35</span> |
| Defense | <span class="stat-value stat-low">30</span> |
| Sp. Atk | <span class="stat-value stat-high">100</span> |
| Sp. Def | <span class="stat-value stat-low">35</span> |
| Speed | <span class="stat-value stat-mid">80</span> |
| Total | <span class="stat-value stat-mid">310</span> |

**Notable Level-Up Moves**
- Hypnosis (Lv 1)
- Lick (Lv 1)
- Spite (Lv 8)
- Mean Look (Lv 13)
- Curse (Lv 16)
- Night Shade (Lv 21)
- Confuse Ray (Lv 28)
- Dream Eater (Lv 33)
- Destiny Bond (Lv 36)
</div>
</div>

**Flavor Text**
> Born from gases, anyone would faint if engulfed by its gaseous body, which contains poison.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-gastly-92-panel-1">
## Haunter (#093)
<details class="pokemon-card-container" open>
<summary>Haunter overview</summary>
_Gas Pokémon_ • Types: Ghost / Poison • Egg Groups: Indeterminate • Base EXP: 142

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Levitate

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Fighting (0×)
- Poison (0.25×)
- Bug (0.25×)
- Fairy (0.5×)

*Weak to*
- Ground (2×)
- Psychic (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Gastly → Haunter (Lv 25) → Gengar (Trade)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">45</span> |
| Attack | <span class="stat-value stat-low">50</span> |
| Defense | <span class="stat-value stat-low">45</span> |
| Sp. Atk | <span class="stat-value stat-high">115</span> |
| Sp. Def | <span class="stat-value stat-mid">55</span> |
| Speed | <span class="stat-value stat-high">95</span> |
| Total | <span class="stat-value stat-mid">405</span> |

**Notable Level-Up Moves**
- Hypnosis (Lv 1)
- Lick (Lv 1)
- Spite (Lv 1)
- Spite (Lv 8)
- Mean Look (Lv 13)
- Curse (Lv 16)
- Night Shade (Lv 21)
- Shadow Punch (Lv 25)
- Confuse Ray (Lv 31)
- Dream Eater (Lv 39)
- Destiny Bond (Lv 48)
</div>
</div>

**Flavor Text**
> Its tongue is made of gas. If licked, its victim starts shaking constantly until death eventually comes.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-gastly-92-panel-2">
## Gengar (#094)
<details class="pokemon-card-container" open>
<summary>Gengar overview</summary>
_Shadow Pokémon_ • Types: Ghost / Poison • Egg Groups: Indeterminate • Base EXP: 225

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Cursed Body

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Fighting (0×)
- Poison (0.25×)
- Bug (0.25×)
- Fairy (0.5×)

*Weak to*
- Ground (2×)
- Psychic (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Gastly → Haunter (Lv 25) → Gengar (Trade)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">60</span> |
| Attack | <span class="stat-value stat-mid">65</span> |
| Defense | <span class="stat-value stat-mid">60</span> |
| Sp. Atk | <span class="stat-value stat-high">130</span> |
| Sp. Def | <span class="stat-value stat-mid">75</span> |
| Speed | <span class="stat-value stat-high">110</span> |
| Total | <span class="stat-value stat-mid">500</span> |

**Notable Level-Up Moves**
- Hypnosis (Lv 1)
- Lick (Lv 1)
- Spite (Lv 1)
- Spite (Lv 8)
- Mean Look (Lv 13)
- Curse (Lv 16)
- Night Shade (Lv 21)
- Shadow Punch (Lv 25)
- Confuse Ray (Lv 31)
- Dream Eater (Lv 39)
- Destiny Bond (Lv 48)
</div>
</div>

**Flavor Text**
> On the night of a full moon, if shadows move on their own and laugh, it must be Gengar’s doing.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-gastly-92-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-gastly-92-panel-0 { display: block; }
#pokemon-tabs-gastly-92-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-gastly-92-panel-1 { display: block; }
#pokemon-tabs-gastly-92-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-gastly-92-panel-2 { display: block; }
</style>
