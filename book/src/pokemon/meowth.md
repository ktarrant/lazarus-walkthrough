<div class="pokemon-tabs" id="pokemon-tabs-meowth-52">
<input type="radio" name="pokemon-tabs-meowth-52-group" id="pokemon-tabs-meowth-52-tab-0" checked>
<label for="pokemon-tabs-meowth-52-tab-0">Meowth</label>
<input type="radio" name="pokemon-tabs-meowth-52-group" id="pokemon-tabs-meowth-52-tab-1">
<label for="pokemon-tabs-meowth-52-tab-1">Persian</label>
<input type="radio" name="pokemon-tabs-meowth-52-group" id="pokemon-tabs-meowth-52-tab-2">
<label for="pokemon-tabs-meowth-52-tab-2">Perrserker</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-meowth-52-panel-0">
## Meowth (#052)
<details class="pokemon-card-container" open>
<summary>Meowth overview</summary>
_Scratch Cat Pokémon_ • Types: Normal
Height: 0.4 m · Weight: 4.2 kg · Base EXP: 58

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Pickup
- Technician
- Unnerve *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Ghost (0×)

*Weak to*
- Fighting (2×)

**Evolution Paths**
- Meowth → Persian (Lv 28)
- Meowth → Perrserker (Lv 28)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 40 |
| Attack | 45 |
| Defense | 35 |
| Sp. Atk | 40 |
| Sp. Def | 40 |
| Speed | 90 |
| Total | 290 |

**Notable Level-Up Moves**
- Bite (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Fake Out (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Growl (Lv 1, Red Blue)
- Scratch (Lv 1, Red Blue)
- Feint (Lv 4, Sword Shield)
- Taunt (Lv 9, Lets Go Pikachu Lets Go Eevee)
</div>
</div>

**Flavor Text**
> It loves to collect shiny things. If it’s in a good mood, it might even let its Trainer have a look at its hoard of treasures.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-meowth-52-panel-1">
## Persian (#053)
<details class="pokemon-card-container" open>
<summary>Persian overview</summary>
_Classy Cat Pokémon_ • Types: Normal
Height: 1.0 m · Weight: 32.0 kg · Base EXP: 154

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Limber
- Technician
- Unnerve *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Ghost (0×)

*Weak to*
- Fighting (2×)

**Evolution Paths**
- Meowth → Persian (Lv 28)
- Meowth → Perrserker (Lv 28)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 65 |
| Attack | 70 |
| Defense | 60 |
| Sp. Atk | 65 |
| Sp. Def | 65 |
| Speed | 115 |
| Total | 440 |

**Notable Level-Up Moves**
- Amnesia (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Bite (Lv 1, Red Blue)
- Fake Out (Lv 1, Diamond Pearl)
- Feint (Lv 1, Sword Shield)
- Growl (Lv 1, Red Blue)
- Hypnosis (Lv 1, Lets Go Pikachu Lets Go Eevee)
</div>
</div>

**Flavor Text**
> Getting this prideful Pokémon to warm up to you takes a lot of effort, and it will claw at you the moment it gets annoyed.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-meowth-52-panel-2">
## Perrserker (#863)
<details class="pokemon-card-container" open>
<summary>Perrserker overview</summary>
_Viking Pokémon_ • Types: Steel
Height: 0.8 m · Weight: 28.0 kg · Base EXP: 154

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Battle Armor
- Tough Claws
- Steely Spirit *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Grass (0.5×)
- Ice (0.5×)
- Poison (0×)
- Flying (0.5×)
- Psychic (0.5×)
- Bug (0.5×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Fighting (2×)
- Ground (2×)

**Evolution Paths**
- Meowth → Persian (Lv 28)
- Meowth → Perrserker (Lv 28)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 70 |
| Attack | 110 |
| Defense | 100 |
| Sp. Atk | 50 |
| Sp. Def | 60 |
| Speed | 50 |
| Total | 440 |

**Notable Level-Up Moves**
- Fake Out (Lv 1, Sword Shield)
- Growl (Lv 1, Sword Shield)
- Hone Claws (Lv 1, Sword Shield)
- Iron Defense (Lv 1, Sword Shield)
- Iron Head (Lv 1, Sword Shield)
- Metal Burst (Lv 1, Sword Shield)
</div>
</div>

**Flavor Text**
> What appears to be an iron helmet is actually hardened hair. This Pokémon lives for the thrill of battle.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-meowth-52-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-meowth-52-panel-0 { display: block; }
#pokemon-tabs-meowth-52-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-meowth-52-panel-1 { display: block; }
#pokemon-tabs-meowth-52-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-meowth-52-panel-2 { display: block; }
</style>
