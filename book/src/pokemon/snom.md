<div class="pokemon-tabs" id="pokemon-tabs-snom-872">
<input type="radio" name="pokemon-tabs-snom-872-group" id="pokemon-tabs-snom-872-tab-0" checked>
<label for="pokemon-tabs-snom-872-tab-0">Snom</label>
<input type="radio" name="pokemon-tabs-snom-872-group" id="pokemon-tabs-snom-872-tab-1">
<label for="pokemon-tabs-snom-872-tab-1">Frosmoth</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-snom-872-panel-0">
## Snom (#872)
<details class="pokemon-card-container" open>
<summary>Snom overview</summary>
_Worm Pokémon_ • Types: Ice / Bug
Height: 0.3 m · Weight: 3.8 kg · Base EXP: 37

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shield Dust
- Ice Scales *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Ice (0.5×)
- Ground (0.5×)

*Weak to*
- Fire (4×)
- Flying (2×)
- Rock (4×)
- Steel (2×)

**Evolution Paths**
- Snom → Frosmoth (Friendship 160)

**Encounter Locations**
- Froslass Cavern BF2 — Grass (Day) (5%)
- Pythios Cemetery — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 30 |
| Attack | 25 |
| Defense | 35 |
| Sp. Atk | 45 |
| Sp. Def | 30 |
| Speed | 20 |
| Total | 185 |

**Notable Level-Up Moves**
- Powder Snow (Lv 1, Sword Shield)
- Struggle Bug (Lv 1, Sword Shield)
</div>
</div>

**Flavor Text**
> It spits out thread imbued with a frigid sort of energy and uses it to tie its body to branches, disguising itself as an icicle while it sleeps.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-snom-872-panel-1">
## Frosmoth (#873)
<details class="pokemon-card-container" open>
<summary>Frosmoth overview</summary>
_Frost Moth Pokémon_ • Types: Ice / Bug
Height: 1.3 m · Weight: 42.0 kg · Base EXP: 166

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shield Dust
- Ice Scales *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Ice (0.5×)
- Ground (0.5×)

*Weak to*
- Fire (4×)
- Flying (2×)
- Rock (4×)
- Steel (2×)

**Evolution Paths**
- Snom → Frosmoth (Friendship 160)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 70 |
| Attack | 65 |
| Defense | 60 |
| Sp. Atk | 125 |
| Sp. Def | 90 |
| Speed | 65 |
| Total | 475 |

**Notable Level-Up Moves**
- Attract (Lv 1, Sword Shield)
- Helping Hand (Lv 1, Sword Shield)
- Icy Wind (Lv 1, Sword Shield)
- Powder Snow (Lv 1, Sword Shield)
- Struggle Bug (Lv 1, Sword Shield)
- Stun Spore (Lv 4, Sword Shield)
</div>
</div>

**Flavor Text**
> Icy scales fall from its wings like snow as it flies over fields and mountains. The temperature of its wings is less than −290 degrees Fahrenheit.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-snom-872-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-snom-872-panel-0 { display: block; }
#pokemon-tabs-snom-872-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-snom-872-panel-1 { display: block; }
</style>
