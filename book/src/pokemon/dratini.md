<div class="pokemon-tabs" id="pokemon-tabs-dratini-147">
<input type="radio" name="pokemon-tabs-dratini-147-group" id="pokemon-tabs-dratini-147-tab-0" checked>
<label for="pokemon-tabs-dratini-147-tab-0">Dratini</label>
<input type="radio" name="pokemon-tabs-dratini-147-group" id="pokemon-tabs-dratini-147-tab-1">
<label for="pokemon-tabs-dratini-147-tab-1">Dragonair</label>
<input type="radio" name="pokemon-tabs-dratini-147-group" id="pokemon-tabs-dratini-147-tab-2">
<label for="pokemon-tabs-dratini-147-tab-2">Dragonite</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-dratini-147-panel-0">
## Dratini (#147)
<details class="pokemon-card-container" open>
<summary>Dratini overview</summary>
_Dragon Pokémon_ • Types: Dragon
Height: 1.8 m · Weight: 3.3 kg · Base EXP: 60

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shed Skin
- Marvel Scale *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)

*Weak to*
- Ice (2×)
- Dragon (2×)
- Fairy (2×)

**Evolution Paths**
- Dratini → Dragonair (Lv 30) → Dragonite (Lv 55)

**Encounter Locations**
- Fresco Isles — Fishing (5%)
- Fresco Isles — Surfing (5%)
- Pythios Cemetery — Fishing (20%)
- Pythios Cemetery — Fishing (15%)
- Péntepetal City — Surfing (9%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 41 |
| Attack | 64 |
| Defense | 45 |
| Sp. Atk | 50 |
| Sp. Def | 50 |
| Speed | 50 |
| Total | 300 |

**Notable Level-Up Moves**
- Leer (Lv 1, Red Blue)
- Wrap (Lv 1, Red Blue)
- Thunder Wave (Lv 5, Diamond Pearl)
- Twister (Lv 5, Sword Shield)
- Dragon Rage (Lv 14, Lets Go Pikachu Lets Go Eevee)
- Dragon Tail (Lv 15, Sword Shield)
</div>
</div>

**Flavor Text**
> Dratini dwells near bodies of rapidly flowing water, such as the plunge pools of waterfalls. As it grows, Dratini will shed its skin many times.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-dratini-147-panel-1">
## Dragonair (#148)
<details class="pokemon-card-container" open>
<summary>Dragonair overview</summary>
_Dragon Pokémon_ • Types: Dragon
Height: 4.0 m · Weight: 16.5 kg · Base EXP: 147

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shed Skin
- Marvel Scale *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)

*Weak to*
- Ice (2×)
- Dragon (2×)
- Fairy (2×)

**Evolution Paths**
- Dratini → Dragonair (Lv 30) → Dragonite (Lv 55)

**Encounter Locations**
- Davosi Straits — Surfing (5%)
- Pollen Road — Surfing (5%)
- Pythios Cemetery — Fishing (5%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 61 |
| Attack | 84 |
| Defense | 65 |
| Sp. Atk | 70 |
| Sp. Def | 70 |
| Speed | 70 |
| Total | 420 |

**Notable Level-Up Moves**
- Dragon Rage (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Leer (Lv 1, Red Blue)
- Thunder Wave (Lv 1, Red Blue)
- Twister (Lv 1, Gold Silver)
- Wrap (Lv 1, Red Blue)
- Dragon Tail (Lv 15, Sword Shield)
</div>
</div>

**Flavor Text**
> This Pokémon lives in pristine oceans and lakes. It can control the weather, and it uses this power to fly into the sky, riding on the wind.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-dratini-147-panel-2">
## Dragonite (#149)
<details class="pokemon-card-container" open>
<summary>Dragonite overview</summary>
_Dragon Pokémon_ • Types: Dragon / Flying
Height: 2.2 m · Weight: 210.0 kg · Base EXP: 270

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Inner Focus
- Multiscale *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Grass (0.25×)
- Fighting (0.5×)
- Ground (0×)
- Bug (0.5×)

*Weak to*
- Ice (4×)
- Rock (2×)
- Dragon (2×)
- Fairy (2×)

**Evolution Paths**
- Dratini → Dragonair (Lv 30) → Dragonite (Lv 55)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 91 |
| Attack | 134 |
| Defense | 95 |
| Sp. Atk | 100 |
| Sp. Def | 100 |
| Speed | 80 |
| Total | 600 |

**Notable Level-Up Moves**
- Agility (Lv 1, Red Blue)
- Aqua Jet (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Dragon Rage (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Extreme Speed (Lv 1, Sword Shield)
- Fire Punch (Lv 1, Diamond Pearl)
- Hurricane (Lv 1, X Y)
</div>
</div>

**Flavor Text**
> It’s a kindhearted Pokémon. If it spots a drowning person or Pokémon, Dragonite simply must help them.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-dratini-147-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-dratini-147-panel-0 { display: block; }
#pokemon-tabs-dratini-147-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-dratini-147-panel-1 { display: block; }
#pokemon-tabs-dratini-147-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-dratini-147-panel-2 { display: block; }
</style>
