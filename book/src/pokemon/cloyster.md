<div class="pokemon-tabs" id="pokemon-tabs-cloyster-91">
<input type="radio" name="pokemon-tabs-cloyster-91-group" id="pokemon-tabs-cloyster-91-tab-0">
<label for="pokemon-tabs-cloyster-91-tab-0">Shellder</label>
<input type="radio" name="pokemon-tabs-cloyster-91-group" id="pokemon-tabs-cloyster-91-tab-1" checked>
<label for="pokemon-tabs-cloyster-91-tab-1">Cloyster</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-cloyster-91-panel-0">
## Shellder (#090)
<details class="pokemon-card-container" open>
<summary>Shellder overview</summary>
_Bivalve Pokémon_ • Types: Water
Height: 0.3 m · Weight: 4.0 kg · Base EXP: 61

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shell Armor
- Skill Link
- Overcoat *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Ice (0.5×)
- Steel (0.5×)

*Weak to*
- Electric (2×)
- Grass (2×)

**Evolution Paths**
- Shellder → Cloyster (Use Water Stone)

**Encounter Locations**
- Corrin Crossing — Fishing (30%)
- Corrin Crossing — Fishing (20%)
- Davosi Straits — Fishing (60%)
- Erinys Path (East) — Surfing (10%)
- Froslass Cavern BF2 — Fishing (30%)
- Froslass Cavern BF2 — Fishing (40%)
- Froslass Cavern BF2 — Surfing (60%)
- Kipos Town — Fishing (70%)
- Nyx Trails — Fishing (30%)
- Nyx Trails — Fishing (20%)
- Palati City — Fishing (20%)
- Pollen Road — Fishing (60%)
- Port Pello — Fishing (30%)
- Port Pello — Fishing (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 30 |
| Attack | 65 |
| Defense | 100 |
| Sp. Atk | 45 |
| Sp. Def | 25 |
| Speed | 40 |
| Total | 305 |

**Notable Level-Up Moves**
- Tackle (Lv 1, Red Blue)
- Water Gun (Lv 1, Sun Moon)
- Withdraw (Lv 1, Red Blue)
- Ice Shard (Lv 8, Sword Shield)
- Icicle Spear (Lv 8, Firered Leafgreen)
- Supersonic (Lv 8, Diamond Pearl)
</div>
</div>

**Flavor Text**
> It swims facing backward by opening and closing its two-piece shell. It is surprisingly fast.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-cloyster-91-panel-1">
## Cloyster (#091)
<details class="pokemon-card-container" open>
<summary>Cloyster overview</summary>
_Bivalve Pokémon_ • Types: Water / Ice
Height: 1.5 m · Weight: 132.5 kg · Base EXP: 184

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shell Armor
- Skill Link
- Overcoat *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Water (0.5×)
- Ice (0.25×)

*Weak to*
- Electric (2×)
- Grass (2×)
- Fighting (2×)
- Rock (2×)

**Evolution Paths**
- Shellder → Cloyster (Use Water Stone)

**Encounter Locations**
- Davosi Straits — Fishing (15%)
- Pollen Road — Fishing (15%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 50 |
| Attack | 95 |
| Defense | 180 |
| Sp. Atk | 85 |
| Sp. Def | 45 |
| Speed | 70 |
| Total | 525 |

**Notable Level-Up Moves**
- Aurora Beam (Lv 1, Red Blue)
- Barrier (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Clamp (Lv 1, Red Blue)
- Hydro Pump (Lv 1, X Y)
- Ice Beam (Lv 1, Sword Shield)
- Ice Shard (Lv 1, Sword Shield)
</div>
</div>

**Flavor Text**
> Its shell is extremely hard. It cannot be shattered, even with a bomb. The shell opens only when it is attacking.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-cloyster-91-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-cloyster-91-panel-0 { display: block; }
#pokemon-tabs-cloyster-91-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-cloyster-91-panel-1 { display: block; }
</style>
