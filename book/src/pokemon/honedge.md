<div class="pokemon-tabs" id="pokemon-tabs-honedge-679">
<input type="radio" name="pokemon-tabs-honedge-679-group" id="pokemon-tabs-honedge-679-tab-0" checked>
<label for="pokemon-tabs-honedge-679-tab-0">Honedge</label>
<input type="radio" name="pokemon-tabs-honedge-679-group" id="pokemon-tabs-honedge-679-tab-1">
<label for="pokemon-tabs-honedge-679-tab-1">Doublade</label>
<input type="radio" name="pokemon-tabs-honedge-679-group" id="pokemon-tabs-honedge-679-tab-2">
<label for="pokemon-tabs-honedge-679-tab-2">Aegislash Shield</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-honedge-679-panel-0">
## Honedge (#679)
<details class="pokemon-card-container" open>
<summary>Honedge overview</summary>
_Sword Pokémon_ • Types: Steel / Ghost • Egg Groups: Mineral • Base EXP: 65

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- No Guard

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Ice (0.5×)
- Fighting (0×)
- Poison (0×)
- Flying (0.5×)
- Psychic (0.5×)
- Bug (0.25×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Ground (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Honedge → Doublade (Lv 35) → Aegislash (Use Dusk Stone)

**Encounter Locations**
- Pythios Town — Grass (Day) (2%)
- Pythios Town — Grass (Night) (2%)
- Sofos City — Grass (Day) (2%)
- Sofos City — Grass (Night) (2%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">45</span> |
| Attack | <span class="stat-value stat-mid">80</span> |
| Defense | <span class="stat-value stat-high">100</span> |
| Sp. Atk | <span class="stat-value stat-low">35</span> |
| Sp. Def | <span class="stat-value stat-low">37</span> |
| Speed | <span class="stat-value stat-low">28</span> |
| Total | <span class="stat-value stat-mid">325</span> |
</div>
</div>

**Flavor Text**
> Honedge’s soul once belonged to a person who was killed a long time ago by the sword that makes up Honedge’s body.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-honedge-679-panel-1">
## Doublade (#680)
<details class="pokemon-card-container" open>
<summary>Doublade overview</summary>
_Sword Pokémon_ • Types: Steel / Ghost • Egg Groups: Mineral • Base EXP: 157

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- No Guard

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Ice (0.5×)
- Fighting (0×)
- Poison (0×)
- Flying (0.5×)
- Psychic (0.5×)
- Bug (0.25×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Ground (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Honedge → Doublade (Lv 35) → Aegislash (Use Dusk Stone)

**Encounter Locations**
- Kaptara Island (East) — Grass (Day) (2%)
- Kaptara Island (East) — Grass (Night) (2%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">59</span> |
| Attack | <span class="stat-value stat-high">110</span> |
| Defense | <span class="stat-value stat-high">150</span> |
| Sp. Atk | <span class="stat-value stat-low">45</span> |
| Sp. Def | <span class="stat-value stat-low">49</span> |
| Speed | <span class="stat-value stat-low">35</span> |
| Total | <span class="stat-value stat-mid">448</span> |
</div>
</div>

**Flavor Text**
> Honedge evolves into twins. The two blades rub together to emit a metallic sound that unnerves opponents.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-honedge-679-panel-2">
## Aegislash Shield (#681)
<details class="pokemon-card-container" open>
<summary>Aegislash Shield overview</summary>
_Royal Sword Pokémon_ • Types: Steel / Ghost • Egg Groups: Mineral • Base EXP: 234

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Stance Change

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Grass (0.5×)
- Ice (0.5×)
- Fighting (0×)
- Poison (0×)
- Flying (0.5×)
- Psychic (0.5×)
- Bug (0.25×)
- Rock (0.5×)
- Dragon (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (2×)
- Ground (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Honedge → Doublade (Lv 35) → Aegislash (Use Dusk Stone)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">60</span> |
| Attack | <span class="stat-value stat-low">50</span> |
| Defense | <span class="stat-value stat-high">140</span> |
| Sp. Atk | <span class="stat-value stat-low">50</span> |
| Sp. Def | <span class="stat-value stat-high">140</span> |
| Speed | <span class="stat-value stat-mid">60</span> |
| Total | <span class="stat-value stat-mid">500</span> |
</div>
</div>

**Flavor Text**
> In this defensive stance, Aegislash uses its steel body and a force field of spectral power to reduce the damage of any attack.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-honedge-679-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-honedge-679-panel-0 { display: block; }
#pokemon-tabs-honedge-679-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-honedge-679-panel-1 { display: block; }
#pokemon-tabs-honedge-679-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-honedge-679-panel-2 { display: block; }
</style>
