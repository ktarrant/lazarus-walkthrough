<details class="pokemon-card-container" open>
<summary>Drakloak (#270)</summary>
<div class="pokemon-tabs" id="pokemon-tabs-drakloak">
<input type="radio" name="pokemon-tabs-drakloak-group" id="pokemon-tabs-drakloak-tab-0">
<label for="pokemon-tabs-drakloak-tab-0">Dreepy</label>
<input type="radio" name="pokemon-tabs-drakloak-group" id="pokemon-tabs-drakloak-tab-1" checked>
<label for="pokemon-tabs-drakloak-tab-1">Drakloak</label>
<input type="radio" name="pokemon-tabs-drakloak-group" id="pokemon-tabs-drakloak-tab-2">
<label for="pokemon-tabs-drakloak-tab-2">Dragapult</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-drakloak-panel-0">
Types: Dragon / Ghost • Egg Groups: Amorphous / Dragon

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Clear Body
- Infiltrator
- Cursed Body *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Fire (0.5×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Fighting (0×)
- Poison (0.5×)
- Bug (0.5×)

*Weak to*
- Ice (2×)
- Ghost (2×)
- Dragon (2×)
- Dark (2×)
- Fairy (2×)

**TM/HM Moves**
- TM17 - Protect
- TM32 - Double Team
- TM42 - Facade
- TM44 - Rest
- TM45 - Attract
- TM58 - Thunder Wave

**Encounter Locations**
- Fresco Isles — Grass (Night) (5%)
- Riverwalk Trail (South) — Grass (Night) (2%)
- Sea of Asteri (West) — Grass (Day) (2%)
- Sea of Asteri (West) — Grass (Night) (2%)
- Tower of Dioxippus — Grass (Night) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">28</span> |
| Attack | <span class="stat-value stat-mid">60</span> |
| Defense | <span class="stat-value stat-low">30</span> |
| Sp. Atk | <span class="stat-value stat-low">40</span> |
| Sp. Def | <span class="stat-value stat-low">30</span> |
| Speed | <span class="stat-value stat-mid">82</span> |
| Total | <span class="stat-value stat-low">270</span> |

**Level-Up Moves**
- Astonish (Lv 1)
- Infestation (Lv 1)
- Quick Attack (Lv 1)
- Bite (Lv 1)
- Water Gun (Lv 10)
- Flail (Lv 17)
- Hex (Lv 24)

**Egg Moves**
- Curse
- Grudge
- Confuse Ray
- Double Team
- Disable
- Dragon Tail
- Sucker Punch

**Tutor Moves**
- Endure
- Sleep Talk
- Snore
- Swift
</div>
</div>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-drakloak-panel-1">
Types: Dragon / Ghost • Egg Groups: Amorphous / Dragon

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Clear Body
- Infiltrator
- Cursed Body *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Fire (0.5×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Fighting (0×)
- Poison (0.5×)
- Bug (0.5×)

*Weak to*
- Ice (2×)
- Ghost (2×)
- Dragon (2×)
- Dark (2×)
- Fairy (2×)

**TM/HM Moves**
- TM11 - Sunny Day
- TM16 - Light Screen
- TM17 - Protect
- TM23 - Hex
- TM24 - Thunderbolt
- TM25 - Thunder
- TM30 - Shadow Ball
- TM32 - Double Team
- TM33 - Reflect
- TM35 - Flamethrower
- TM38 - Fire Blast
- TM42 - Facade
- TM44 - Rest
- TM45 - Attract
- TM46 - Thief
- TM47 - Steel Wing
- TM51 - Will-O-Wisp
- TM56 - Scald
- TM58 - Thunder Wave
- TM60 - Dragon Dance
- HM03 - Surf
- HM08 - Dive

**Evolution Info**
Lv. 28

**Encounter Locations**
- Tower of Dioxippus — Grass (Night) (5%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">68</span> |
| Attack | <span class="stat-value stat-mid">90</span> |
| Defense | <span class="stat-value stat-low">50</span> |
| Sp. Atk | <span class="stat-value stat-mid">60</span> |
| Sp. Def | <span class="stat-value stat-low">50</span> |
| Speed | <span class="stat-value stat-high">102</span> |
| Total | <span class="stat-value stat-mid">420</span> |

**Level-Up Moves**
- Dragon Breath (Lv Evo)
- Astonish (Lv 1)
- Infestation (Lv 1)
- Quick Attack (Lv 1)
- Bite (Lv 1)
- Lock-On (Lv 6)
- Assurance (Lv 12)
- Hex (Lv 18)
- Agility (Lv 24)
- Double Hit (Lv 30)
- U-Turn (Lv 36)
- Dragon Pulse (Lv 38)
- Dragon Dance (Lv 44)
- Phantom Force (Lv 48)
- Body Slam (Lv 54)
- Dragon Darts (Lv 60)
- Dragon Rush (Lv 61)
- Double-Edge (Lv 66)
- Last Resort (Lv 72)

**Egg Moves**
- Curse
- Grudge
- Confuse Ray
- Double Team
- Disable
- Dragon Tail
- Sucker Punch

**Tutor Moves**
- Double-Edge
- Endure
- Sleep Talk
- Snore
- Swift
</div>
</div>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-drakloak-panel-2">
Types: Dragon / Ghost • Egg Groups: Amorphous / Dragon

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Clear Body
- Infiltrator
- Cursed Body *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Fire (0.5×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Fighting (0×)
- Poison (0.5×)
- Bug (0.5×)

*Weak to*
- Ice (2×)
- Ghost (2×)
- Dragon (2×)
- Dark (2×)
- Fairy (2×)

**TM/HM Moves**
- TM02 - Dragon Claw
- TM11 - Sunny Day
- TM16 - Light Screen
- TM17 - Protect
- TM22 - Solar Beam
- TM23 - Hex
- TM24 - Thunderbolt
- TM25 - Thunder
- TM30 - Shadow Ball
- TM32 - Double Team
- TM33 - Reflect
- TM35 - Flamethrower
- TM38 - Fire Blast
- TM42 - Facade
- TM44 - Rest
- TM45 - Attract
- TM46 - Thief
- TM47 - Steel Wing
- TM51 - Will-O-Wisp
- TM56 - Scald
- TM58 - Thunder Wave
- TM60 - Dragon Dance
- HM02 - Fly
- HM03 - Surf
- HM08 - Dive

**Evolution Info**
Lv. 60
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">88</span> |
| Attack | <span class="stat-value stat-high">120</span> |
| Defense | <span class="stat-value stat-mid">75</span> |
| Sp. Atk | <span class="stat-value stat-high">100</span> |
| Sp. Def | <span class="stat-value stat-mid">75</span> |
| Speed | <span class="stat-value stat-high">142</span> |
| Total | <span class="stat-value stat-high">600</span> |

**Level-Up Moves**
- Dragon Darts (Lv Evo)
- Dragon Breath (Lv 1)
- Sucker Punch (Lv 1)
- Astonish (Lv 1)
- Infestation (Lv 1)
- Quick Attack (Lv 1)
- Bite (Lv 1)
- Lock-On (Lv 6)
- Assurance (Lv 12)
- Hex (Lv 18)
- Agility (Lv 24)
- Double Hit (Lv 30)
- U-Turn (Lv 36)
- Dragon Pulse (Lv 38)
- Dragon Dance (Lv 44)
- Phantom Force (Lv 48)
- Body Slam (Lv 54)
- Dragon Darts (Lv 60)
- Dragon Rush (Lv 61)
- Double-Edge (Lv 66)
- Last Resort (Lv 72)

**Egg Moves**
- Curse
- Grudge
- Confuse Ray
- Double Team
- Disable
- Dragon Tail
- Sucker Punch

**Tutor Moves**
- Body Slam
- Double-Edge
- Endure
- Sleep Talk
- Snore
- Swift
</div>
</div>
</div>
</div>
</div>
<style>
#pokemon-tabs-drakloak-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-drakloak-panel-0 { display: block; }
#pokemon-tabs-drakloak-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-drakloak-panel-1 { display: block; }
#pokemon-tabs-drakloak-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-drakloak-panel-2 { display: block; }
</style>
</details>
