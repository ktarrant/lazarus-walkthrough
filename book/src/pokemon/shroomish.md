<div class="pokemon-tabs" id="pokemon-tabs-shroomish-285">
<input type="radio" name="pokemon-tabs-shroomish-285-group" id="pokemon-tabs-shroomish-285-tab-0" checked>
<label for="pokemon-tabs-shroomish-285-tab-0">Shroomish</label>
<input type="radio" name="pokemon-tabs-shroomish-285-group" id="pokemon-tabs-shroomish-285-tab-1">
<label for="pokemon-tabs-shroomish-285-tab-1">Breloom</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-shroomish-285-panel-0">
## Shroomish (#285)
<details class="pokemon-card-container" open>
<summary>Shroomish overview</summary>
_Mushroom Pokémon_ • Types: Grass
Height: 0.4 m · Weight: 4.5 kg · Base EXP: 59

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Effect Spore
- Poison Heal
- Quick Feet *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ground (0.5×)

*Weak to*
- Fire (2×)
- Ice (2×)
- Poison (2×)
- Flying (2×)
- Bug (2×)

**Evolution Paths**
- Shroomish → Breloom (Lv 23)

**Encounter Locations**
- Acrisia City — Grass (Day) (10%)
- Acrisia City — Grass (Night) (10%)
- Wanderer's Woods (North) — Grass (Day) (20%)
- Wanderer's Woods (North) — Grass (Night) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 40 |
| Defense | 60 |
| Sp. Atk | 40 |
| Sp. Def | 60 |
| Speed | 35 |
| Total | 295 |

**Notable Level-Up Moves**
- Absorb (Lv 1, Ruby Sapphire)
- Tackle (Lv 1, Omega Ruby Alpha Sapphire)
- Stun Spore (Lv 5, Omega Ruby Alpha Sapphire)
- Leech Seed (Lv 8, Omega Ruby Alpha Sapphire)
- Mega Drain (Lv 12, Omega Ruby Alpha Sapphire)
- Headbutt (Lv 15, Omega Ruby Alpha Sapphire)
</div>
</div>

**Flavor Text**
> It prefers damp places. By day it remains still in the forest shade. It releases toxic powder from its head.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-shroomish-285-panel-1">
## Breloom (#286)
<details class="pokemon-card-container" open>
<summary>Breloom overview</summary>
_Mushroom Pokémon_ • Types: Grass / Fighting
Height: 1.2 m · Weight: 39.2 kg · Base EXP: 161

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Effect Spore
- Poison Heal
- Technician *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)
- Ground (0.5×)
- Rock (0.5×)
- Dark (0.5×)

*Weak to*
- Fire (2×)
- Ice (2×)
- Poison (2×)
- Flying (4×)
- Psychic (2×)
- Fairy (2×)

**Evolution Paths**
- Shroomish → Breloom (Lv 23)

**Encounter Locations**
- Lastlight Road — Grass (Day) (5%)
- Lastlight Road — Grass (Night) (5%)
- Nyx Trails — Grass (Day) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 130 |
| Defense | 80 |
| Sp. Atk | 60 |
| Sp. Def | 60 |
| Speed | 70 |
| Total | 460 |

**Notable Level-Up Moves**
- Absorb (Lv 1, Ruby Sapphire)
- Giga Drain (Lv 1, Brilliant Diamond And Shining Pearl)
- Growth (Lv 1, Brilliant Diamond And Shining Pearl)
- Leech Seed (Lv 1, Ruby Sapphire)
- Mach Punch (Lv 1, Sun Moon)
- Poison Powder (Lv 1, Brilliant Diamond And Shining Pearl)
</div>
</div>

**Flavor Text**
> Its short arms stretch when it throws punches. Its technique is equal to that of pro boxers.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-shroomish-285-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-shroomish-285-panel-0 { display: block; }
#pokemon-tabs-shroomish-285-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-shroomish-285-panel-1 { display: block; }
</style>
