<div class="pokemon-tabs" id="pokemon-tabs-electrode-101">
<input type="radio" name="pokemon-tabs-electrode-101-group" id="pokemon-tabs-electrode-101-tab-0">
<label for="pokemon-tabs-electrode-101-tab-0">Voltorb</label>
<input type="radio" name="pokemon-tabs-electrode-101-group" id="pokemon-tabs-electrode-101-tab-1" checked>
<label for="pokemon-tabs-electrode-101-tab-1">Electrode</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-electrode-101-panel-0">
## Voltorb (#100)
<details class="pokemon-card-container" open>
<summary>Voltorb overview</summary>
_Ball Pokémon_ • Types: Electric
Height: 0.5 m · Weight: 10.4 kg · Base EXP: 66

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Soundproof
- Static
- Aftermath *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Voltorb → Electrode (Lv 30)

**Encounter Locations**
- Jusmail Town — Grass (Night) (10%)
- Sea of Vulcai — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 40 |
| Attack | 30 |
| Defense | 50 |
| Sp. Atk | 55 |
| Sp. Def | 55 |
| Speed | 100 |
| Total | 330 |

**Notable Level-Up Moves**
- Charge (Lv 1, Ruby Sapphire)
- Screech (Lv 1, Red Blue)
- Tackle (Lv 1, Red Blue)
- Light Screen (Lv 3, Lets Go Pikachu Lets Go Eevee)
- Sonic Boom (Lv 4, Omega Ruby Alpha Sapphire)
- Thunder Shock (Lv 4, Brilliant Diamond And Shining Pearl)
</div>
</div>

**Flavor Text**
> It was discovered when Poké Balls were introduced. It is said that there is some connection.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-electrode-101-panel-1">
## Electrode (#101)
<details class="pokemon-card-container" open>
<summary>Electrode overview</summary>
_Ball Pokémon_ • Types: Electric
Height: 1.2 m · Weight: 66.6 kg · Base EXP: 172

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Soundproof
- Static
- Aftermath *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Electric (0.5×)
- Flying (0.5×)
- Steel (0.5×)

*Weak to*
- Ground (2×)

**Evolution Paths**
- Voltorb → Electrode (Lv 30)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 50 |
| Defense | 70 |
| Sp. Atk | 80 |
| Sp. Def | 80 |
| Speed | 150 |
| Total | 490 |

**Notable Level-Up Moves**
- Charge (Lv 1, Ruby Sapphire)
- Eerie Impulse (Lv 1, Sun Moon)
- Light Screen (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Magnetic Flux (Lv 1, X Y)
- Screech (Lv 1, Red Blue)
- Self Destruct (Lv 1, Gold Silver)
</div>
</div>

**Flavor Text**
> It explodes in response to even minor stimuli. It is feared, with the nickname of “The Bomb Ball.”
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-electrode-101-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-electrode-101-panel-0 { display: block; }
#pokemon-tabs-electrode-101-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-electrode-101-panel-1 { display: block; }
</style>
