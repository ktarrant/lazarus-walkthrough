<div class="pokemon-tabs" id="pokemon-tabs-seel-86">
<input type="radio" name="pokemon-tabs-seel-86-group" id="pokemon-tabs-seel-86-tab-0" checked>
<label for="pokemon-tabs-seel-86-tab-0">Seel</label>
<input type="radio" name="pokemon-tabs-seel-86-group" id="pokemon-tabs-seel-86-tab-1">
<label for="pokemon-tabs-seel-86-tab-1">Dewgong</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-seel-86-panel-0">
## Seel (#086)
<details class="pokemon-card-container" open>
<summary>Seel overview</summary>
_Sea Lion Pokémon_ • Types: Water
Height: 1.1 m · Weight: 90.0 kg · Base EXP: 65

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Thick Fat
- Hydration
- Ice Body *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Ice (0.5×)
- Steel (0.5×)

*Weak to*
- Electric (2×)
- Grass (2×)

**Evolution Paths**
- Seel → Dewgong (Lv 34)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 65 |
| Attack | 45 |
| Defense | 55 |
| Sp. Atk | 45 |
| Sp. Def | 70 |
| Speed | 45 |
| Total | 325 |

**Notable Level-Up Moves**
- Headbutt (Lv 1, Red Blue)
- Growl (Lv 3, Diamond Pearl)
- Charm (Lv 7, Scarlet Violet)
- Water Gun (Lv 7, Brilliant Diamond And Shining Pearl)
- Water Sport (Lv 7, Diamond Pearl)
- Encore (Lv 8, Lets Go Pikachu Lets Go Eevee)
</div>
</div>

**Flavor Text**
> A Pokémon that lives on icebergs. It swims in the sea using the point on its head to break up ice.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-seel-86-panel-1">
## Dewgong (#087)
<details class="pokemon-card-container" open>
<summary>Dewgong overview</summary>
_Sea Lion Pokémon_ • Types: Water / Ice
Height: 1.7 m · Weight: 120.0 kg · Base EXP: 166

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Thick Fat
- Hydration
- Ice Body *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Water (0.5×)
- Ice (0.25×)

*Weak to*
- Electric (2×)
- Grass (2×)
- Fighting (2×)
- Rock (2×)

**Evolution Paths**
- Seel → Dewgong (Lv 34)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 90 |
| Attack | 70 |
| Defense | 80 |
| Sp. Atk | 70 |
| Sp. Def | 95 |
| Speed | 70 |
| Total | 475 |

**Notable Level-Up Moves**
- Aqua Jet (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Aurora Beam (Lv 1, Red Blue)
- Bubble Beam (Lv 1, Brilliant Diamond And Shining Pearl)
- Encore (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Fake Out (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Growl (Lv 1, Red Blue)
</div>
</div>

**Flavor Text**
> Its body is covered with a pure white fur. The colder the weather, the more active it becomes.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-seel-86-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-seel-86-panel-0 { display: block; }
#pokemon-tabs-seel-86-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-seel-86-panel-1 { display: block; }
</style>
