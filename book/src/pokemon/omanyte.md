<div class="pokemon-tabs" id="pokemon-tabs-omanyte-138">
<input type="radio" name="pokemon-tabs-omanyte-138-group" id="pokemon-tabs-omanyte-138-tab-0" checked>
<label for="pokemon-tabs-omanyte-138-tab-0">Omanyte</label>
<input type="radio" name="pokemon-tabs-omanyte-138-group" id="pokemon-tabs-omanyte-138-tab-1">
<label for="pokemon-tabs-omanyte-138-tab-1">Omastar</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-omanyte-138-panel-0">
## Omanyte (#138)
<details class="pokemon-card-container" open>
<summary>Omanyte overview</summary>
_Spiral Pokémon_ • Types: Rock / Water
Height: 0.4 m · Weight: 7.5 kg · Base EXP: 71

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Swift Swim
- Shell Armor
- Weak Armor *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Fire (0.25×)
- Ice (0.5×)
- Poison (0.5×)
- Flying (0.5×)

*Weak to*
- Electric (2×)
- Grass (4×)
- Fighting (2×)
- Ground (2×)

**Evolution Paths**
- Omanyte → Omastar (Lv 40)

**Encounter Locations**
- Erinys Path (West) — Grass (Day) (10%)
- Erinys Path (West) — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 35 |
| Attack | 40 |
| Defense | 100 |
| Sp. Atk | 90 |
| Sp. Def | 55 |
| Speed | 35 |
| Total | 355 |

**Notable Level-Up Moves**
- Bind (Lv 1, Sword Shield)
- Constrict (Lv 1, Gold Silver)
- Water Gun (Lv 1, Red Blue)
- Withdraw (Lv 1, Red Blue)
- Rollout (Lv 5, Sword Shield)
- Bite (Lv 7, Diamond Pearl)
</div>
</div>

**Flavor Text**
> Because some Omanyte manage to escape after being restored or are released into the wild by people, this species is becoming a problem.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-omanyte-138-panel-1">
## Omastar (#139)
<details class="pokemon-card-container" open>
<summary>Omastar overview</summary>
_Spiral Pokémon_ • Types: Rock / Water
Height: 1.0 m · Weight: 35.0 kg · Base EXP: 173

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Swift Swim
- Shell Armor
- Weak Armor *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Fire (0.25×)
- Ice (0.5×)
- Poison (0.5×)
- Flying (0.5×)

*Weak to*
- Electric (2×)
- Grass (4×)
- Fighting (2×)
- Ground (2×)

**Evolution Paths**
- Omanyte → Omastar (Lv 40)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 70 |
| Attack | 60 |
| Defense | 125 |
| Sp. Atk | 115 |
| Sp. Def | 70 |
| Speed | 55 |
| Total | 495 |

**Notable Level-Up Moves**
- Bide (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Bind (Lv 1, Sword Shield)
- Bite (Lv 1, Gold Silver)
- Constrict (Lv 1, Gold Silver)
- Crunch (Lv 1, Sword Shield)
- Horn Attack (Lv 1, Red Blue)
</div>
</div>

**Flavor Text**
> Weighed down by a large and heavy shell, Omastar couldn’t move very fast. Some say it went extinct because it was unable to catch food.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-omanyte-138-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-omanyte-138-panel-0 { display: block; }
#pokemon-tabs-omanyte-138-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-omanyte-138-panel-1 { display: block; }
</style>
