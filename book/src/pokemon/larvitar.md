<div class="pokemon-tabs" id="pokemon-tabs-larvitar-246">
<input type="radio" name="pokemon-tabs-larvitar-246-group" id="pokemon-tabs-larvitar-246-tab-0" checked>
<label for="pokemon-tabs-larvitar-246-tab-0">Larvitar</label>
<input type="radio" name="pokemon-tabs-larvitar-246-group" id="pokemon-tabs-larvitar-246-tab-1">
<label for="pokemon-tabs-larvitar-246-tab-1">Pupitar</label>
<input type="radio" name="pokemon-tabs-larvitar-246-group" id="pokemon-tabs-larvitar-246-tab-2">
<label for="pokemon-tabs-larvitar-246-tab-2">Tyranitar</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-larvitar-246-panel-0">
## Larvitar (#246)
<details class="pokemon-card-container" open>
<summary>Larvitar overview</summary>
_Rock Skin Pokémon_ • Types: Rock / Ground
Height: 0.6 m · Weight: 72.0 kg · Base EXP: 60

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Guts
- Sand Veil *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Fire (0.5×)
- Electric (0×)
- Poison (0.25×)
- Flying (0.5×)
- Rock (0.5×)

*Weak to*
- Water (4×)
- Grass (4×)
- Ice (2×)
- Fighting (2×)
- Ground (2×)
- Steel (2×)

**Evolution Paths**
- Larvitar → Pupitar (Lv 30) → Tyranitar (Lv 55)

**Encounter Locations**
- Fresco Isles — Grass (Day) (2%)
- Fresco Isles — Grass (Night) (2%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 50 |
| Attack | 64 |
| Defense | 50 |
| Sp. Atk | 45 |
| Sp. Def | 50 |
| Speed | 41 |
| Total | 300 |

**Notable Level-Up Moves**
- Bite (Lv 1, Gold Silver)
- Leer (Lv 1, Gold Silver)
- Tackle (Lv 1, Sword Shield)
- Rock Throw (Lv 3, Sword Shield)
- Sandstorm (Lv 5, Diamond Pearl)
- Payback (Lv 6, Sword Shield)
</div>
</div>

**Flavor Text**
> Born deep underground, it comes aboveground and becomes a pupa once it has finished eating the surrounding soil.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-larvitar-246-panel-1">
## Pupitar (#247)
<details class="pokemon-card-container" open>
<summary>Pupitar overview</summary>
_Hard Shell Pokémon_ • Types: Rock / Ground
Height: 1.2 m · Weight: 152.0 kg · Base EXP: 144

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shed Skin

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Fire (0.5×)
- Electric (0×)
- Poison (0.25×)
- Flying (0.5×)
- Rock (0.5×)

*Weak to*
- Water (4×)
- Grass (4×)
- Ice (2×)
- Fighting (2×)
- Ground (2×)
- Steel (2×)

**Evolution Paths**
- Larvitar → Pupitar (Lv 30) → Tyranitar (Lv 55)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 70 |
| Attack | 84 |
| Defense | 70 |
| Sp. Atk | 65 |
| Sp. Def | 70 |
| Speed | 51 |
| Total | 410 |

**Notable Level-Up Moves**
- Bite (Lv 1, Gold Silver)
- Iron Defense (Lv 1, Sword Shield)
- Leer (Lv 1, Gold Silver)
- Payback (Lv 1, Sword Shield)
- Rock Throw (Lv 1, Sword Shield)
- Sandstorm (Lv 1, Gold Silver)
</div>
</div>

**Flavor Text**
> Even sealed in its shell, it can move freely. Hard and fast, it has outstanding destructive power.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-larvitar-246-panel-2">
## Tyranitar (#248)
<details class="pokemon-card-container" open>
<summary>Tyranitar overview</summary>
_Armor Pokémon_ • Types: Rock / Dark
Height: 2.0 m · Weight: 202.0 kg · Base EXP: 270

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Sand Stream
- Unnerve *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Fire (0.5×)
- Poison (0.5×)
- Flying (0.5×)
- Psychic (0×)
- Ghost (0.5×)
- Dark (0.5×)

*Weak to*
- Water (2×)
- Grass (2×)
- Fighting (4×)
- Ground (2×)
- Bug (2×)
- Steel (2×)
- Fairy (2×)

**Evolution Paths**
- Larvitar → Pupitar (Lv 30) → Tyranitar (Lv 55)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 100 |
| Attack | 134 |
| Defense | 110 |
| Sp. Atk | 95 |
| Sp. Def | 100 |
| Speed | 61 |
| Total | 600 |

**Notable Level-Up Moves**
- Bite (Lv 1, Gold Silver)
- Dark Pulse (Lv 1, Scarlet Violet)
- Fire Fang (Lv 1, Diamond Pearl)
- Ice Fang (Lv 1, Diamond Pearl)
- Iron Defense (Lv 1, Sword Shield)
- Leer (Lv 1, Gold Silver)
</div>
</div>

**Flavor Text**
> Its body can’t be harmed by any sort of attack, so it is very eager to make challenges against enemies.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-larvitar-246-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-larvitar-246-panel-0 { display: block; }
#pokemon-tabs-larvitar-246-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-larvitar-246-panel-1 { display: block; }
#pokemon-tabs-larvitar-246-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-larvitar-246-panel-2 { display: block; }
</style>
