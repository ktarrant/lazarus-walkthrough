<div class="pokemon-tabs" id="pokemon-tabs-lanturn-171">
<input type="radio" name="pokemon-tabs-lanturn-171-group" id="pokemon-tabs-lanturn-171-tab-0">
<label for="pokemon-tabs-lanturn-171-tab-0">Chinchou</label>
<input type="radio" name="pokemon-tabs-lanturn-171-group" id="pokemon-tabs-lanturn-171-tab-1" checked>
<label for="pokemon-tabs-lanturn-171-tab-1">Lanturn</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-lanturn-171-panel-0">
## Chinchou (#170)
<details class="pokemon-card-container" open>
<summary>Chinchou overview</summary>
_Angler Pokémon_ • Types: Water / Electric
Height: 0.5 m · Weight: 12.0 kg · Base EXP: 66

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Volt Absorb
- Illuminate
- Water Absorb *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Ice (0.5×)
- Flying (0.5×)
- Steel (0.25×)

*Weak to*
- Grass (2×)
- Ground (2×)

**Evolution Paths**
- Chinchou → Lanturn (Lv 27)

**Encounter Locations**
- Fresco Isles — Fishing (15%)
- Marmaro Island — Surfing (60%)
- Myrrini Island — Fishing (15%)
- Sea of Asteri (East) — Fishing (15%)
- Sea of Asteri (West) — Fishing (15%)
- Sea of Vulcai — Fishing (15%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 75 |
| Attack | 38 |
| Defense | 38 |
| Sp. Atk | 56 |
| Sp. Def | 56 |
| Speed | 67 |
| Total | 330 |

**Notable Level-Up Moves**
- Bubble (Lv 1, Gold Silver)
- Supersonic (Lv 1, Diamond Pearl)
- Thunder Wave (Lv 1, Gold Silver)
- Water Gun (Lv 1, X Y)
- Electro Ball (Lv 4, Sword Shield)
- Flail (Lv 9, Diamond Pearl)
</div>
</div>

**Flavor Text**
> Its antennae, which evolved from a fin, have both positive and negative charges flowing through them.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-lanturn-171-panel-1">
## Lanturn (#171)
<details class="pokemon-card-container" open>
<summary>Lanturn overview</summary>
_Light Pokémon_ • Types: Water / Electric
Height: 1.2 m · Weight: 22.5 kg · Base EXP: 161

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Volt Absorb
- Illuminate
- Water Absorb *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Ice (0.5×)
- Flying (0.5×)
- Steel (0.25×)

*Weak to*
- Grass (2×)
- Ground (2×)

**Evolution Paths**
- Chinchou → Lanturn (Lv 27)

**Encounter Locations**
- Myrrini Island — Fishing (5%)
- Sea of Asteri (East) — Fishing (5%)
- Sea of Asteri (West) — Fishing (5%)
- Sea of Vulcai — Fishing (5%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 125 |
| Attack | 58 |
| Defense | 58 |
| Sp. Atk | 76 |
| Sp. Def | 76 |
| Speed | 67 |
| Total | 460 |

**Notable Level-Up Moves**
- Bubble (Lv 1, Gold Silver)
- Eerie Impulse (Lv 1, X Y)
- Electro Ball (Lv 1, Omega Ruby Alpha Sapphire)
- Spit Up (Lv 1, Sun Moon)
- Spotlight (Lv 1, Ultra Sun Ultra Moon)
- Stockpile (Lv 1, Sun Moon)
</div>
</div>

**Flavor Text**
> The light it emits is so bright that it can illuminate the sea’s surface from a depth of over three miles.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-lanturn-171-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-lanturn-171-panel-0 { display: block; }
#pokemon-tabs-lanturn-171-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-lanturn-171-panel-1 { display: block; }
</style>
