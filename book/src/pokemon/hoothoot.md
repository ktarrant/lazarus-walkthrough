<div class="pokemon-tabs" id="pokemon-tabs-hoothoot-163">
<input type="radio" name="pokemon-tabs-hoothoot-163-group" id="pokemon-tabs-hoothoot-163-tab-0" checked>
<label for="pokemon-tabs-hoothoot-163-tab-0">Hoothoot</label>
<input type="radio" name="pokemon-tabs-hoothoot-163-group" id="pokemon-tabs-hoothoot-163-tab-1">
<label for="pokemon-tabs-hoothoot-163-tab-1">Noctowl</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-hoothoot-163-panel-0">
## Hoothoot (#163)
<details class="pokemon-card-container" open>
<summary>Hoothoot overview</summary>
_Owl Pokémon_ • Types: Normal / Flying
Height: 0.7 m · Weight: 21.2 kg · Base EXP: 52

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Insomnia
- Keen Eye
- Tinted Lens *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Ground (0×)
- Bug (0.5×)
- Ghost (0×)

*Weak to*
- Electric (2×)
- Ice (2×)
- Rock (2×)

**Evolution Paths**
- Hoothoot → Noctowl (Lv 20)

**Encounter Locations**
- Acrisia City — Grass (Night) (20%)
- Bronze Fields (North) — Grass (Night) (20%)
- Pythios Town — Grass (Night) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 30 |
| Defense | 30 |
| Sp. Atk | 36 |
| Sp. Def | 56 |
| Speed | 50 |
| Total | 262 |

**Notable Level-Up Moves**
- Foresight (Lv 1, Platinum)
- Growl (Lv 1, Gold Silver)
- Peck (Lv 1, Sword Shield)
- Tackle (Lv 1, Gold Silver)
- Hypnosis (Lv 4, Sun Moon)
- Echoed Voice (Lv 6, Sword Shield)
</div>
</div>

**Flavor Text**
> It always stands on one foot. It changes feet so fast, the movement can rarely be seen.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-hoothoot-163-panel-1">
## Noctowl (#164)
<details class="pokemon-card-container" open>
<summary>Noctowl overview</summary>
_Owl Pokémon_ • Types: Normal / Flying
Height: 1.6 m · Weight: 40.8 kg · Base EXP: 158

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Insomnia
- Keen Eye
- Tinted Lens *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.5×)
- Ground (0×)
- Bug (0.5×)
- Ghost (0×)

*Weak to*
- Electric (2×)
- Ice (2×)
- Rock (2×)

**Evolution Paths**
- Hoothoot → Noctowl (Lv 20)

**Encounter Locations**
- Sea of Vulcai — Grass (Night) (10%)
- Sofos City — Grass (Night) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 100 |
| Attack | 50 |
| Defense | 50 |
| Sp. Atk | 86 |
| Sp. Def | 96 |
| Speed | 70 |
| Total | 452 |

**Notable Level-Up Moves**
- Dream Eater (Lv 1, X Y)
- Echoed Voice (Lv 1, Sword Shield)
- Foresight (Lv 1, Gold Silver)
- Growl (Lv 1, Gold Silver)
- Hypnosis (Lv 1, Diamond Pearl)
- Peck (Lv 1, Gold Silver)
</div>
</div>

**Flavor Text**
> Its eyes are specially developed to enable it to see clearly even in murky darkness and minimal light.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-hoothoot-163-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-hoothoot-163-panel-0 { display: block; }
#pokemon-tabs-hoothoot-163-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-hoothoot-163-panel-1 { display: block; }
</style>
