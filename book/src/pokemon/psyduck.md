<div class="pokemon-tabs" id="pokemon-tabs-psyduck-54">
<input type="radio" name="pokemon-tabs-psyduck-54-group" id="pokemon-tabs-psyduck-54-tab-0" checked>
<label for="pokemon-tabs-psyduck-54-tab-0">Psyduck</label>
<input type="radio" name="pokemon-tabs-psyduck-54-group" id="pokemon-tabs-psyduck-54-tab-1">
<label for="pokemon-tabs-psyduck-54-tab-1">Golduck</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-psyduck-54-panel-0">
## Psyduck (#054)
<details class="pokemon-card-container" open>
<summary>Psyduck overview</summary>
_Duck Pokémon_ • Types: Water
Height: 0.8 m · Weight: 19.6 kg · Base EXP: 64

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Damp
- Cloud Nine
- Swift Swim *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Ice (0.5×)
- Steel (0.5×)

*Weak to*
- Electric (2×)
- Grass (2×)

**Evolution Paths**
- Psyduck → Golduck (Lv 33)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 50 |
| Attack | 52 |
| Defense | 48 |
| Sp. Atk | 65 |
| Sp. Def | 50 |
| Speed | 55 |
| Total | 320 |

**Notable Level-Up Moves**
- Bubble (Lv 1, Legends Arceus)
- Scratch (Lv 1, Red Blue)
- Tail Whip (Lv 1, Sword Shield)
- Water Sport (Lv 1, Ruby Sapphire)
- Water Gun (Lv 3, Sword Shield)
- Confusion (Lv 6, Sword Shield)
</div>
</div>

**Flavor Text**
> Psyduck is constantly beset by headaches. If the Pokémon lets its strange power erupt, apparently the pain subsides for a while.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-psyduck-54-panel-1">
## Golduck (#055)
<details class="pokemon-card-container" open>
<summary>Golduck overview</summary>
_Duck Pokémon_ • Types: Water
Height: 1.7 m · Weight: 76.6 kg · Base EXP: 175

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Damp
- Cloud Nine
- Swift Swim *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Ice (0.5×)
- Steel (0.5×)

*Weak to*
- Electric (2×)
- Grass (2×)

**Evolution Paths**
- Psyduck → Golduck (Lv 33)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 80 |
| Attack | 82 |
| Defense | 78 |
| Sp. Atk | 95 |
| Sp. Def | 80 |
| Speed | 85 |
| Total | 500 |

**Notable Level-Up Moves**
- Aqua Jet (Lv 1, Platinum)
- Bubble (Lv 1, Legends Arceus)
- Confusion (Lv 1, Gold Silver)
- Disable (Lv 1, Red Blue)
- Encore (Lv 1, Lets Go Pikachu Lets Go Eevee)
- Me First (Lv 1, Sun Moon)
</div>
</div>

**Flavor Text**
> This Pokémon lives in gently flowing rivers. It paddles through the water with its long limbs, putting its graceful swimming skills on display.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-psyduck-54-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-psyduck-54-panel-0 { display: block; }
#pokemon-tabs-psyduck-54-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-psyduck-54-panel-1 { display: block; }
</style>
