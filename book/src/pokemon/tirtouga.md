<div class="pokemon-tabs" id="pokemon-tabs-tirtouga-564">
<input type="radio" name="pokemon-tabs-tirtouga-564-group" id="pokemon-tabs-tirtouga-564-tab-0" checked>
<label for="pokemon-tabs-tirtouga-564-tab-0">Tirtouga</label>
<input type="radio" name="pokemon-tabs-tirtouga-564-group" id="pokemon-tabs-tirtouga-564-tab-1">
<label for="pokemon-tabs-tirtouga-564-tab-1">Carracosta</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-tirtouga-564-panel-0">
## Tirtouga (#564)
<details class="pokemon-card-container" open>
<summary>Tirtouga overview</summary>
_Prototurtle Pokémon_ • Types: Water / Rock • Egg Groups: Water1 / Water3 • Base EXP: 71

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Solid Rock
- Sturdy
- Swift Swim *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Fire (0.25×)
- Ice (0.5×)
- Poison (0.5×)
- Flying (0.5×)

*Weak to*
- Electric (2×)
- Grass (4×)
- Fighting (2×)
- Ground (2×)

**Evolution Paths**
- Tirtouga → Carracosta (Lv 37)

**Encounter Locations**
- Palati City — Fishing (60%)
- Palati City — Fishing (15%)
- Palati City — Surfing (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">54</span> |
| Attack | <span class="stat-value stat-mid">78</span> |
| Defense | <span class="stat-value stat-high">103</span> |
| Sp. Atk | <span class="stat-value stat-mid">53</span> |
| Sp. Def | <span class="stat-value stat-low">45</span> |
| Speed | <span class="stat-value stat-low">22</span> |
| Total | <span class="stat-value stat-mid">355</span> |
</div>
</div>

**Flavor Text**
> This Pokémon inhabited ancient seas. Although it can only crawl, it still comes up onto land in search of prey.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-tirtouga-564-panel-1">
## Carracosta (#565)
<details class="pokemon-card-container" open>
<summary>Carracosta overview</summary>
_Prototurtle Pokémon_ • Types: Water / Rock • Egg Groups: Water1 / Water3 • Base EXP: 173

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Solid Rock
- Sturdy
- Swift Swim *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Fire (0.25×)
- Ice (0.5×)
- Poison (0.5×)
- Flying (0.5×)

*Weak to*
- Electric (2×)
- Grass (4×)
- Fighting (2×)
- Ground (2×)

**Evolution Paths**
- Tirtouga → Carracosta (Lv 37)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">74</span> |
| Attack | <span class="stat-value stat-high">108</span> |
| Defense | <span class="stat-value stat-high">133</span> |
| Sp. Atk | <span class="stat-value stat-mid">83</span> |
| Sp. Def | <span class="stat-value stat-mid">65</span> |
| Speed | <span class="stat-value stat-low">32</span> |
| Total | <span class="stat-value stat-mid">495</span> |
</div>
</div>

**Flavor Text**
> Carracosta completely devours its prey—bones, shells, and all. Because of this, Carracosta’s own shell grows thick and sturdy.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-tirtouga-564-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-tirtouga-564-panel-0 { display: block; }
#pokemon-tabs-tirtouga-564-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-tirtouga-564-panel-1 { display: block; }
</style>
