<div class="pokemon-tabs" id="pokemon-tabs-zorua-570">
<input type="radio" name="pokemon-tabs-zorua-570-group" id="pokemon-tabs-zorua-570-tab-0" checked>
<label for="pokemon-tabs-zorua-570-tab-0">Zorua</label>
<input type="radio" name="pokemon-tabs-zorua-570-group" id="pokemon-tabs-zorua-570-tab-1">
<label for="pokemon-tabs-zorua-570-tab-1">Zoroark</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-zorua-570-panel-0">
## Zorua (#570)
<details class="pokemon-card-container" open>
<summary>Zorua overview</summary>
_Tricky Fox Pokémon_ • Types: Dark
Height: 0.7 m · Weight: 12.5 kg · Base EXP: 66

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Illusion

**Type Matchups**

*Resists / Immune to*
- Psychic (0×)
- Ghost (0.5×)
- Dark (0.5×)

*Weak to*
- Fighting (2×)
- Bug (2×)
- Fairy (2×)

**Evolution Paths**
- Zorua → Zoroark (Lv 30)

**Encounter Locations**
- Erinys Path (West) — Grass (Day) (10%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 40 |
| Attack | 65 |
| Defense | 40 |
| Sp. Atk | 80 |
| Sp. Def | 40 |
| Speed | 65 |
| Total | 330 |

**Notable Level-Up Moves**
- Leer (Lv 1, Black White)
- Scratch (Lv 1, Black White)
- Torment (Lv 4, Sword Shield)
- Pursuit (Lv 5, Black White)
- Hone Claws (Lv 8, Sword Shield)
- Fake Tears (Lv 9, Black White)
</div>
</div>

**Flavor Text**
> Zorua is a timid Pokémon. This disposition seems to be what led to the development of Zorua’s ability to take on the forms of other creatures.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-zorua-570-panel-1">
## Zoroark (#571)
<details class="pokemon-card-container" open>
<summary>Zoroark overview</summary>
_Illusion Fox Pokémon_ • Types: Dark
Height: 1.6 m · Weight: 81.1 kg · Base EXP: 179

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Illusion

**Type Matchups**

*Resists / Immune to*
- Psychic (0×)
- Ghost (0.5×)
- Dark (0.5×)

*Weak to*
- Fighting (2×)
- Bug (2×)
- Fairy (2×)

**Evolution Paths**
- Zorua → Zoroark (Lv 30)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 60 |
| Attack | 105 |
| Defense | 60 |
| Sp. Atk | 120 |
| Sp. Def | 60 |
| Speed | 105 |
| Total | 510 |

**Notable Level-Up Moves**
- Hone Claws (Lv 1, Black White)
- Imprison (Lv 1, X Y)
- Leer (Lv 1, Black White)
- Night Daze (Lv 1, X Y)
- Night Slash (Lv 1, Sun Moon)
- Pursuit (Lv 1, Black White)
</div>
</div>

**Flavor Text**
> This Pokémon cares deeply about others of its kind, and it will conjure terrifying illusions to keep its den and pack safe.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-zorua-570-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-zorua-570-panel-0 { display: block; }
#pokemon-tabs-zorua-570-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-zorua-570-panel-1 { display: block; }
</style>
