<div class="pokemon-tabs" id="pokemon-tabs-whiscash-340">
<input type="radio" name="pokemon-tabs-whiscash-340-group" id="pokemon-tabs-whiscash-340-tab-0">
<label for="pokemon-tabs-whiscash-340-tab-0">Barboach</label>
<input type="radio" name="pokemon-tabs-whiscash-340-group" id="pokemon-tabs-whiscash-340-tab-1" checked>
<label for="pokemon-tabs-whiscash-340-tab-1">Whiscash</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-whiscash-340-panel-0">
## Barboach (#339)
<details class="pokemon-card-container" open>
<summary>Barboach overview</summary>
_Whiskers Pokémon_ • Types: Water / Ground
Height: 0.4 m · Weight: 1.9 kg · Base EXP: 58

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Oblivious
- Anticipation
- Hydration *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Electric (0×)
- Poison (0.5×)
- Rock (0.5×)
- Steel (0.5×)

*Weak to*
- Grass (4×)

**Evolution Paths**
- Barboach → Whiscash (Lv 30)

**Encounter Locations**
- Kalami City — Fishing (40%)
- Kalami City — Surfing (10%)
- Riverwalk Trail (South) — Fishing (40%)
- Riverwalk Trail (South) — Surfing (10%)
- Wanderer's Woods (North) — Fishing (30%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 50 |
| Attack | 48 |
| Defense | 43 |
| Sp. Atk | 46 |
| Sp. Def | 41 |
| Speed | 60 |
| Total | 288 |

**Notable Level-Up Moves**
- Mud Slap (Lv 1, Ruby Sapphire)
- Water Gun (Lv 1, Sword Shield)
- Mud Sport (Lv 6, Ruby Sapphire)
- Rest (Lv 6, Sword Shield)
- Snore (Lv 6, Sword Shield)
- Water Pulse (Lv 6, Legends Arceus)
</div>
</div>

**Flavor Text**
> Its slimy body is hard to grasp. In one region, it is said to have been born from hardened mud.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-whiscash-340-panel-1">
## Whiscash (#340)
<details class="pokemon-card-container" open>
<summary>Whiscash overview</summary>
_Whiskers Pokémon_ • Types: Water / Ground
Height: 0.9 m · Weight: 23.6 kg · Base EXP: 164

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Oblivious
- Anticipation
- Hydration *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Electric (0×)
- Poison (0.5×)
- Rock (0.5×)
- Steel (0.5×)

*Weak to*
- Grass (4×)

**Evolution Paths**
- Barboach → Whiscash (Lv 30)

**Encounter Locations**
- Riverwalk Trail (South) — Fishing (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 110 |
| Attack | 78 |
| Defense | 73 |
| Sp. Atk | 76 |
| Sp. Def | 71 |
| Speed | 60 |
| Total | 468 |

**Notable Level-Up Moves**
- Belch (Lv 1, Sun Moon)
- Mud Slap (Lv 1, Ruby Sapphire)
- Mud Sport (Lv 1, Ruby Sapphire)
- Rest (Lv 1, Sword Shield)
- Snore (Lv 1, Sword Shield)
- Thrash (Lv 1, Sun Moon)
</div>
</div>

**Flavor Text**
> It makes its nest at the bottom of swamps. It will eat anything—if it is alive, Whiscash will eat it.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-whiscash-340-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-whiscash-340-panel-0 { display: block; }
#pokemon-tabs-whiscash-340-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-whiscash-340-panel-1 { display: block; }
</style>
