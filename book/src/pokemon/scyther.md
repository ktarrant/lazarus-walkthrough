<div class="pokemon-tabs" id="pokemon-tabs-scyther-123">
<input type="radio" name="pokemon-tabs-scyther-123-group" id="pokemon-tabs-scyther-123-tab-0" checked>
<label for="pokemon-tabs-scyther-123-tab-0">Scyther</label>
<input type="radio" name="pokemon-tabs-scyther-123-group" id="pokemon-tabs-scyther-123-tab-1">
<label for="pokemon-tabs-scyther-123-tab-1">Scizor</label>
<input type="radio" name="pokemon-tabs-scyther-123-group" id="pokemon-tabs-scyther-123-tab-2">
<label for="pokemon-tabs-scyther-123-tab-2">Kleavor</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-scyther-123-panel-0">
## Scyther (#123)
<details class="pokemon-card-container" open>
<summary>Scyther overview</summary>
_Mantis Pokémon_ • Types: Bug / Flying • Egg Groups: Bug • Base EXP: 100

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Swarm
- Technician
- Steadfast *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Grass (0.25×)
- Fighting (0.25×)
- Ground (0×)
- Bug (0.5×)

*Weak to*
- Fire (2×)
- Electric (2×)
- Ice (2×)
- Flying (2×)
- Rock (4×)

**Evolution Paths**
- Scyther → Scizor (Hold Metal Coat)
- Scyther → Kleavor (Use Black Augurite)

**Encounter Locations**
- Sea of Asteri (West) — Grass (Day) (8%)
- Sea of Asteri (West) — Grass (Night) (4%)
- Sea of Vulcai — Grass (Day) (20%)
- Sea of Vulcai — Grass (Night) (20%)
- Wakewater Isle — Grass (Day) (5%)
- Wakewater Isle — Grass (Night) (5%)
- Wanderer's Woods (South) — Grass (Day) (2%)
- Wanderer's Woods (South) — Grass (Night) (2%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">70</span> |
| Attack | <span class="stat-value stat-high">110</span> |
| Defense | <span class="stat-value stat-mid">80</span> |
| Sp. Atk | <span class="stat-value stat-mid">55</span> |
| Sp. Def | <span class="stat-value stat-mid">80</span> |
| Speed | <span class="stat-value stat-high">105</span> |
| Total | <span class="stat-value stat-mid">500</span> |

**Notable Level-Up Moves**
- Leer (Lv 1)
- Quick Attack (Lv 1)
- Focus Energy (Lv 6)
- Pursuit (Lv 11)
- False Swipe (Lv 16)
- Agility (Lv 21)
- Wing Attack (Lv 26)
- Slash (Lv 31)
- Swords Dance (Lv 36)
- Double Team (Lv 41)
- Fury Cutter (Lv 46)
</div>
</div>

**Flavor Text**
> As Scyther fights more and more battles, its scythes become sharper and sharper. With a single slice, Scyther can fell a massive tree.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-scyther-123-panel-1">
## Scizor (#212)
<details class="pokemon-card-container" open>
<summary>Scizor overview</summary>
_Pincer Pokémon_ • Types: Bug / Steel • Egg Groups: Bug • Base EXP: 175

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Swarm
- Technician
- Light Metal *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Grass (0.25×)
- Ice (0.5×)
- Poison (0×)
- Psychic (0.5×)
- Bug (0.5×)
- Dragon (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Fire (4×)

**Evolution Paths**
- Scyther → Scizor (Hold Metal Coat)
- Scyther → Kleavor (Use Black Augurite)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">70</span> |
| Attack | <span class="stat-value stat-high">130</span> |
| Defense | <span class="stat-value stat-high">100</span> |
| Sp. Atk | <span class="stat-value stat-mid">55</span> |
| Sp. Def | <span class="stat-value stat-mid">80</span> |
| Speed | <span class="stat-value stat-mid">65</span> |
| Total | <span class="stat-value stat-mid">500</span> |

**Notable Level-Up Moves**
- Leer (Lv 1)
- Quick Attack (Lv 1)
- Focus Energy (Lv 6)
- Pursuit (Lv 11)
- False Swipe (Lv 16)
- Agility (Lv 21)
- Metal Claw (Lv 26)
- Slash (Lv 31)
- Swords Dance (Lv 36)
- Double Team (Lv 41)
- Fury Cutter (Lv 46)
</div>
</div>

**Flavor Text**
> Bulky pincers account for one third of Scizor’s body weight. A single swing of one of these pincers will crush a boulder completely.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-scyther-123-panel-2">
## Kleavor (#900)
<details class="pokemon-card-container" open>
<summary>Kleavor overview</summary>
_Axe Pokémon_ • Types: Bug / Rock • Egg Groups: Bug • Base EXP: 175

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Swarm
- Sheer Force
- Sharpness *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0.5×)
- Poison (0.5×)

*Weak to*
- Water (2×)
- Rock (2×)
- Steel (2×)

**Evolution Paths**
- Scyther → Scizor (Hold Metal Coat)
- Scyther → Kleavor (Use Black Augurite)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">70</span> |
| Attack | <span class="stat-value stat-high">135</span> |
| Defense | <span class="stat-value stat-high">95</span> |
| Sp. Atk | <span class="stat-value stat-low">45</span> |
| Sp. Def | <span class="stat-value stat-mid">70</span> |
| Speed | <span class="stat-value stat-mid">85</span> |
| Total | <span class="stat-value stat-mid">500</span> |
</div>
</div>

**Flavor Text**
> A violent creature that fells towering trees with its crude axes and shields itself with hard stone. If one should chance upon this Pokémon in the wilds, one's only recourse is to flee.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-scyther-123-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-scyther-123-panel-0 { display: block; }
#pokemon-tabs-scyther-123-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-scyther-123-panel-1 { display: block; }
#pokemon-tabs-scyther-123-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-scyther-123-panel-2 { display: block; }
</style>
