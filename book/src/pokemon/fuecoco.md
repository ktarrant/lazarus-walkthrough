<div class="pokemon-tabs" id="pokemon-tabs-fuecoco-909">
<input type="radio" name="pokemon-tabs-fuecoco-909-group" id="pokemon-tabs-fuecoco-909-tab-0" checked>
<label for="pokemon-tabs-fuecoco-909-tab-0">Fuecoco</label>
<input type="radio" name="pokemon-tabs-fuecoco-909-group" id="pokemon-tabs-fuecoco-909-tab-1">
<label for="pokemon-tabs-fuecoco-909-tab-1">Crocalor</label>
<input type="radio" name="pokemon-tabs-fuecoco-909-group" id="pokemon-tabs-fuecoco-909-tab-2">
<label for="pokemon-tabs-fuecoco-909-tab-2">Skeledirge</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-fuecoco-909-panel-0">
## Fuecoco (#909)
<details class="pokemon-card-container" open>
<summary>Fuecoco overview</summary>
_Fire Croc Pokémon_ • Types: Fire
Height: 0.4 m · Weight: 9.8 kg · Base EXP: 62

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Blaze
- Unaware *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Grass (0.5×)
- Ice (0.5×)
- Bug (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Water (2×)
- Ground (2×)
- Rock (2×)

**Evolution Paths**
- Fuecoco → Crocalor (Lv 16) → Skeledirge (Lv 36)

**Encounter Locations**
- Sea of Vulcai — Grass (Day) (5%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 67 |
| Attack | 45 |
| Defense | 59 |
| Sp. Atk | 63 |
| Sp. Def | 40 |
| Speed | 36 |
| Total | 310 |

**Notable Level-Up Moves**
- Ember (Lv 1, Scarlet Violet)
- Leer (Lv 1, Scarlet Violet)
- Tackle (Lv 1, Scarlet Violet)
- Round (Lv 7, Scarlet Violet)
- Bite (Lv 12, Scarlet Violet)
- Incinerate (Lv 15, Scarlet Violet)
</div>
</div>

**Flavor Text**
> Its flame sac is small, so energy is always leaking out. This energy is released from the dent atop Fuecoco’s head and flickers to and fro.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-fuecoco-909-panel-1">
## Crocalor (#910)
<details class="pokemon-card-container" open>
<summary>Crocalor overview</summary>
_Fire Croc Pokémon_ • Types: Fire
Height: 1.0 m · Weight: 30.7 kg · Base EXP: 144

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Blaze
- Unaware *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Grass (0.5×)
- Ice (0.5×)
- Bug (0.5×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Water (2×)
- Ground (2×)
- Rock (2×)

**Evolution Paths**
- Fuecoco → Crocalor (Lv 16) → Skeledirge (Lv 36)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 81 |
| Attack | 55 |
| Defense | 78 |
| Sp. Atk | 90 |
| Sp. Def | 58 |
| Speed | 49 |
| Total | 411 |

**Notable Level-Up Moves**
- Ember (Lv 1, Scarlet Violet)
- Leer (Lv 1, Scarlet Violet)
- Tackle (Lv 1, Scarlet Violet)
- Lick (Lv 7, Scarlet Violet)
- Round (Lv 10, Scarlet Violet)
- Bite (Lv 12, Scarlet Violet)
</div>
</div>

**Flavor Text**
> The valve in Crocalor’s flame sac is closely connected to its vocal cords. This Pokémon utters a guttural cry as it spews flames every which way.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-fuecoco-909-panel-2">
## Skeledirge (#911)
<details class="pokemon-card-container" open>
<summary>Skeledirge overview</summary>
_Singer Pokémon_ • Types: Fire / Ghost
Height: 1.6 m · Weight: 326.5 kg · Base EXP: 265

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Blaze
- Unaware *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Fire (0.5×)
- Grass (0.5×)
- Ice (0.5×)
- Fighting (0×)
- Poison (0.5×)
- Bug (0.25×)
- Steel (0.5×)
- Fairy (0.5×)

*Weak to*
- Water (2×)
- Ground (2×)
- Rock (2×)
- Ghost (2×)
- Dark (2×)

**Evolution Paths**
- Fuecoco → Crocalor (Lv 16) → Skeledirge (Lv 36)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 104 |
| Attack | 75 |
| Defense | 100 |
| Sp. Atk | 110 |
| Sp. Def | 75 |
| Speed | 66 |
| Total | 530 |

**Notable Level-Up Moves**
- Ember (Lv 1, Scarlet Violet)
- Leer (Lv 1, Scarlet Violet)
- Tackle (Lv 1, Scarlet Violet)
- Lick (Lv 7, Scarlet Violet)
- Round (Lv 10, Scarlet Violet)
- Scary Face (Lv 12, Scarlet Violet)
</div>
</div>

**Flavor Text**
> Skeledirge’s gentle singing soothes the souls of all that hear it. It burns its enemies to a crisp with flames of over 5,400 degrees Fahrenheit.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-fuecoco-909-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-fuecoco-909-panel-0 { display: block; }
#pokemon-tabs-fuecoco-909-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-fuecoco-909-panel-1 { display: block; }
#pokemon-tabs-fuecoco-909-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-fuecoco-909-panel-2 { display: block; }
</style>
