<div class="pokemon-tabs" id="pokemon-tabs-dragonair-148">
<input type="radio" name="pokemon-tabs-dragonair-148-group" id="pokemon-tabs-dragonair-148-tab-0">
<label for="pokemon-tabs-dragonair-148-tab-0">Dratini</label>
<input type="radio" name="pokemon-tabs-dragonair-148-group" id="pokemon-tabs-dragonair-148-tab-1" checked>
<label for="pokemon-tabs-dragonair-148-tab-1">Dragonair</label>
<input type="radio" name="pokemon-tabs-dragonair-148-group" id="pokemon-tabs-dragonair-148-tab-2">
<label for="pokemon-tabs-dragonair-148-tab-2">Dragonite</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-dragonair-148-panel-0">
## Dratini (#147)
<details class="pokemon-card-container" open>
<summary>Dratini overview</summary>
_Dragon Pokémon_ • Types: Dragon • Egg Groups: Water1 / Dragon • Base EXP: 60

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shed Skin
- Marvel Scale *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)

*Weak to*
- Ice (2×)
- Dragon (2×)
- Fairy (2×)

**Evolution Paths**
- Dratini → Dragonair (Lv 30) → Dragonite (Lv 55)

**Encounter Locations**
- Fresco Isles — Fishing (5%)
- Fresco Isles — Surfing (5%)
- Pythios Cemetery — Fishing (20%)
- Pythios Cemetery — Fishing (15%)
- Péntepetal City — Surfing (9%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-low">41</span> |
| Attack | <span class="stat-value stat-mid">64</span> |
| Defense | <span class="stat-value stat-low">45</span> |
| Sp. Atk | <span class="stat-value stat-low">50</span> |
| Sp. Def | <span class="stat-value stat-low">50</span> |
| Speed | <span class="stat-value stat-low">50</span> |
| Total | <span class="stat-value stat-low">300</span> |

**Notable Level-Up Moves**
- Leer (Lv 1)
- Wrap (Lv 1)
- Twister (Lv 5)
- Thunder Wave (Lv 10)
- Dragon Tail (Lv 15)
- Agility (Lv 20)
- Slam (Lv 25)
- Aqua Tail (Lv 31)
- Dragon Rush (Lv 35)
- Safeguard (Lv 40)
- Rain Dance (Lv 45)
- Dragon Dance (Lv 50)
- Outrage (Lv 55)
- Hyper Beam (Lv 60)
</div>
</div>

**Flavor Text**
> Dratini dwells near bodies of rapidly flowing water, such as the plunge pools of waterfalls. As it grows, Dratini will shed its skin many times.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-dragonair-148-panel-1">
## Dragonair (#148)
<details class="pokemon-card-container" open>
<summary>Dragonair overview</summary>
_Dragon Pokémon_ • Types: Dragon • Egg Groups: Water1 / Dragon • Base EXP: 147

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Shed Skin
- Marvel Scale *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Electric (0.5×)
- Grass (0.5×)

*Weak to*
- Ice (2×)
- Dragon (2×)
- Fairy (2×)

**Evolution Paths**
- Dratini → Dragonair (Lv 30) → Dragonite (Lv 55)

**Encounter Locations**
- Davosi Straits — Surfing (5%)
- Pollen Road — Surfing (5%)
- Pythios Cemetery — Fishing (5%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">61</span> |
| Attack | <span class="stat-value stat-mid">84</span> |
| Defense | <span class="stat-value stat-mid">65</span> |
| Sp. Atk | <span class="stat-value stat-mid">70</span> |
| Sp. Def | <span class="stat-value stat-mid">70</span> |
| Speed | <span class="stat-value stat-mid">70</span> |
| Total | <span class="stat-value stat-mid">420</span> |

**Notable Level-Up Moves**
- Leer (Lv 1)
- Thunder Wave (Lv 1)
- Twister (Lv 1)
- Wrap (Lv 1)
- Dragon Tail (Lv 15)
- Agility (Lv 20)
- Slam (Lv 25)
- Aqua Tail (Lv 33)
- Dragon Rush (Lv 39)
- Safeguard (Lv 46)
- Rain Dance (Lv 53)
- Dragon Dance (Lv 60)
- Outrage (Lv 67)
- Hyper Beam (Lv 74)
</div>
</div>

**Flavor Text**
> This Pokémon lives in pristine oceans and lakes. It can control the weather, and it uses this power to fly into the sky, riding on the wind.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-dragonair-148-panel-2">
## Dragonite (#149)
<details class="pokemon-card-container" open>
<summary>Dragonite overview</summary>
_Dragon Pokémon_ • Types: Dragon / Flying • Egg Groups: Water1 / Dragon • Base EXP: 270

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Inner Focus
- Multiscale *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Fire (0.5×)
- Water (0.5×)
- Grass (0.25×)
- Fighting (0.5×)
- Ground (0×)
- Bug (0.5×)

*Weak to*
- Ice (4×)
- Rock (2×)
- Dragon (2×)
- Fairy (2×)

**Evolution Paths**
- Dratini → Dragonair (Lv 30) → Dragonite (Lv 55)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-high">91</span> |
| Attack | <span class="stat-value stat-high">134</span> |
| Defense | <span class="stat-value stat-high">95</span> |
| Sp. Atk | <span class="stat-value stat-high">100</span> |
| Sp. Def | <span class="stat-value stat-high">100</span> |
| Speed | <span class="stat-value stat-mid">80</span> |
| Total | <span class="stat-value stat-high">600</span> |

**Notable Level-Up Moves**
- Extreme Speed (Lv 1)
- Fire Punch (Lv 1)
- Hurricane (Lv 1)
- Leer (Lv 1)
- Roost (Lv 1)
- Thunder Punch (Lv 1)
- Thunder Wave (Lv 1)
- Twister (Lv 1)
- Wing Attack (Lv 1)
- Wrap (Lv 1)
- Dragon Tail (Lv 15)
- Agility (Lv 20)
- Slam (Lv 25)
- Aqua Tail (Lv 33)
- Dragon Rush (Lv 39)
- Outrage (Lv 41)
- Safeguard (Lv 46)
- Rain Dance (Lv 53)
- Dragon Dance (Lv 62)
- Hyper Beam (Lv 80)
</div>
</div>

**Flavor Text**
> It’s a kindhearted Pokémon. If it spots a drowning person or Pokémon, Dragonite simply must help them.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-dragonair-148-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-dragonair-148-panel-0 { display: block; }
#pokemon-tabs-dragonair-148-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-dragonair-148-panel-1 { display: block; }
#pokemon-tabs-dragonair-148-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-dragonair-148-panel-2 { display: block; }
</style>
