<div class="pokemon-tabs" id="pokemon-tabs-wigglytuff-40">
<input type="radio" name="pokemon-tabs-wigglytuff-40-group" id="pokemon-tabs-wigglytuff-40-tab-0">
<label for="pokemon-tabs-wigglytuff-40-tab-0">Igglybuff</label>
<input type="radio" name="pokemon-tabs-wigglytuff-40-group" id="pokemon-tabs-wigglytuff-40-tab-1">
<label for="pokemon-tabs-wigglytuff-40-tab-1">Jigglypuff</label>
<input type="radio" name="pokemon-tabs-wigglytuff-40-group" id="pokemon-tabs-wigglytuff-40-tab-2" checked>
<label for="pokemon-tabs-wigglytuff-40-tab-2">Wigglytuff</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-wigglytuff-40-panel-0">
## Igglybuff (#174)
<details class="pokemon-card-container" open>
<summary>Igglybuff overview</summary>
_Balloon Pokémon_ • Types: Normal / Fairy • Egg Groups: No Eggs • Base EXP: 42

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Cute Charm
- Competitive
- Friend Guard *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Bug (0.5×)
- Ghost (0×)
- Dragon (0×)
- Dark (0.5×)

*Weak to*
- Poison (2×)
- Steel (2×)

**Evolution Paths**
- Igglybuff → Jigglypuff (Friendship 160) → Wigglytuff (Use Moon Stone)

**Encounter Locations**
- Kipos Town — Grass (Day) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-mid">90</span> |
| Attack | <span class="stat-value stat-low">30</span> |
| Defense | <span class="stat-value stat-low">15</span> |
| Sp. Atk | <span class="stat-value stat-low">40</span> |
| Sp. Def | <span class="stat-value stat-low">20</span> |
| Speed | <span class="stat-value stat-low">15</span> |
| Total | <span class="stat-value stat-low">210</span> |

**Notable Level-Up Moves**
- Copycat (Lv 1)
- Pound (Lv 1)
- Sing (Lv 1)
- Defense Curl (Lv 4)
- Sweet Kiss (Lv 8)
- Disarming Voice (Lv 12)
- Disable (Lv 16)
- Charm (Lv 20)
</div>
</div>

**Flavor Text**
> Igglybuff loves to sing. Its marshmallow-like body gives off a faint sweet smell.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-wigglytuff-40-panel-1">
## Jigglypuff (#039)
<details class="pokemon-card-container" open>
<summary>Jigglypuff overview</summary>
_Balloon Pokémon_ • Types: Normal / Fairy • Egg Groups: Fairy • Base EXP: 95

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Cute Charm
- Competitive
- Friend Guard *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Bug (0.5×)
- Ghost (0×)
- Dragon (0×)
- Dark (0.5×)

*Weak to*
- Poison (2×)
- Steel (2×)

**Evolution Paths**
- Igglybuff → Jigglypuff (Friendship 160) → Wigglytuff (Use Moon Stone)

**Encounter Locations**
- Myrrini Island — Grass (Day) (10%)
- Port Pello — Grass (Day) (20%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-high">115</span> |
| Attack | <span class="stat-value stat-low">45</span> |
| Defense | <span class="stat-value stat-low">20</span> |
| Sp. Atk | <span class="stat-value stat-low">45</span> |
| Sp. Def | <span class="stat-value stat-low">25</span> |
| Speed | <span class="stat-value stat-low">20</span> |
| Total | <span class="stat-value stat-low">270</span> |

**Notable Level-Up Moves**
- Charm (Lv 1)
- Copycat (Lv 1)
- Defense Curl (Lv 1)
- Disable (Lv 1)
- Disarming Voice (Lv 1)
- Pound (Lv 1)
- Sing (Lv 1)
- Sweet Kiss (Lv 1)
- Echoed Voice (Lv 4)
- Covet (Lv 8)
- Spit Up (Lv 12)
- Stockpile (Lv 12)
- Swallow (Lv 12)
- Round (Lv 16)
- Rest (Lv 20)
- Body Slam (Lv 24)
- Mimic (Lv 28)
- Gyro Ball (Lv 32)
- Hyper Voice (Lv 36)
- Play Rough (Lv 40)
- Double Edge (Lv 44)
</div>
</div>

**Flavor Text**
> Jigglypuff has top-notch lung capacity, even by comparison to other Pokémon. It won’t stop singing its lullabies until its foes fall asleep.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-wigglytuff-40-panel-2">
## Wigglytuff (#040)
<details class="pokemon-card-container" open>
<summary>Wigglytuff overview</summary>
_Balloon Pokémon_ • Types: Normal / Fairy • Egg Groups: Fairy • Base EXP: 196

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Cute Charm
- Competitive
- Frisk *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Bug (0.5×)
- Ghost (0×)
- Dragon (0×)
- Dark (0.5×)

*Weak to*
- Poison (2×)
- Steel (2×)

**Evolution Paths**
- Igglybuff → Jigglypuff (Friendship 160) → Wigglytuff (Use Moon Stone)

**Encounter Locations**
- Port Pello — Grass (Day) (5%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | <span class="stat-value stat-high">140</span> |
| Attack | <span class="stat-value stat-mid">70</span> |
| Defense | <span class="stat-value stat-low">45</span> |
| Sp. Atk | <span class="stat-value stat-mid">85</span> |
| Sp. Def | <span class="stat-value stat-low">50</span> |
| Speed | <span class="stat-value stat-low">45</span> |
| Total | <span class="stat-value stat-mid">435</span> |

**Notable Level-Up Moves**
- Body Slam (Lv 1)
- Charm (Lv 1)
- Copycat (Lv 1)
- Covet (Lv 1)
- Defense Curl (Lv 1)
- Disable (Lv 1)
- Disarming Voice (Lv 1)
- Double Edge (Lv 1)
- Echoed Voice (Lv 1)
- Gyro Ball (Lv 1)
- Hyper Voice (Lv 1)
- Mimic (Lv 1)
- Play Rough (Lv 1)
- Pound (Lv 1)
- Rest (Lv 1)
- Round (Lv 1)
- Sing (Lv 1)
- Spit Up (Lv 1)
- Stockpile (Lv 1)
- Swallow (Lv 1)
- Sweet Kiss (Lv 1)
</div>
</div>

**Flavor Text**
> The more air it takes in, the more it inflates. If opponents catch it in a bad mood, it will inflate itself to an enormous size to intimidate them.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-wigglytuff-40-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-wigglytuff-40-panel-0 { display: block; }
#pokemon-tabs-wigglytuff-40-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-wigglytuff-40-panel-1 { display: block; }
#pokemon-tabs-wigglytuff-40-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-wigglytuff-40-panel-2 { display: block; }
</style>
