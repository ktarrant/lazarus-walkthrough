<div class="pokemon-tabs" id="pokemon-tabs-snorunt-361">
<input type="radio" name="pokemon-tabs-snorunt-361-group" id="pokemon-tabs-snorunt-361-tab-0" checked>
<label for="pokemon-tabs-snorunt-361-tab-0">Snorunt</label>
<input type="radio" name="pokemon-tabs-snorunt-361-group" id="pokemon-tabs-snorunt-361-tab-1">
<label for="pokemon-tabs-snorunt-361-tab-1">Glalie</label>
<input type="radio" name="pokemon-tabs-snorunt-361-group" id="pokemon-tabs-snorunt-361-tab-2">
<label for="pokemon-tabs-snorunt-361-tab-2">Froslass</label>
<div class="pokemon-tab-panels">
<div class="pokemon-tab-panel" id="pokemon-tabs-snorunt-361-panel-0">
## Snorunt (#361)
<details class="pokemon-card-container" open>
<summary>Snorunt overview</summary>
_Snow Hat Pokémon_ • Types: Ice
Height: 0.7 m · Weight: 16.8 kg · Base EXP: 60

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Inner Focus
- Ice Body
- Moody *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Ice (0.5×)

*Weak to*
- Fire (2×)
- Fighting (2×)
- Rock (2×)
- Steel (2×)

**Evolution Paths**
- Snorunt → Glalie (Lv 42)
- Snorunt → Froslass (Use Dawn Stone)

**Encounter Locations**
- Froslass Cavern BF1 — Grass (Day) (30%)
- Froslass Cavern BF2 — Grass (Day) (30%)
- Froslass Cavern F1 — Grass (Day) (30%)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 50 |
| Attack | 50 |
| Defense | 50 |
| Sp. Atk | 50 |
| Sp. Def | 50 |
| Speed | 50 |
| Total | 300 |

**Notable Level-Up Moves**
- Astonish (Lv 1, Sword Shield)
- Headbutt (Lv 1, Scarlet Violet)
- Leer (Lv 1, Ruby Sapphire)
- Powder Snow (Lv 1, Ruby Sapphire)
- Double Team (Lv 4, Diamond Pearl)
- Ice Shard (Lv 6, Legends Arceus)
</div>
</div>

**Flavor Text**
> It’s said that if they are seen at midnight, they’ll cause heavy snow. They eat snow and ice to survive.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-snorunt-361-panel-1">
## Glalie (#362)
<details class="pokemon-card-container" open>
<summary>Glalie overview</summary>
_Face Pokémon_ • Types: Ice
Height: 1.5 m · Weight: 256.5 kg · Base EXP: 168

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Inner Focus
- Ice Body
- Moody *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Ice (0.5×)

*Weak to*
- Fire (2×)
- Fighting (2×)
- Rock (2×)
- Steel (2×)

**Evolution Paths**
- Snorunt → Glalie (Lv 42)
- Snorunt → Froslass (Use Dawn Stone)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 80 |
| Attack | 80 |
| Defense | 80 |
| Sp. Atk | 80 |
| Sp. Def | 80 |
| Speed | 80 |
| Total | 480 |

**Notable Level-Up Moves**
- Astonish (Lv 1, Sword Shield)
- Bite (Lv 1, Ruby Sapphire)
- Double Team (Lv 1, Ruby Sapphire)
- Freeze Dry (Lv 1, Sun Moon)
- Headbutt (Lv 1, Scarlet Violet)
- Ice Ball (Lv 1, Legends Arceus)
</div>
</div>

**Flavor Text**
> It has a body of ice that won’t melt, even with fire. It can instantly freeze moisture in the atmosphere.
</details>
</div>
<div class="pokemon-tab-panel" id="pokemon-tabs-snorunt-361-panel-2">
## Froslass (#478)
<details class="pokemon-card-container" open>
<summary>Froslass overview</summary>
_Snow Land Pokémon_ • Types: Ice / Ghost
Height: 1.3 m · Weight: 26.6 kg · Base EXP: 168

<div class="pokemon-card">
<div class="card-column">
**Abilities**
- Snow Cloak
- Cursed Body *(Hidden)*

**Type Matchups**

*Resists / Immune to*
- Normal (0×)
- Ice (0.5×)
- Fighting (0×)
- Poison (0.5×)
- Bug (0.5×)

*Weak to*
- Fire (2×)
- Rock (2×)
- Ghost (2×)
- Dark (2×)
- Steel (2×)

**Evolution Paths**
- Snorunt → Glalie (Lv 42)
- Snorunt → Froslass (Use Dawn Stone)
</div>
<div class="card-column">
**Base Stats**

| Stat | Value |
| --- | --- |
| HP | 70 |
| Attack | 80 |
| Defense | 70 |
| Sp. Atk | 80 |
| Sp. Def | 70 |
| Speed | 110 |
| Total | 480 |

**Notable Level-Up Moves**
- Astonish (Lv 1, Diamond Pearl)
- Bite (Lv 1, Sword Shield)
- Crunch (Lv 1, Sword Shield)
- Destiny Bond (Lv 1, X Y)
- Double Team (Lv 1, Diamond Pearl)
- Headbutt (Lv 1, Sword Shield)
</div>
</div>

**Flavor Text**
> After a woman met her end on a snowy mountain, her regrets lingered on. From them, this Pokémon was born. Its favorite food is frozen souls.
</details>
</div>
</div>
</div>
<style>
#pokemon-tabs-snorunt-361-tab-0:checked ~ .pokemon-tab-panels #pokemon-tabs-snorunt-361-panel-0 { display: block; }
#pokemon-tabs-snorunt-361-tab-1:checked ~ .pokemon-tab-panels #pokemon-tabs-snorunt-361-panel-1 { display: block; }
#pokemon-tabs-snorunt-361-tab-2:checked ~ .pokemon-tab-panels #pokemon-tabs-snorunt-361-panel-2 { display: block; }
</style>
